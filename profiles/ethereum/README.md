...
docker compose down -v --remove-orphans # 停止 + genesis / chain データ破棄
```

`--remove-orphans` は必須。collector の `addNode`/`addWorkbench`
(GUI からのノード追加・ワークベンチ追加)で動的に作成したコンテナは
docker-compose.yml に対応するサービス定義を持たないため、これを付けないと
削除されずに残り、ネットワークの削除も失敗する(Issue #359)。
...
`docker compose down -v --remove-orphans` で共有ボリュームを含めて
作り直す必要がある。`down`(`-v` なし)だけでは古い genesis が再利用され、
変更が反映されない。
...
まっさらな chain で始めたいときは `docker compose down -v --remove-orphans`
で共有ボリュームを破棄する。マーカーごと消えるため、次回起動時に genesis が
再生成される(こちらは常に有効な手動リセット)。
...
- 長時間(ハング閾値超)稼働し続けているスタックに対して `restart-node.sh`
  ...
  `docker compose down -v --remove-orphans` で genesis を作り直すのが
  最終手段。
...
`current_slot` だけ増え続ける)場合は、`docker compose down -v
--remove-orphans` で共有ボリュームを破棄し、genesis を作り直すこと
(チェーンの進行状態は失われる)。
