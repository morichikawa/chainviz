...
- **前提**: チェーンプロファイルが1つ（Ethereum）だけの間は作れない機能なので、
  Phase 7（2つ目のプロファイル = Solana 追加時点）から着手する
...
│  - フロントからの操作要求を Docker API に反映    │
│    （ノード/ワークベンチの追加・削除、操作）     │
└───────────────────────┬─────────────────────────┘
              状態差分 │ ▲ 操作コマンド
      （WebSocket）    ▼ │（WebSocket 双方向）
┌─────────────────────────────────────────────────┐
│ GUI フロントエンド（ブラウザ）                   │
│  - 無限キャンバス（ズーム/パン/ドラッグ）        │
│  - ノードグラフ描画 + ホバー詳細 + アニメーション │
│  - キャンバス上でノード/ワークベンチを追加・削除 │
└─────────────────────────────────────────────────┘
```
...
- **キャンバス上でのノード/ワークベンチの追加・削除（操作）**: 環境作成時に
  ...
  - **新しいノードが P2P に参加できるようにする**のが技術的な要点。genesis は
    ノード環境テンプレートに含まれる静的ファイルをそのままマウントして共有し、
    bootnode 情報は Collector が B層 のために既にポーリングしている
    `admin_nodeInfo` から今生きているノードの enode を取り出して新しい
    コンテナの起動コマンドに渡す（新しい状態を持たず、既存のワールドステートを
    再利用する）
...
Phase 1〜5 はすべて「Ethereum チェーンプロファイル」を作り込む工程。
...
2. **Phase 2 — P2P グラフ**:
   ...
   波アニメーションもここで作る
3. **Phase 3 — 生きているチェーン**:
   ブロック生成・tx 投入をリアルタイム反映。ブロック伝播パルス、tx ライフサイクル表示（C層 完成）。
...
- [x] 非 EVM チェーンの着手順（Phase 6 以降） → 知名度順に決定。
...
- [x] Collector を「観察だけでなく操作もできる窓口」にする → 決定。
      ...
      （詳細は「アーキテクチャ案」参照）
- [x] 新規ノード追加時の P2P 参加方法 → 決定。genesis はチェーンプロファイルの
      ノード環境テンプレートに含まれる静的ファイルをそのままマウントするだけで
      共有する。bootnode 情報は、Collector が B層（P2P）の可視化のために
      既にポーリングしている `admin_nodeInfo` から、今生きているノードの
      enode を取り出して新しいコンテナの起動コマンド（`--bootnodes` 等）に
      渡す方式にする（新しい状態を持たず、既存のワールドステートを再利用する）
