...
- **キャンバス上でのノード/ワークベンチの追加・削除（操作）**: 環境作成時に
  ...
  - **新しいノードが P2P に参加できるようにする**のが技術的な要点。genesis は
    ...
    変わらない）。bootnode 情報は Collector が B層 のために既にポーリングして
    いる `admin_nodeInfo` から今生きているノードの enode を取り出して新しい
...
- [x] 新規ノード追加時の P2P 参加方法 → 決定。genesis は生成時刻を埋め込む
      ...
      情報は、Collector が B層（P2P）の可視化のために既にポーリングして
      いる `admin_nodeInfo` から、今生きているノードの enode を取り出して
      新しいコンテナの起動コマンド（`--bootnodes` 等）に渡す方式にする
      （新しい状態を持たず、既存のワールドステートを再利用する）
