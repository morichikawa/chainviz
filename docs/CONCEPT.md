...
- **ノード間・ワークベンチ間のやり取りは「通信ログ」パネルでも時系列に
  追える**。パルス等の一瞬の表現を置き換えるのではなく補完する記録で、
  RPC 呼び出し・ブロック受信・tx の遷移などを遡って確認できる（Issue #317）
- 画面上の専門用語（mempool、peer、gas など）はホバーでその場に解説が出る（後述の用語解説機能）
- **暗号のしくみは「砂場」でさわって学べる**: ブロックのハッシュ連結
  （改ざんすると後続がどうなるか）のような暗号学的な仕組みは、実チェーンに
  影響しない学習用のデモをサイドパネルで開き、値を書き換えながら本物の
  計算（keccak256 等）で体験できる（まずハッシュ連結。Issue #401。
  署名・検証も同じ「入力を編集 → 実計算 → 検証結果」のパターンで
  拡充予定。Issue #402）
...
- **UI 言語は画面の片隅の切り替えボタンでいつでも変更できる**。デフォルトは
  ...
  自体も変更できる（後述）
- 見た目のトーンはダーク基調で、半透明のガラス質感（すりガラスのパネル・
  ポップオーバー）と控えめなグラデーションで質感を持たせる。ただし役割別の
  枠色・エッジ色・状態色の意味体系と情報の可読性を装飾より優先する
  （Issue #327。具体的な配色・数値は `docs/worklog/issue-327.md`）
...
この A〜D の区分は可視化上の見せ方・データ源の違いによる**便宜的な分類**
であり、実装上の厳密なレイヤー境界（モジュール分割やパッケージ構成）を
規定するものではない。実際、collector 内部はドメイン単位（`adapters/` /
`world-state/` / `docker/` など。`docs/ARCHITECTURE.md` 参照）で構成されて
おり、A〜D 層と1対1には対応しない。
...
→ **改訂（2026-07-16、Issue #322）**: slot time の短縮（2 秒）は当初の決定どおり
Phase 2 から運用してきたが、「2 秒間隔のブロック生成は現実の Ethereum（12 秒）と
かけ離れており、学習アプリとしてかえって混乱を招く」というユーザーフィードバックを
受け、**現実と同じ 12 秒に戻す**。待ち時間の体感は、次のブロック生成までの残り時間を
示すインジケータ（Issue #343）で補う。なお「トランザクション発行時のみブロックを
生成する」方式は、実際の PoS Ethereum が tx の有無に関わらず一定間隔で（空ブロックを
含め）ブロックを生成し続けるという本質的な挙動を隠すため不採用とした。
...
- [x] プライベートネットの合意方式 → genesis 共有の PoS ネット（reth + lighthouse 最小構成、
      ...
      （slot time の短縮はその後 Issue #322 で撤回し、現実と同じ 12 秒に改訂。
      同じく「技術候補 > ノード環境」の改訂注記を参照）
