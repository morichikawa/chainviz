...
- [ ] ブロックが連なって積み上がっていく様子を視覚表現する
      ...
      parentHash連結を確認できる。第1段階(基本表示・着地)はQA合格。
      collector側はWorldStateStoreにブロック番号ベースの保持窓
      (BLOCK_RETENTION=32)を追加。既存のブロック伝播パルス・txライフ
      サイクル表示は維持(補完する新規要素)。ホバー連動ハイライト(第2段階)
      は実チェーン環境でQA差し戻し(表示窓の前進でハイライト対象タイルが
      窓外へ流出し消える)。修正中。docs/worklog/issue-298.md参照)
