...
- 担当: qa
- ブランチ: issue-373-fitview-timing-fix
- 目的: 差し戻し対応(`fitCanvasView` の堅牢化)が UI-MULTI-01 の回帰を
  解消し、他スペックにデグレを生じていないかを実 Docker + 実ブラウザで
  最終確認する。

**検証環境**

- 共有 Docker スタック(`chainviz-ethereum`, slot=12秒)を再利用。並行作業の
  テストプロセス(playwright/vitest 等)が動いていないことを確認したうえで、
  過去の e2e 実行の残留 managed コンテナ
  (`chainviz-ethereum-e2e-ui-carol-2-3`)を1つ除去し、6ノード+静的
  ワークベンチのみの決定的なベースラインにしてから検証した。
- Playwright の実 Chromium はシステムに `libnspr4.so` 等が無く起動できな
  かったため、`LD_LIBRARY_PATH` に
  `/home/zoe/chrome-deps/root/usr/lib/x86_64-linux-gnu` を追加して起動した
  (過去 QA と同じ運用)。

**結果**

1. 修正版(堅牢化した `fitCanvasView`)で UI-MULTI-01 を `--repeat-each=4`
   で実行 → 4/4 合格。
2. `multi-client` + `commands-workbench` + `commands-node` をまとめて実行
   (カード追加・削除の連鎖でホスト負荷とワールドステートのカード数が増える
   現実的な条件)→ 9/9 合格(UI-MULTI-01・UI-CMD-07 を含む)。
3. `commands-workbench`(UI-CMD-05〜07)・`node-internals`(UI-D-01/02、
   UI-D-03 は Issue #346 の別問題のため対象外)→ 5/5 合格。今回の変更に
   よるデグレは無い。
4. 全テスト実行後、`chainviz-ethereum` プロジェクトの managed コンテナ
   残留が無いことを確認。`cleanup.ts` の安全網(`removeInfraCardIfPresent`
   経由の堅牢化 `fitCanvasView`)も正しく機能している。

**修正前(単純版)の再現について(正直な限定)**

- 対比確認のため、`viewport.ts` を一時的に「フィットボタンを1回押すだけ・
  再試行なし」の単純版(差し戻し前相当)に差し替えて UI-MULTI-01 を
  `--repeat-each=4` で実行したが、4/4 合格した。すなわち、このアイドル状態の
  高速ホスト(20コア・低負荷)では、回帰の本質である「対象カードの
  React Flow 内部計測が完了する前にフィットが発火し、対象が視野外に
  取り残される競合窓」が現れず、単純版でも失敗を再現できなかった。これは
  実装担当・detective が別セッションで報告した「クリーンな低負荷環境では
  回帰を再現できない(負荷依存)」という観測と一致する。
- 競合窓を広げるための人為的な高負荷生成は、本サンドボックス環境の制約で
  実行できなかった。
- 確認後、`viewport.ts` は `git checkout` で正版へ復元済み(作業ツリーは
  クリーン)。

**判定: 合格**

- 差し戻し修正後の版は、単独 repeat 実行(4/4)・複数スペック連続で負荷が
  かかる実行(9/9)のいずれでも UI-MULTI-01 が安定して合格し、UI-CMD-05〜07・
  UI-D-01/02 にデグレは無い。cleanup 安全網によるコンテナ残留も無い。
- 修正前(単純版)の 4/4 失敗そのものは今回のホストでは再現できなかったが、
  これは前回 QA(実 Docker + 実ブラウザ)で既に「single-click 版は
  UI-MULTI-01 で 4/4 失敗」として観測・記録済みの事実である。堅牢化した
  `fitCanvasView` は「対象が視野内に入るのを確認してから進む/入っていなければ
  再フィットする」という失敗し得ない一般的に健全な条件であり、対象が最初
  から視野内にあるケースでは旧実装と同じ1回クリックで完了するため、既存の
  合格ケースを壊す方向には働かない。以上より、本差し戻し修正は完了条件を
  満たすと判断する。

**申し送り**

- 本ホストでの修正前再現は環境依存(低負荷でレースが起きない)で実施できな
  かった点は上記のとおり記録した。より確実な差分確認が必要なら、フルスイート
  相当の負荷がかかる条件での single-click 版の再実行が有効(ただし今回の
  負荷生成はサンドボックス制約で不可だった)。
- `docs/PLAN.md` の Issue #373 のチェックボックスは差し戻し対応時に既に
  `[x]` 済み。本再QAでも合格判定のため変更不要。
