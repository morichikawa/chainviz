...
### 2026-07-16 実環境検証(chainviz-qa)
- 担当: qa
- ブランチ: issue-327-visual-design(worktree上で検証)
- 結果: 合格。Issue #327の要望(透明感・グラデーションのおしゃれな見た目)を
  視認性を損なわずに満たしていることを実描画で確認した
- 手順: モックモード(VITE_COLLECTOR_URL未設定)でvite devを起動し、
  Playwright(chromium headless)でスクリーンショット取得とcomputed styleの
  検査、実操作(ドラッグ・ポップオーバー開閉・レイヤーフィルタ・言語切替)を実施
- 確認点:
  - オーバーレイパネル5種(canvas-toolbar / layer-filter-bar / p2p-legend /
    contract-list-panel / mempool-panel)すべてにbackdrop-filter blur(14px)が
    適用され、地・縁・影が同一値でそろっていることを確認
  - infra-popoverはホバーで開きblur(16px)・ダークガラス地・役割別枠色を維持。
    背後のカードが透けつつ本文の可読性を保持
  - トースト(error/info)はガラス地で、errorは赤枠・赤みがかった地。文字色は--text
  - カード群はbackdrop-filterなしの縦グラデーション+上端ハイライト、役割別
    border-colorを維持
  - 背景に青+インディゴのラジアルグラデーション。React Flowの#141414に
    上書きされていない(透過漏れなし)
  - タイトルのグラデーションテキストが有効
  - ドラッグ・ポップオーバー開閉・レイヤーフィルタ・言語切替にデグレなし。
    全操作を通じてコンソール/ページエラーはゼロ
- 注意点(スコープ外): 英語モードでp2p-legendの凡例文が日英混在になっている
  が、これはi18nの翻訳文字列の問題で#327のCSS変更とは無関係(変更前から存在)。
  別Issue化の候補として記録に留める
