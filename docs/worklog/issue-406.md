...
### 2026-07-19 Issue #406 QA検証結果（qa）: 合格

- 担当: qa
- ブランチ: issue-406-hash-input-clarity
- 判定: **合格**（差し戻しなし）
- 検証環境: 共有 Docker スタック（`chainviz-ethereum`。稼働中を再利用、
  freshStart なし）+ 公式 UI E2E ハーネス、およびモックモード
  （`VITE_COLLECTOR_URL` 未設定）の vite dev server + Playwright/chromium
  （`LD_LIBRARY_PATH=/home/zoe/chrome-deps/root/usr/lib/x86_64-linux-gnu`
  を付けて chromium の共有ライブラリ不足を解消）。他エージェントの作業を
  妨げないよう、事前に e2e ロック未取得・Playwright 未起動を確認してから
  実施し、スタックは down していない。
- 確認結果（PLAN.md #406 完了条件と対照）:
  1. ハッシュ改ざんデモの処理帯が2行構成になり、2行目に
     「x = ブロック番号 | 親ブロックのハッシュ | データ（上の3項目を…）」が
     常時表示されることを実ブラウザで確認。データ欄を編集すると f(x) の
     出力（このブロックのハッシュ）が追従して変わることも確認
     （0x4e0e47… → 0x5c5c5d…）。なお x 行の文言は入力の「項目名」を示す
     凡例であり（UX設計 §3 の意図的な設計。直上のフィールドラベルと完全
     一致させ読者が照合できるようにするため）、データ値そのものは
     表示しない。値の変化はハッシュ出力側に現れる。
  2. 署名検証デモの署名側処理帯に
     「x = keccak256(送信者 | 宛先 | 金額)。内容をまず keccak256 で…」、
     検証側処理帯に「x = 届いた署名 と keccak256(送信者 | 宛先 | 金額)…」が
     表示されることを確認。EN 切り替えでも
     「keccak256(sender | to | amount)…」に追従することを確認。
  3. 両デモの keccak256 語をホバーすると用語集ポップオーバーが開き、
     name「keccak256（ケチャック256）」、definition に SHA-3 標準化前の
     Keccak・256bit(32バイト)出力・イーサリアムでの用途が表示されることを
     確認（ハッシュデモ 3 アンカー / 署名デモ 3 アンカー）。
  4. ポップオーバーの relatedTerms が生キーでなく解決済み用語名
     「ハッシュ, 署名, ブロック」で表示されることを確認。keccak256 語を
     クリックすると用語集サイドパネルが開き、関連語として
     ハッシュ/署名/ブロックへナビゲート可能なことを確認。逆方向
     （hash/signature → keccak256）の相互リンクは
     glossaryRelatedTermsIntegrity.test.ts が双方向性を固定済み。
  5. 公式 E2E `UI-HASH-01`（改ざん→連鎖修復→リセット）・`UI-SIG-01`
     （改ざん→なりすまし不成立→正しい再署名）を再実行し 2 passed。
     既存の基本動作は壊れていない。
  6. 処理帯コンテナ（`.hash-chain-demo__compute` /
     `.signature-demo__compute`）の `aria-hidden` が解除され（属性なし）、
     装飾記号トークン（`*-compute-fn` の f(x) / f⁻¹(x) / x =）のみ
     `aria-hidden="true"` であることを実 DOM で確認。アルゴリズム名・x の
     説明行は読み上げ対象に残り、装飾用サブツリーに紛れていない。
- 補足（差し戻し不要）: PLAN.md の #406 チェックボックスは実装コミット
  段階で既にチェック済みだった（qa 検証前チェックは本来の手順とは前後
  するが、内容は合格のためそのままとする）。
