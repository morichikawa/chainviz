...
### 2026-07-12 Issue #298 QA再検証（ホバー連動ハイライトの窓流出修正の確認）

- 担当: qa
- ブランチ: issue-298-block-stacking-visualization（worktree、working tree clean）
- 対象: コミット b929a90（表示窓の凍結フック `useFrozenRibbonTiles` 導入）
  以降。前回QA差し戻し（UI-B-06: ホバー後に表示窓が前進しハイライト対象
  タイルが窓外へ流出して消える）の修正が実チェーン環境で解消されているかを
  前回と同じ手順で再検証した。

#### 検証環境

- 既存稼働中の共有 Docker スタック（`profiles/ethereum`）を再利用。reth1 の
  RPC（`eth_blockNumber`）で 2 秒スロットの進行を確認（検証中 11001→11548 と
  進行）。globalSetup が collector（4125）・vite（5275, 実 collector 接続）を
  起動し chromium から検証（モックではない）。
- 注意: スタックには別/前セッション由来の reth3/4/5・beacon3/4/5 が同居して
  おり（計10ノード）、P2P グラフ・カード数が多い「混雑状態」だった。これらは
  検証開始前から存在し QA の生成物ではないため、共有インフラを乱さないよう
  停止・削除はしていない。前回QAが指摘した「混雑の影響」がそのまま残る環境。
- ブラウザ起動に必要な NSS/NSPR 系ライブラリが system に無く、既存の
  `/home/zoe/chrome-deps` に展開済みのものを `LD_LIBRARY_PATH` で参照して
  chromium を起動した（環境要因。アプリとは無関係）。

#### 結果サマリ

- UI-B-05: 合格（リボン表示・タイル連なり・最新番号の増加・ポップオーバー）。
- UI-B-06: 自動テストは 2/2 で失敗。ただし失敗の原因はアプリの不具合ではなく、
  混雑レイアウトで送信元ウォレットカードがビューポート下端の外へ押し出される
  ことに起因する e2e の頑健性問題である（下記調査を参照）。
- 凍結フィックス自体は設計どおり機能しており、前回差し戻した不具合
  （ホバー中に表示窓が前進しハイライト対象タイルが窓外へ流出して消える）は
  解消されていることを手動プローブで確定的に確認した。

#### UI-B-06 自動テスト失敗の内訳

- 1 回目: `.chain-ribbon-tile--highlight` の `toHaveCount(1)` は一度通過した
  が、直後の `getAttribute` でハイライトが消え、要素待ちのままテスト
  タイムアウト（150s）。
- 2 回目: `toHaveCount(1)` が 30s 間ずっと 0 のままタイムアウト（前回QAと
  同じ症状）。

#### 原因調査（一時プローブ spec を e2e に置いて実測、実行後に削除）

チップにホバー後、ハイライト・チップの :hover 状態・リボン最新番号・タイルの
DOM ノード同一性を細かくポーリングして切り分けた。

1. 単発ホバー（プローブ1）: `chip.hover()` 直後からチップの :hover が false の
   まま、ハイライトも出ない。表示窓は前進し続ける（凍結が発動していない）。
2. 毎回再ホバー（プローブ2）: :hover が true の間はハイライトが 1 件付き、
   同一タイル `0xb475903b…` が 6 ブロック以上（#11377→#11383）進行しても
   窓に残り続けた（＝凍結が効いている証拠）。:hover が false へ落ちるのは
   ブロック到来のタイミングと一致し、そのときハイライトも 0 になる。
3. DOM ノード同一性（プローブ3）: チップに独自タグを付けて追跡したところ、
   ブロックが進んでもチップは終始「同一 DOM ノード・同一座標 (610,1274)」で、
   再マウントも再配置もしていない。にもかかわらず :hover が false のまま。
   座標 y=1274 はビューポート高さ 1200 を超えており、チップが画面外
   （下端の外）にあることが判明。混雑レイアウトで送信元ウォレットカードが
   下方へ押し出されているため、Playwright の合成マウスがチップ上に留まれず
   持続ホバーが成立しない。
4. 画面内・静止ホバー（プローブ4、決定的）: canvas をズームアウトして
   チップを画面内（y=667）に入れ、マウスを静止させたまま単発ホバーすると、
   :hover=true・ハイライト 1 件（同一タイル `0xd2e11976…`）が 12 秒間継続し、
   その間リボン最新番号は #11548 で完全に凍結した（ホバー中は表示窓が
   前進しない＝凍結フィックスの意図どおりの挙動）。

#### 結論と判定

- 前回差し戻した不具合（ホバー中に表示窓が前進してハイライト対象タイルが
  窓外へ流出し消える）は、凍結フィックスにより解消されている（プローブ4が
  決定的。ホバー中は窓が凍結し、ハイライト対象タイルがブロック進行後も
  保持され続ける）。**凍結ロジック（`useFrozenRibbonTiles` / `ChainRibbonCard`）
  への追加修正は不要。**
- 一方、`chain-ribbon.spec.ts` の UI-B-06 は、送信元ウォレットカードが
  ビューポート外へ押し出される混雑環境では、tx チップへの持続的な合成ホバーが
  維持できず green にならない。これは e2e の頑健性の問題（ホバー前にチップを
  ビューポート内へ確実に収める処理が無い）であり、完了条件「UI-B-06 も green」を
  この共有環境では満たせなかった。
- 判定: **保留（PLAN.md のチェックは付けない）**。アプリの修正は検証で正しさを
  確認済みだが、完了条件である UI-B-06 の green を現環境で示せていない。統括の
  判断で次のいずれかを推奨する:
  1. 比較的空いている（カードがビューポート内に収まる）クリーンなスタックで
     UI-B-06 を再実行して green を確認する（コード変更不要の可能性が高い）。
  2. frontend が UI-B-06 を頑健化する（持続ホバー前に tx チップを
     `scrollIntoViewIfNeeded` 等でビューポート内に収める、または混雑時でも
     カードが画面内に来るようレイアウト/ズームを調整する）。
  いずれの場合も凍結ロジック本体は変更不要。

#### 後片付け

- 検証で追加したワークベンチ由来コンテナ（`e2e-ribbon-recipient-*` 2件・
  `probe-*` 4件）を `docker rm -f` で削除し、`docker ps -a` で残存が無いことを
  確認した（前回、removeWorkbench 後もコンテナが Up のまま残る事象があった
  ため、削除後の実消滅まで確認。今回 `e2e-ribbon-recipient-mrhqhsds-2` が
  カード削除後も Up のまま残っていたのを本削除で解消）。
- 一時的に置いた検証用プローブ spec（`packages/e2e/src/ui/_tmp_ribbon_probe.spec.ts`）は
  削除済み。collector（4125）・vite（5275）の残プロセスが無いことも確認。
- ベーススタックおよび別/前セッション由来の reth3/4/5・beacon3/4/5 には
  手を付けていない。

### 2026-07-12 Issue #298 QA最終検証（クリーンな正規スタックでのUI-B-06再検証）

- 担当: qa
- ブランチ: issue-298-block-stacking-visualization（worktree、開始時 working tree clean）
- 経緯: 前回QA（同日の直前の記録）は「UI-B-06 が green にならないのは、他/前
  セッション由来の検証用ノード（reth3/4/5・beacon3/4/5）でキャンバスが混雑し、
  送信元ウォレットカードのチップがビューポート外へ押し出されるため」と推定して
  保留とした。統括がユーザーの許可を得てこれら検証用ノード・ワークベンチを削除し、
  現在は正規構成（reth1-2 / beacon1-2 / validator1-2 / workbench-1）のみ。クリーンな
  状態で前回と同じ検証を再実行した。

#### 検証環境

- 既存稼働中の正規 Docker スタック（`profiles/ethereum`）を再利用。reth1 の RPC
  （`eth_blockNumber`）で 2 秒スロットの進行を確認（検証中 0x32f0→0x32f7… と進行）。
  `docker ps -a` で混雑ノードが実際に無く、正規7台＋genesis(Exited 0, one-shot)のみで
  あることを確認済み。
- globalSetup が collector（4125）・vite（5275、実 collector 接続）を起動し chromium
  から検証（モックではない）。ブラウザ起動に必要な NSS/NSPR 系ライブラリが system に
  無く、既存の `/home/zoe/chrome-deps` を `LD_LIBRARY_PATH` で参照して chromium を
  起動（環境要因。アプリとは無関係）。

#### 結果サマリ

- UI-B-05: 合格（リボン表示・タイル連なり・最新番号の増加・ポップオーバー、4.0s で green）。
- UI-B-06: 依然として失敗（`.chain-ribbon-tile--highlight` の `toHaveCount(1)` が
  30s 間ずっと 0 のままタイムアウト）。**混雑ノードを削除しても症状は変わらなかった。**
- 凍結ロジック（ホバー中に表示窓を凍結しハイライトを保持する仕組み）は、チップを
  ビューポート内へ入れて持続ホバーすれば設計どおり機能することを一時プローブで
  決定的に再確認した（下記）。アプリ側の追加修正は不要。

#### 原因（前回推定の訂正。混雑ではなくビューポート外配置）

一時プローブ spec（`packages/e2e/src/ui/_tmp_ribbon_probe.spec.ts`、実行後削除）で、
チップの座標・:hover 状態・ハイライト・リボン最新番号を実測して切り分けた。

1. 送信元ウォレットカードの tx チップは、正規構成（混雑解消後）でも
   `boundingBox` が y≈1272 と、ビューポート高さ 1200 を超えた**下端の外**に
   位置していた。混雑が原因ではなく、`addWorkbenchAndGetWallet` で受け取り用
   ワークベンチを追加した結果グリッドが下へ伸び、送信元ウォレットカードが
   既定ビュー（fitView 後）の描画範囲の下に押し出されるためと判明。
2. その状態では Playwright の合成マウスがチップ上に留まれず `:hover=false` の
   まま。チップの onMouseEnter が発火しないため `hoveredBlockHash` が立たず、
   ハイライトも凍結も起きない（UI-B-06 の `toHaveCount(1)` が 0 のまま）。
3. React Flow の「Zoom Out」を数回押してチップをビューポート内（y≈942）へ
   入れてから持続ホバーすると、`chipHover=true`・ハイライト 1 件が継続し、
   その間リボン最新番号は #13568 で完全に凍結した（ブロックが 2 秒ごとに
   生成され続けても表示窓が前進しない＝凍結フィックスの意図どおりの挙動を
   決定的に確認）。手動でのホバー保持確認（検証依頼の項目3）もこれで充足。
4. なお、ビューポート内でも「チップの tx を含むブロックのタイルが、ホバー
   時点でリボンの表示窓（直近8タイル≒16秒）内に残っていること」が必要で、
   included からホバーまでの遅延が延びるとタイルが窓外へ流れて出ない、という
   二次的なタイミング依存もある（別プローブで toHaveCount(1) が一度 0 で
   落ちた事例を観測）。

#### 判定

- **保留（差し戻し）。`docs/PLAN.md` のチェックは付けない。**
- アプリ（凍結ロジック＋チップ↔タイルの相互ハイライト）は機能的に正しいことを
  検証で確認済み。UI-B-05 も green。しかし完了条件である「UI-B-06 も green」は
  クリーンな正規スタックでも満たせなかった。前回の保留理由（混雑）は誤りで、
  真因は e2e テスト UI-B-06 がビューポート外に押し出された送信元ウォレットカードの
  チップを、画面内へ収めずに直接ホバーしようとしている点にある（テストの頑健性
  の問題）。
- 差し戻し先: **frontend**（UI 層 e2e シナリオ `chain-ribbon.spec.ts` を所管）。
  必要な対応（いずれもアプリ本体＝凍結ロジックの変更は不要）:
  1. tx チップの持続ホバー前に、チップを確実にビューポート内へ収める
     （React Flow の Zoom Out / Fit View コントロール操作、または
     `scrollIntoViewIfNeeded` 相当）。実測ではズームアウトでチップを画面内に
     入れれば `chipHover=true`・ハイライト保持まで正しく動く。
  2. included からホバーまでの遅延で対象タイルが表示窓（8タイル≒16秒）外へ
     流出しないよう、ホバーを included 検出直後に速やかに行う（または対象
     ブロックが窓内にあることを前提に組む）。
- いずれの場合も凍結ロジック本体（`useFrozenRibbonTiles` / `ChainRibbonCard` /
  `RibbonHoverContext`）への追加修正は不要。

#### 後片付け

- 一時プローブ spec（`_tmp_ribbon_probe.spec.ts`）は削除済み。
- 検証で追加したワークベンチ由来コンテナ（`e2e-ribbon-recipient-*` 1件・
