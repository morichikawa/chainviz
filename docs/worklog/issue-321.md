...
### 2026-07-16 実環境検証（qa。合格）

- 担当: qa
- ブランチ: issue-321-contract-source-view
- 判定: **合格**（Issue #321 の期待する対応をすべて満たす）
- 検証環境: 既存の共有 Docker スタック（profiles/ethereum、稼働中のもの）を
  再利用。catalog.json のソース埋め込みは collector が起動時にホストの
  `profiles/ethereum/contracts/catalog.json` を直接読む（`readContractCatalog`。
  Docker イメージには焼き込まれない）ため、`docker compose down -v` による
  スタック再構築は不要で、collector の再起動のみで反映される。今回は
  `docker compose down -v` は実行していない。scripts/dev-up.sh で collector
  （port 4000 / proxy 4001）・frontend（port 5173）を起動し、実物の GUI を
  Playwright（chromium）で操作して確認した。
- 確認した内容:
  - catalog.json に ChainvizToken / Counter のソース（source.fileName /
    language / code）が埋め込まれていることを確認（codeLen それぞれ 3150 /
    823）。
  - ワークベンチの定型操作（runWorkbenchOperation の deployContract）で
    ChainvizToken（constructorArgs で initialSupply を渡す）と Counter を
    デプロイし、WebSocket 経由の world state で ContractEntity.sourceCode が
    catalogKey とともに転記されることを確認。届いたソースは
    `profiles/ethereum/contracts/src/*.sol` とバイト単位で一致（diff で
    IDENTICAL）。
  - 未知（カタログ未照合）のコントラクトは、ワークベンチシェルから
    `forge create`（GUI 操作を経由しない = registerContractDeployment が
    呼ばれない経路）でデプロイして作成。ContractEntity.sourceCode が
    undefined（未知のコントラクト）になることを確認。
  - GUI（実 frontend + 実 collector）での UI 挙動を Playwright で確認（22
    項目すべて pass）:
    - コントラクトカード（ChainvizToken / Counter / 未知）すべてに
      「ソースコードを見る」ボタンが出る。
    - 既知コントラクトのボタン押下で右ドックのサイドパネルが開き、行番号
      付き（1 始まり、ChainvizToken は 88 行）・シンタックスハイライト付き
      （keyword / comment / string / number / type の複数種のトークンに
      色分け）でソース全文が表示される。ファイル名（ChainvizToken.sol /
      Counter.sol）も表示。
    - 未知コントラクトのボタン押下では「ソースは復元できない」旨の説明
      （バイトコードのみが載っており元ソース = ABI は復元できない、カタログ
      掲載分のみ表示可能）が表示され、ソースコードブロックは出ない。
    - 閉じるボタン・Esc キーの両方でパネルが閉じる。
    - 別カードのボタンを押すとパネルが排他的に置き換わる（同時に開くのは
      常に 1 枚）。
    - 日英切り替えでパネルタイトル（ソースコード / Source code）・未知時の
      説明文・ボタン文言（ソースコードを見る / View source code）が正しく
      切り替わる。
  - ダングリングガード（対象コントラクトが world state から消えた場合に
    パネルを自動で閉じる）は、通常運用ではコントラクトが削除されないため
    実環境では発火させられない。SidePanelHost.test.tsx のユニットテスト
    （dangling guard / dangling transition の 2 ケース）でカバーされている
    ことを確認した。
- 後片付け: scripts/dev-down.sh（--docker なし）で collector / frontend を
  停止。Docker スタックは指示どおり保持（down は実行していない）。検証用に
  デプロイした 3 コントラクト（ChainvizToken 1 / Counter 2 / 未知 1）は
  チェーン上に残るが、無害。
