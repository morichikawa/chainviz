...
- 担当: qa（検証）
- 判定: **合格**（完了条件を満たす）
- 検証環境: 実ブラウザ（Playwright/Chromium ヘッドレス）で frontend の vite
  dev server を起動し、実 `createChainvizClient`（モックではない実 WebSocket
  クライアント）を使って検証した。VITE_COLLECTOR_URL を、この検証用に
  立てた偽 collector（Node の ws サーバー。接続時に空スナップショットを送り、
  コマンド受信・commandResult/diff の送出・接続の強制切断を制御できる）へ
  向けた。ブラウザ上の実 DOM（ゴーストカード・トースト・接続バッジ）を
  観測して判定した。
- 実施シナリオと結果（全19チェック合格）:
  - A（接続時の回帰確認）: 接続バッジが「接続済み」。addNode で EL/CL の
    ゴースト2枚が出現し、偽 collector が addNode コマンドを受信、
    commandResult(ok:true)+entityAdded diff を返すと実ノードカードが出現して
    ゴーストが解消。addWorkbench も同様にゴースト出現→コマンド受信→実体到着で
    解消。ここまででエラートーストは0件（正常系は従来どおり動作）。
  - B（Issue #235 の再現手順そのもの / 核心）: 偽 collector を停止して接続
    バッジが「切断」に変わった状態で addNode を押すと、ゴーストが1枚も
    生成されず、即座に「ノードの追加に失敗しました: collector に接続されて
    いません」のエラートーストが表示された。addWorkbench も同様に、ゴースト
    無しで「ワークベンチの追加に失敗しました: collector に接続されて
    いません」が即出。切断中の addNode ではゴーストが作られない＝連打防止の
    pending 無効化にも入らないため、追加ボタンが無効化されたまま固着しない
    ことも確認（disabled=false）。
  - C（タイムアウト経路）: 再接続して接続済みに戻し、addNode でゴーストを
    出した直後に偽 collector を停止（commandResult も diff も返さない）。
    GHOST_TIMEOUT_MS（60秒）経過後、ゴーストが消えると同時に「ノードの追加に
    失敗しました: 応答がありませんでした（タイムアウト）」のエラートースト
    が表示されることを確認。
- 空振りでないことの裏取り（負のコントロール）: 修正対象4ファイル
  （client.ts / useCommands.ts / commandMessages.ts / messages.ts）を一時的に
  修正前コミット（`39f5764^`）の状態へ戻して同じ切断中 addNode を実行した
  ところ、ゴーストが2枚生成され・エラートーストは0件（＝Issue #235 の
  バグがそのまま再現）することを実ブラウザで確認した。確認後に4ファイルを
  修正後（HEAD）へ復元し、作業ツリーがクリーン（変更0件）であることを確認
  した。これにより「修正前は再現し、修正後は解消する」ことを実機で両方
  確認できた。
- 結論: 元 Issue（collector 停止中に送信した addNode/addWorkbench がゴースト
  消滅のみでエラートーストが出ない）は実機上で解消されている。回帰（接続時の
  通常動作）・追加のタイムアウト経路のいずれも期待どおり。完了条件を満たす。
- 補足（申し送り。合格には影響しない）: この検証環境では Playwright の
  chrome-headless-shell が共有ライブラリ（libnspr4.so 等）不足で起動できず、
  スクラッチパッドに展開済みの依存ライブラリを LD_LIBRARY_PATH で参照して
  起動した。UI-CMD 系の正式な Playwright E2E も同じライブラリ不足の影響を
  受けうるため、恒常的に実行するなら実行環境側の依存整備が別途必要。
