...
### 2026-07-12 frontend側テスト強化（エッジケース・異常系・境界値）

- 担当: tester
- ブランチ: issue-296-fork-color-coding
- frontend実装担当が書いた基本テスト（ハッピーパス中心）に対し、見落とし
  やすいエッジケース・境界値・設計上のトレードオフを固定するテストを追加
  した。実装ロジックは変更していない（追加したのはテストファイル2件のみ）。
- 1ファイル1責務の原則に従い、既存の基本テストファイルを膨らませず、
  エッジケース専用の新規ファイルに分離した:
  - `packages/frontend/src/entities/forkState.edge.test.ts`（9件）:
    - 祖先探索の打ち切り上限（maxSteps）ちょうどの境界。必要ホップ数に
      等しいmaxStepsで辿り切れること、1つ足りないと unknown になること、
      境界ちょうどでも辿った先が違えば fork と確定すること、直近の親が
      無く1ホップも辿れないケース。
    - 既定maxStepsが保持ブロック数から導出されるため、全ブロックが揃った
      深いフォーク（5段の枝）を誤って打ち切らないことの回帰ガード。
    - 3分岐以上（3-way fork）が3グループに分かれること、3分岐のうち
      遅延ノードが正しく自分の枝と同一グループになること。
    - Union-Findの過小検出トレードオフ: 親不明の判定不能tipが本物の
      フォーク2枝それぞれとunknown関係になると、両枝がそのtip経由で同一
      グループへ吸収され、本物のフォークが見逃される（設計メモに明記の
      既知挙動）。単独では2グループに分かれること（baseline）と対で固定。
  - `packages/frontend/src/entities/useForkColors.edge.test.ts`（4件）:
    - 色プール（0〜3の4色）を使い切る5分岐で、5番目が巡回して色0を
      再利用すること（割り当て結果 [0,1,2,3,0] を固定）。全indexが
      パレット範囲内に収まること。
    - 一部の枝だけ収束（3枝のうち1枝がbranch Aへreorg）したとき、リセットが
      早期発火せず残る枝の色が安定して保たれ、収束した枝が合流先と同色に
      なること。
    - 3分岐で3色が割り当てられること。
    - 未観測（headBlockHash空）のvalidatorが色付け対象から除外されること。
- 回帰テストの有効性確認（意図的に実装を壊して検出できることを確認し、
  確認後に元へ戻した。CLAUDE.md「直したはずで済ませない」への対応）:
  - `resolveAncestorAtHeight`の打ち切り判定を`steps >= maxSteps`から
    `steps > maxSteps`に改変 → maxSteps境界テスト（1つ足りない側）が失敗。
  - `detectForkGroups`のUnion-Find併合条件を`relation !== "fork"`から
    `relation === "same"`（unknownを併合しない）に改変 → 過小検出
    トレードオフの吸収テストが失敗。
  - `nextUnusedColor`のプール枯渇時の戻り値を`used.size % パレット数`から
    `パレット数`（範囲外の4）に改変 → 色プール枯渇テストが失敗。
  - `useForkColorAssignment`のリセット条件を`groups.length === 0`から
    `groups.length === 0 || groups.length < 前回グループ数`に改変 →
    一部収束テスト（早期リセット検出）が失敗。
- `pnpm lint` / `pnpm --filter @chainviz/frontend build` / `test`
  （130ファイル、1976件）はいずれも成功。
- 実装のバグらしき挙動は見つからなかった。Union-Findの過小検出は設計メモ
  （ARCHITECTURE.md §9.2）に沿った意図的なトレードオフであり、
  characterization testとして固定した（差し戻しは不要）。

### 2026-07-12 レビュー（査読担当）

- 担当: reviewer
- ブランチ: issue-296-fork-color-coding（cherry-pick合流後）
- 判定: **合格**
- 確認内容:
  - cherry-pick合流の健全性: `git log main..HEAD` の21コミットに重複・
    取りこぼしなし。merge-base からの差分は Issue #296 の内容のみで、
    collector側とfrontend側の実装が矛盾なく合流している。
  - 設計メモの決定事項3点の遵守を確認: (1) 既存の `NodeEntity.headBlockHash`
    を器に使い新エンティティ・新DiffEvent種別なし、(2) `recordHead` は既存の
    newHeads コールバック内のみで追加RPC・追加購読ゼロ、(3) `HeadTipCache` と
    `toEntity` はフォーク判定を一切持たず観測事実のみ配信。
  - `packages/shared` の差分は `headBlockHash` の JSDoc 追記のみ（構造変更
    なし）を diff で確認。
  - `detectForkGroups` の Union-Find（経路圧縮つき）は正しく、"fork" 確定
    ペアのみ分離・"same"/"unknown" は安全側で併合する実装。過小検出の
    トレードオフはコードコメント・ARCHITECTURE.md §9.2・characterization
    テスト（`forkState.edge.test.ts` の bridge 吸収テスト）の三点で固定
    されている。
  - `prune` は pollInfra の毎サイクル、今回観測の node 集合で呼ばれており、
    Map 肥大化防止・removeNode→addNode の亡霊 tip 防止はテストで担保
    （実装を意図的に壊して検出することの確認記録もworklogにあり）。
  - 祖先探索の打ち切り上限は保持ブロック数から動的に導出（決め打ち定数
    なし）。3秒レイテンシの前提条件は ARCHITECTURE.md §9.1 とコードの両方に
    明記されている。
  - 新規コードに catch 節なし（エラー握りつぶしなし）。既存の購読エラー
    ハンドラの経路は不変。
  - `InfraPopover` の「見ている tip」欄は `entity.kind === "node"` ガード
    内にあり、workbench への漏れなし。フロントにチェーン固有語彙の漏れなし
    （newHeads 等は collector の Adapter 内と shared の JSDoc 説明例に限定）。
  - `pnpm build` / `pnpm lint` / `pnpm test`（shared 62・collector 1332・
    frontend 1976・e2e 158）すべて exit 0 を実測。
  - glossary `fork` の日本語定義は既存の3拍子（定義→なぜ起きるか→chainviz
    ではどう見えるか）を満たす。relatedTerms（peer/gossip/p2p）は全て実在。
  - docs（ARCHITECTURE.md §9、PLAN.md チェックボックス+リンク、WORKLOG.md
    索引行）は実装と整合。コミットは全て Conventional Commits 準拠で
    1変更1コミットの粒度。
- 非ブロッキングの指摘（差し戻し不要、将来の改善候補）:
  - `useForkColors.ts` の `nextUnusedColor` はパレット枯渇時に
    `used.size % FORK_COLOR_PALETTE_SIZE` を返すが、`used` が Set のため
    6グループ目以降も常に色0になる（コメントの「巡回して割り当てる」は
    厳密には5グループ目のみ）。2バリデーター環境では実質到達しない経路で
    表示も破綻しないため許容。
- 統括への申し送り: 本ブランチは #295 マージ前の main から分岐しているため、
  main へのマージ時に `docs/WORKLOG.md`（索引）で軽微なコンフリクトが発生
  する（`git merge-tree` で実測確認済み。双方が末尾に行を追加しただけ
  なので両方の行を保持すれば解決する）。コード側のコンフリクトはない。

### 2026-07-12 QA検証（検証担当）

- 担当: qa
- ブランチ: issue-296-fork-color-coding
- 判定: **合格**
- 検証環境: 既に稼働中だった `chainviz-ethereum` の docker compose スタック
  （10コンテナ、reth1/2/3・beacon1/2/3・validator1/2・workbench）を流用。
  本Issueは `profiles/` に変更が無いためそのまま利用した。本ブランチで
  ビルドした collector を一時ポート 4200/4201（常設4000/4001と衝突しない値）
  で起動し、frontend は vite dev サーバー（5199）で起動して検証した。

**1. チェーン進行の確認**

- reth1 の RPC（8545）で `eth_blockNumber` を2回取得し、6657→6660 と
  進行していることを確認。検証中も継続してブロックが積み上がっていた。

**2. collector が headBlockHash を埋めること（実環境）**

- collector を 4200 で起動し、WebSocket スナップショットを取得。
  - reth1/2/3・beacon1/2/3 の `headBlockHash` が全て同一の tip ハッシュで
    埋まっている（同一チェーンに追従中のため一致は正しい挙動）。
  - validator1/2 は `headBlockHash` が空文字列のまま（P2P非参加で
    `executionTargets`/`receivedAtKeys` の対象外。設計どおり）。
  - 8秒間隔で2回取得し、reth1 の tip ハッシュと blockHeight が
    （6708/height6708 → 6717 のように）新しい値へ更新され続けることを確認
    （購読が継続して効いており、静的初期値ではない）。

**3. 正常時（フォーク無し）に色分けが出ないこと（実環境ブラウザ）**

- frontend を `VITE_COLLECTOR_URL=ws://localhost:4200` で起動し、実 collector
  へ接続（ヘッダのバッジが「接続済み」= モックではなく実接続であることを
  確認）。Playwright（headless chromium）で描画を取得。
  - `infra-card--fork-*`（フォーク色分けクラス）を持つカードは 0 件。
  - 実ノード群（beacon1/2/3・reth1/2/3・validator1/2）全カードに色分けの
    縁取りが出ていないことをスクリーンショットで目視確認。
  - コンソールエラー 0 件。

**4. フォーク誘発の試み**

- 設計メモ・ARCHITECTURE.md §9 の第一候補である「ホスト側 iptables
  （DOCKER-USER チェーン）で beacon 間 P2P を DROP する」方式は sudo が
  パスワードを要求するため、本環境では実行不可（`sudo -n` 失敗を確認）。
  実環境での自然/人為フォーク誘発は本環境では不可能と判断し、設計メモが
  想定した代替手段（モックデータ駆動のUI確認）に切り替えた。

**5. モックデータでのフォーク色分け・ポップオーバー・収束（ブラウザ）**

- `websocket/mockData.ts` の `createForkMockSnapshot()`・
  `mockForkConvergeDiffs()` を使い、frontend を実際にブラウザ（Playwright
  headless chromium）で描画して確認した。確認用の一時エントリ
  （`packages/frontend/qa-fork-entry.tsx`・`qa-fork.html`）で App に
  フォークシナリオを流すクライアントを注入し、検証後に削除（git status
  clean を確認済み）。
  - フォーク時: branch A（chainviz-lighthouse-1 / chainviz-reth-1）が
    ゴールドの縁取り、branch B（chainviz-lighthouse-2 / chainviz-reth-2）が
    シアンの縁取りで色分けされ、2枝が別色になることを目視確認
    （色付きカード4枚 = 2枝×2ノード）。
  - validator-1 / validator-2（headBlockHash 未観測）には色が付かないこと
    を確認（フォーク判定の対象外）。
  - ノードカードのポップオーバーに「見ている tip」欄が短縮ハッシュ
    （例: `0xaaaa0082`）付きで表示されることを確認。
  - `mockForkConvergeDiffs()` を適用（branch B が branch A の tip へ
    reorg）すると、全カードの色分けが消え（0件）、ポップオーバーの
    「見ている tip」欄も消えることを確認（「収束すると色が揃う＝色が消える」
    表現が機能している）。
  - コンソールエラー 0 件。

**PLAN.md 完了条件との照合**

- 「collectorはeth_subscribe(newHeads)からHeadTipCache経由で埋め」: 実環境で
  headBlockHash が同一tipで埋まり更新され続けることを確認（項目2）。追加RPCは
  発生していない（既存のnewHeads購読のみ。ロギングプロキシのログにも
  異常なし）。
- 「frontendはparentHashの祖先関係比較でフォーク判定・色分け表現を行う」:
  モックのフォークシナリオで色分け・ポップオーバー・収束を実ブラウザで
  確認（項目5）。正常時（同一tip）に色が出ないことも実環境で確認（項目3）。
- 完了条件を満たしていると判断する。

**後始末**

- 一時起動した collector（4200/4201）・vite（5199）は停止済み。docker
  compose スタックは検証前から稼働していたもので、読み取り専用の観測のみ
  行い破壊的操作はしていない（10コンテナ稼働のまま維持）。一時ファイルは
  削除し、作業ツリーは clean。

**補足（差し戻し不要の申し送り）**

- 実環境でのフォーク誘発は sudo 前提で本環境では不可能なため、UI 側の
  フォーク表示検証はモックデータ駆動で行った（設計メモが想定した代替手段
  どおり）。実環境で色分けが「出ない」正常系は実接続で確認済み、実環境で
  色分けが「出る」異常系はモックで確認済み、という組み合わせで完了条件を
  満たすと判断した。
- PLAN.md の #296 チェックボックスは実装担当のコミット（73ab535）で既に
  チェック済みであり、本QA合格をもってそのチェックは妥当と確認した。
