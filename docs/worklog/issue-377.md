...
### 2026-07-18 Issue #377 レビュー差し戻し対応(tester)

- 担当: tester
- ブランチ: issue-377-glossary-font-size
- 上記レビュー差し戻し(タイ挙動コメントの事実誤認3箇所)に対応した。
  挙動・期待値(アサーション)は一切変更せず、コメント・テスト名の訂正と
  実装への説明コメント追加のみ。node 実測(reviewer と同値)で確認した:
  - 1.4 は |1.3-1.4|=0.09999999999999987 < |1.5-1.4|=0.10000000000000009 で
    1.3 が厳密に近い(タイではない)
  - 1.075 は |1-1.075| === |1.15-1.075|(0.07499999999999996)で真のタイ。
    strict `<` による先発優先が実際に効くのはここで、1.0 が選ばれる
- 変更ファイル: `sidePanelFontScale.test.ts`(1.4 テストの名前・コメント訂正、
  真のタイ 1.075 を専用テストとして分離、境界一覧テストの導入コメント訂正)、
  `useSidePanelFontScale.test.ts`(「1.4 は同点タイ」コメント訂正)、
  `sidePanelFontScale.ts`(`nearestFontScaleStepIndex` にタイ挙動の説明を追加)
- `pnpm lint`(root)・`pnpm test`(frontend: 214ファイル2809テスト)通過。
  reviewer の指示どおり再度の全工程は行っていない。

### 2026-07-18 Issue #377 再レビュー(reviewer)

- 担当: reviewer
- ブランチ: issue-377-glossary-font-size
- 内容: 差し戻し(タイ挙動コメントの事実誤認3箇所)への対応
  (コミット 54ef217・0d9d6b8)の再レビュー
- レビュー結果: **合格**
  - 差分を全読し、挙動・期待値(アサーション)の変更が無いことを確認。
    変更はテスト名・コメント・実装への説明コメント追加のみ
  - コメント中の数値リテラルを node で再実測し、いずれも JS の最短表現と
    一致することを確認(|1.3-1.4|=0.09999999999999987、
    |1.5-1.4|=0.10000000000000009、|1-1.075|=0.07499999999999996)
  - 中点4値(0.925/1.075/1.225/1.4)のうち厳密なタイは 1.075 のみである
    ことを再実測で確認。「1.075 のみがタイ」という新コメントの一般命題も
    正確
  - 真のタイ 1.075 を専用テストに分離した構成は、先発優先ルールを
    実際にタイが起きる入力で固定するもので、指摘の趣旨どおり
  - `nearestFontScaleStepIndex` への説明コメント追加(推奨事項)も正確
  - コミット粒度: テスト修正(54ef217、test(frontend))と worklog 記録
    (0d9d6b8、docs)が分かれており Conventional Commits 準拠
  - `pnpm lint`(root)・`pnpm test`(frontend: 214ファイル2809テスト、
    1.075 分離で+1)通過を自分でも実行して確認
- これで Issue #377 の静的レビューは合格。以降(QA・PR作成・マージ)は
  統括の判断に委ねる

### 2026-07-18 Issue #377 最終QA検証(qa)

- 担当: qa
- ブランチ: issue-377-glossary-font-size
- 検証方法: frontend をモックモード(`VITE_COLLECTOR_URL` 未設定、vite dev
  server、collector/実チェーン非依存)で起動し、Playwright(chromium)で
  実ブラウザ操作して確認した。純フロントのUI設定機能であり、データ源が
  モックでも実 collector でも描画結果は同一なため、issue-362(幅リサイズ)の
  QAと同じモックモード + Playwright の手当てを採用した(node-env の Docker
  スタックは別エージェントが起動中だったため停止・変更していない)。
- 検証結果: **合格**(完了条件をすべて満たす)。実測値は以下。

用語集パネル(`glossary-open-button` で開く):
  - 初期表示は 100%、ルートの `--side-panel-font-scale` は 1。検索欄の
    実フォントは 13px
  - A+ を押すたびに 100%→115%→130%→150% と1段ずつ拡大し、検索欄の
    フォントも 13→14.95→19.5px と実際に大きくなる。150%(最大)で A+ が
    disabled になり、A− は押せる
  - A− を押し切ると 85%(最小)に達し、フォントは 11.05px に縮小。85% で
    A− が disabled になり、A+ は押せる
  - 現在値表示(85%〜150%のいずれでも)をクリックすると 100% にリセット
    され、フォントも初期値に戻る。リセットボタンは端でも disabled に
    ならないことを確認
  - localStorage の `chainviz.sidePanel.fontScale.v1` が操作に追従して
    1.5 / 0.85 / 1 と保存される
  - 130% に設定してページをリロードし用語集を開き直すと、130% 表示・
    `--side-panel-font-scale`=1.3・フォント 16.9px が維持される(永続化)

3パネル共通(単一キー・共通シェル):
  - 通信ログパネル(`canvas-toolbar-comms-log`)を開くと、リロード後に
    用語集で設定した 130% がそのまま反映されている。通信ログ側で A+ して
    150% に変えると localStorage も 1.5 になり、用語集を開き直すと 150% に
    なっている(kind をまたいで同一の倍率を共有)
  - コントラクトソース表示パネル(コントラクトカードの「ソースコードを
    見る」で開く)も、保存値 1.3 のとき 130% 表示・`--side-panel-font-scale`
    =1.3 が適用される。本文(未保有メッセージ/コード)の実フォントは
    100% で 12px、150% で 18px と約1.5倍に拡大することを確認
- 完了条件との対応: A+/A− での変更・最小/最大での disabled・現在値
  クリックでのリセット・リロードをまたぐ永続化・3パネル共通の倍率適用の
  すべてを実機で確認済み。差し戻す不具合は無い。
- 検証中に落ちたアサーションはいずれも検証スクリプト側のクリック回数
  ミス(130%から85%へ3回押すところ2回にしていた)で、修正後は全項目
  合格した。実装側の不具合ではない。
- `docs/PLAN.md` のチェックボックス更新・PR作成・マージ・Issueクローズは
  行っていない(統括に委ねる)。
