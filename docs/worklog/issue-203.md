...
### 2026-07-10 Issue #203 QA検証記録(検証大地 合格)

- 担当: qa(検証)
- ブランチ: issue-203-ui-d-scenarios(HEAD f3be1c6)
- 目的: Issue #203の完了条件と、ステップ10全体(UI層E2E 32シナリオ)の
  完成を実機で検証する。

#### 実施内容と結果(すべて合格)

1. **UIフルラン(`pnpm test:e2e:ui`)を複数回実行**: 2回の完全ランで
   いずれも32シナリオ全件green(1回目3.8分、2回目4.0分)。UI-D-01〜03を
   含む全シナリオが安定してgreen。なお1回のみ、前回ランからの切り替わり
   直後にフォアグラウンド実行が10分で頭打ちになった(進捗が出ないまま
   停止)が、直後にログ出力付きで再実行したところ4.0分で正常完走した。
   globalSetupのロック待ち/vite起動の一過性の遅延とみられ、テスト自体の
   失敗ではないと判断した。

2. **プロトコル層(`pnpm test:e2e`)を実行**: 稼働5時間・チェーン高9515超の
   長時間稼働スタックに対して実行したところ、`commands.test.ts` の
   PROTO-CMD-01(addNodeのブロック追従)が全体タイムアウト540000msで失敗
   (ターゲット高10100に対し追加rethは5502止まり)。これは既知の
   [Issue #229](https://github.com/morichikawa/chainviz/issues/229)の
   再現であり、本Issueの変更に起因する回帰ではない。残る12件
   (removeNode・d-layer 3・a-b-layer 1・error-paths 4・reconnect 2・
   collector-port-collision 1)はすべてgreen。
   - **短時間稼働スタックでの再検証**: `docker compose down -v && up -d` で
     Ethereumスタックをfresh再起動し、チェーン高を低い状態(高さ約5)に
     戻してから `commands.test.ts` のみ再実行したところ、PROTO-CMD-01を
     含む2件ともgreen(addNode追従7.8秒)。これによりPROTO-CMD-01の失敗が
     稼働時間依存(#229)であり、実装の欠陥でないことを確認した。

3. **ユニット/静的確認(`pnpm build && pnpm lint && pnpm test`)**: いずれも
   green。ユニットテストは shared 58・e2e(unit) 77・collector 1103・
   frontend 1372 が全passed。

4. **実ブラウザでの目視確認**: dev環境(collector+vite dev、fresh再起動後の
   スタック)を起動し、Playwright(chromium)で実DOMとスクリーンショットを
   取得して以下を目視確認した。
   - **UI-D-01(内部リンクエッジの常設・別系統の見た目)**: beacon1→reth1の
     エッジがクラス `react-flow__edge-internalLink internal-link-edge` を
     持ち、ピアエッジのクラス(`peer-edge`/`react-flow__edge-peer`)を
     持たないこと、専用の芯要素(`.internal-link-edge__core`)が1個で二重線
     (path 3本 = 鞘+芯)になっていること、ストローク色が rgb(201,212,232)の
     低彩度(無彩色系)であることを確認。スクリーンショット上でも、青破線・
     シアン実線のピアエッジとは異なる中立色の二重線として描かれていた。
   - **UI-D-02(活動パルスの周期性)**: 内部リンクエッジ上の
     `.internal-link-pulse` の個数を約0.3秒間隔でサンプリングし、
     0→1(3.0秒)→0(4.0秒)→1(8.2秒)と出現→消滅→再出現の周期性を実観測。
     スクリーンショットでもエッジ上に白く光るパルスが確認できた。
   - **UI-D-03(同期ステージ・txpool内訳)**: reth1カードにホバーした
     ポップオーバーに同期ステージ15行がすべて数値のチェックポイント
     (210〜417)付きで表示され、txpool行が「pending 0 · queued 0」で
     表示されることを確認。

#### 判定

Issue #203の完了条件、およびステップ10全体(UI層E2E 32シナリオ)の完成を
満たしていると判定する。プロトコル層のPROTO-CMD-01の失敗は既知の#229
(長時間稼働スタック依存)で、短時間稼働スタックでの再検証でgreenを確認
済みのため、ステップ10の完了条件「プロトコル層は引き続きgreen」を
満たすものと判断する。

#### 申し送り(検証中の環境操作)

- 短時間スタックでの再検証のため、共有のEthereumスタックを
  `docker compose down -v && up -d` でfresh再起動した(チェーン高が
  リセットされている。使い捨て許可の範囲内)。検証後もスタックは稼働
  したまま残している。
