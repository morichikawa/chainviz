...
### 2026-07-11 Issue #251 レビュー（chainviz-reviewer）
...
- UX設計の反映: 設計メモ §4-5 の要件どおり。1段目は既存の
  `resolveAddNodeHint` の結果を無変更で使い、2段目は
  `action.addNode.hint.pair.prefix/term/suffix` の3分割
  （`internalEdge.pair.*` と同じ前例）+
  `GlossaryTerm termKey="el-cl-separation"` で組み立てられている。
  各段は `.action-hint__line`（display:block）で改行され、
  ワークベンチ追加ボタン側の hint は変更されていないことを確認した
- `ActionHint` の `hint: string → ReactNode` 拡張: 呼び出し箇所は
  `CanvasToolbar`（2ボタン）と `InfraNodeCard` の3か所で、文字列を渡す
  既存2か所は `string ⊂ ReactNode` のため非破壊。開閉ロジック
  （`useHoverPopover`）は無変更。型拡張の理由が JSDoc コメントに
  残されている点も適切
- テスト強化担当の jsdom 判断: 妥当。React の合成 mouseenter/mouseleave
  は React ツリー基準で発火し、`relatedTarget` 省略（null）は「文書外への
  離脱」と解釈されて祖先チェーン全体に mouseleave が発火するため、実
  ブラウザの「用語から外側ツールチップ本体へ戻る」動きを再現できない。
  `relatedTarget` に外側ツールチップを与える書き換えは実挙動の忠実な
  モデル化であり、`GlossaryTerm` と `ActionHint` が独立した
  `useHoverPopover` を持つ実装構造とも整合する。遅延時間も定数
  `HOVER_POPOVER_CLOSE_DELAY_MS` を参照しており決め打ちの ms 値がない
- i18n の一貫性: `glossary/ethereum/terms/d-internal.yaml` の
  `el-cl-separation` と照合し、用語名（ja「EL/CL分離」/ en
  「EL/CL separation」）・言い回し（ja「The Merge 以降の標準構成」/ en
  "the standard shape since The Merge"）が一致することを確認した。
  括弧は既存 UI 文言と同じ半角
- ビルド・lint・テスト: リポジトリ全体で `pnpm build` / `pnpm lint` /
  `pnpm test` がすべて通ることを確認した（frontend は 118ファイル・
  1835件、テスト強化前から11件純増）
- テストの質: 文言・DOM 構造（`data-testid` アンカー、2段目がちょうど
  1つ）・言語切り替え・ワークベンチ側への非漏洩（回帰）・部分的な
  ブートノード解決（境界）・nullish/数値の ReactNode（異常系）を実出力
  ベースで検証しており、実装を壊せば落ちる内容になっている。
  `CanvasToolbar.test.tsx` の分割と `canvasToolbarHarness.tsx` への
  共通化は1ファイル1責務の方針に沿う
- エラー握りつぶし・環境依存の固定値: 本変更の差分には catch 節・
  タイムアウト定数の新設はなく、該当なし
- コミット粒度: 8コミットすべて Conventional Commits 準拠で、型拡張 /
  i18n キー追加 / ツールチップ2段化 / テスト強化（2件）/ docs が
  それぞれ分かれており適切
- docs: `docs/PLAN.md` のチェック + Issue リンク、`docs/WORKLOG.md` の
  索引行、本ファイルの UX設計・実装・テスト強化の各記録を確認した

#### 非ブロッキングの備考

- `docs/WORKLOG.md` の #251 索引行は UX 設計時に書かれたもので、末尾が
  「実装はfrontendへ引き継ぎ」のままになっている。実装・テスト強化まで
  完了した現状とは記述が一足遅れているが、索引行の要約の鮮度の問題で
  あり実装との齟齬ではないため差し戻し理由とはしない（マージ前に統括の
  判断で一言更新してもよい）

### 2026-07-11 Issue #251 QA検証記録（chainviz-qa）
...
レビュー・実装担当の環境では Chromium の共有ライブラリ不足で実ブラウザ確認が
できていなかったが、本環境では過去セッションが展開済みの依存ライブラリ
（`/home/zoe/chrome-deps/root/usr/lib/x86_64-linux-gnu`）を `LD_LIBRARY_PATH`
に指定することで Playwright（playwright-core 1.56）+ Chromium（chromium-1228）
の実ブラウザ起動に成功した。`pnpm --filter @chainviz/frontend dev`（モック
モード、`VITE_COLLECTOR_URL` 未設定）で起動したフロントを実ブラウザで開き、
ホバー操作を発火させて以下を確認した。

#### 確認結果（依頼の4要件）

1. ノード追加ボタンのホバーで2段構成のツールチップが表示されること: **OK**
   - 1段目（何が起きるか）: 「フォロワーノード(reth + beacon のペア、カード2枚)
     を起動します。chainviz-reth-1 と chainviz-lighthouse-1 を入口(ブートノード)に
     既存ネットワークへ参加し、同期後は他のノードとも自動で繋がります」
   - 2段目（なぜペアか）: 「2枚で1つのノードです。実行(EL)と合意(CL)を別々の
     クライアントが担うのは The Merge 以降の Ethereum の標準構成(EL/CL分離)です」
   - 2段目に `data-testid="glossary-term-el-cl-separation"` のアンカーが1つ存在
   - `.action-hint__line--secondary`（2段目）がちょうど1つ。区切り線でも視覚的に
     1段目と2段目が分かれていることをスクリーンショットで確認
2. 2段目の「EL/CL分離」へカーソルを移すとネスト用語ポップオーバーが開き、
   外側ツールチップが閉じないこと: **OK**（元Issueの核心。実ブラウザで確認）
   - `glossary-popover-el-cl-separation` が開き、本物の glossary 定義文
     （`glossary/ethereum/terms/d-internal.yaml` の `el-cl-separation`）を表示
   - このネストホバー中も `role="tooltip"` 要素が2つ同時に存在し、外側の
     ActionHint ツールチップ（2段構成）は開いたまま。スクリーンショットで
     外側ツールチップ+下部にネスト用語ポップオーバーが両方開いた状態を確認
3. ワークベンチ追加ボタンには2段目が出ない（1段目のみ）こと: **OK**（回帰）
   - ワークベンチ追加ボタンのホバー時、`.action-hint__line--secondary` は0、
     glossary アンカーも0
4. 日本語/英語の言語切り替えで文言が正しく切り替わること: **OK**
   - ja: 「…The Merge 以降の Ethereum の標準構成(EL/CL分離)です」
   - en: "The two cards form one node — running execution (EL) and consensus
     (CL) as separate clients has been the standard shape of an Ethereum node
     since The Merge (EL/CL separation)."
   - 言語トグルで `data-lang` が ja→en に切り替わり、文言も追随することを確認

自動チェック10項目すべて合格（1段目/2段目/アンカー存在/ネストポップオーバー
展開/外側ツールチップ非クローズ/ワークベンチ側非漏洩×2/言語トグル/ja文言/
en文言）。

#### 完了条件の判定

元Issue（なぜペアで追加されるかの説明がツールバー付近に無い）は解消されて
いる。ノード追加ボタンのホバーだけで「2枚=1ノード」「EL/CL分離は The Merge
以降の Ethereum の標準構成」という Why が読め、用語アンカーからさらに詳細な
定義まで（外側ツールチップを閉じずに）辿れることを実ブラウザで確認した。
差し戻しは不要。
