...
### 2026-07-18 Issue #351 QA差し戻し対応の静的レビュー

- 担当: reviewer
- ブランチ: issue-351-parent-block-hover-highlight
- 対象: QA差し戻し対応分の4コミット（120457f 設計メモ / 0fd6696
  fix(frontend) 自己強調抑制 / 9acb85c fix(e2e) UI-B-05識別ベース検証 /
  96318ce 実装完了記録）
- 結果: **合格**（差し戻しなし）
- 確認した内容:
  1. **「実装ロジック自体を直す」判断の妥当性（最重要項目）**: 妥当と
     判定。根拠は (a) UX設計メモ §3 の記述は「直前タイルが強調され」
     「行の parentHash 短縮表示と強調タイルのハッシュ表示が同じ値である
     ことを見比べられる」といずれも単数の強調を前提としており、
     「ホバー中タイル自身も同時に光る」仕様はどこにも存在しない
     （二重強調は `hoveredBlockHash` の兼用から生じた副作用）、
     (b) QA自身も差し戻し時に「UX目的を損なうなら product 側で reverse
     強調の扱いを見直す」を選択肢として提示しており、今回の対応はその
     選択肢の採用にあたる、(c) 変更は既存の強調クラスを「誰に付けるか」
     の条件を絞るだけで、新しい視覚要素・文言・CSS・操作概念を一切
     導入していない。したがって「既存デザインの意図しない漏れを塞ぐ
     エンジニアリング修正」という位置づけは正確で、chainviz-ux への
     再調整は不要と判断する（元設計の単数強調へ収束させる修正であり、
     設計を変える修正ではないため）。なお「ホバー中タイル自身を別
     スタイルで示したい」等の新しい要望が将来出た場合は、それは新規の
     UX設計タスクとして扱うべき（本修正の範囲外）
  2. **`onParentHover` の送り主 hash 引き渡しの実装**: `handleParentHover`
     は `useCallback`（依存配列空）で同一性が安定しており、
     `ChainRibbonPopover` の unmount クリーンアップ `useEffect`（依存
     `[onParentHover, block.hash]`）は実質 mount/unmount の2回しか
     走らない。`parentHighlight` を `{sourceHash, targetHash}` の組で
     1つの state に持つ設計は「送り主と対象は常に同時に確定する」実態と
     一致し、null 化も1操作で済む。抑制条件
     `hoveredBlockHash === block.hash && !isDrivingParentHighlight` は
     送り主タイルにしか影響せず、tx チップ由来の逆方向強調・親行を見て
     いない直接ホバーは無変更（`chainRibbonCrossHighlight.test.tsx` は
     本ブランチで無変更のまま全通過。実装完了記録の UI-B-06 実機合格とも
     整合）。強調が単一 state から導出されるため「同時に光るのは高々
     1タイル（＋窓外なら「⋯」）」が構造的に保証される
  3. **jsdom の `mouseLeave` 祖先伝播の回避**: 正しい代替と判定。React は
     enter/leave を mouseover/mouseout の target と relatedTarget の
     React ツリー上の共通祖先から合成するため、`relatedTarget` 未指定の
     `fireEvent.mouseLeave(row)` は「どこにも属さない場所への離脱」と
     解釈され祖先（タイル div）の leave まで発火する。
     `fireEvent.mouseOut(row, { relatedTarget: tileElement })` はタイルが
     共通祖先となり行の leave だけが発火し、「行を離れてタイル上に留まる」
     実挙動の正確なシミュレーションになる（UX設計メモのテスト観点でも
     `relatedTarget` の必要性は予見されていた）。既存テストの素朴な
     `mouseLeave` を今回触らなかった判断も、アサーション対象に影響しない
     ことが確認済みのため許容範囲（worklog に知見として記録済み）
  4. **`data-parent-hash` の追加**: 許容範囲と判定。本番相当の DOM には
     既に `data-testid`・`data-connected-to-previous` が同じ用途（テスト
     専用の識別）で常時出力されており、既存の流儀と一貫している。値は
     同じ行に短縮表示している parentHash の完全形で情報漏れの懸念もなく、
     shortHex が逆引き不能なため属性露出が必要という理由もコード
     コメントに明記されている
  5. **回帰テストの failing-first 確認**: worklog の記述をレビュー側でも
     追試した。実装2ファイル（`ChainRibbonCard.tsx` /
     `ChainRibbonPopover.tsx`）だけを差し戻し前（0fd6696^）に一時的に
     戻して該当2テストファイルを実行し、新規の自己強調抑制テスト2箇所が
     期待どおり `not.toContain("chain-ribbon-tile--highlight")` で失敗する
     （Received に --highlight が残る）ことを確認、復元後は 17/17 通過。
     worklog の記述は正確
  6. **コミット粒度・形式**: 4コミットが設計メモ / frontend 実装 / e2e /
     完了記録に分かれ、すべて Conventional Commits 形式。frontend 実装と
     e2e 変更を分けた粒度も適切
  7. **品質ゲート**: `pnpm lint` / `pnpm build` / `pnpm test` を
     リポジトリ全体で実行し全通過（shared 74 / collector 1597 / e2e 179 /
     frontend 2642。実装完了記録の数値と一致）
  8. **docs 整合**: `docs/ARCHITECTURE.md` §10.3（ポップオーバーの
     ホバー領域化・凍結条件）は強調の個数に言及しておらず矛盾なし。
     `packages/e2e/SCENARIOS.md` UI-B-05 はシナリオ粒度の記述
     （実マウス移動・強調の持続）のままで、hover() への置き換え後も
     記述と実装は矛盾しない。エラー握りつぶし箇所・チェーン固有語彙の
     漏れ・`packages/shared` の型変更はいずれもなし
- 注意点（差し戻し対象ではない申し送り）:
  - e2e の `parentInWindow` 判定（`parentTile.count()`）とその後の
    アサーションの間で表示窓が動かないことは、ポップオーバー開放中の
    凍結（`openPopoverHashes`）が保証している。凍結条件を将来変更する
    場合はこの e2e の前提も崩れる点に留意
  - 前回レビューで指摘した UI-B-05 の `.last()` 再解決の理論上の競合は
    今回も残っているが、既存パターンの踏襲であり flaky が観測されたら
    改善を検討する扱いのまま
  - 実 Docker での UI-B-05 3回連続合格は実装担当の自己検証。正式な
    再QA（chainviz-qa）は未実施のため、次工程で実施すること

### 2026-07-18 Issue #351 QA差し戻し対応の実機再検証（合格）

- 担当: qa
- ブランチ: issue-351-parent-block-hover-highlight
- 結果: **合格**（Issue #351の完了条件を満たしている）
- 検証環境: 既存の稼働中 `profiles/ethereum` スタックを再利用。開始前に
  他エージェントのe2e実行が無いこと（`chainviz-test-e2e.lock` 不在・
  playwright/chromiumのテストプロセス不在）を確認。チェーン進行を
  `eth_blockNumber` で確認（0x226→0x227→0x228→0x229、約13秒/ブロック）。
  Playwright chromium は
  `LD_LIBRARY_PATH=/home/zoe/chrome-deps/root/usr/lib/x86_64-linux-gnu` を
  付与して起動。

#### 要件1（実マウス操作での挙動確認）: 合格

使い捨ての診断spec（`packages/e2e/src/ui/_qa351_probe.spec.ts`、検証後に
削除済み・コミットせず）で実マウス（`Locator.hover()`）でタイル→
ポップオーバー→「親ブロック」行と辿り、DOMのクラスを直接ダンプして確認した。

- 親が表示窓内のケース（中間タイルを狙って親を必ず窓内にした）で、
  3サイクル連続で以下を確認:
  - 強調される `.chain-ribbon-tile--highlight` は常にちょうど1件で、その
    要素は**親タイル**（例: hovered=0x5c6168dc に対し highlighted=0x332ed64c
    = data-parent-hash と一致）。**ホバー中タイル自身は強調されない**
    （selfHighlight=false）。今回の修正の核心である二重強調の解消を
    実DOMで確認した
  - ポップオーバーは遅延クローズ猶予（200ms）を超える400ms静止後も表示維持
  - マウスを離すと強調は0件・ポップオーバーも消える。3サイクルとも固着なし
- 親が表示窓外のケース（最新タイルを狙うと親が窓外になった）でも、3サイクル
  連続で強調タイルは0件・「⋯」インジケータ（`chain-ribbon-card__older`）に
  `--highlight` が付き、ホバー中タイル自身は非強調、解除で全て消えることを
  確認した

#### 要件2（UI-B-05・UI-B-06のe2e実行）

- **UI-B-05**: 実Dockerスタックで3回（併走ラウンド1・2・3）実行し、3回とも
  合格。前回QAで不合格だった `toHaveCount(1)` 起因の決定的失敗は、識別ベースの
  検証（`data-parent-hash` から親hashを解決し、親が窓内なら親タイルのみ・
  窓外なら「⋯」のみに強調）への置き換えで解消しており、安定して合格する。
  Issue #351の中核であるUI-B-05は盤石。
- **UI-B-06**: 単独実行では3回とも合格。ただしUI-B-05との併走実行では
  ラウンド1（正方向: タイル→ウォレットカード強調が付かず）・ラウンド2
  （逆方向: txチップ→タイル強調が0件）で失敗、ラウンド3は合格という
  間欠的なflakyを観測した。
  - このflakyは**Issue #351のコード変更に起因しない**と判断した。根拠:
    (a) UI-B-06のテスト本体はこのブランチで未変更（`git diff origin/main...HEAD`
    のspec差分はすべてUI-B-05内のハンク）、(b) 今回変更した
    `isReverseHighlighted = hoveredBlockHash === block.hash &&
    !isDrivingParentHighlight` は、UI-B-06中は「親ブロック」行をホバーしない
    ため `isDrivingParentHighlight` が常にfalseで、逆方向強調ロジックは
    #351前と挙動が同一、(c) UI-B-06単独では3/3合格。
  - flakyの実態は、UI-B-06が実送金→txのブロック取り込み待ち→ホバーを直列に
    行う構造上、対象ブロックが表示窓（直近8タイル）から流れ出るまでの時間との
    競合であり（`docs/worklog/issue-298.md` に既出のfitview/表示窓経年の
    課題）、併走実行時の負荷でこの競合を跨ぎやすくなる、既存由来の
    タイミング依存と考えられる。
  - 推奨: このUI-B-06のflaky（併走時のタイミング競合）は#351とは別の
    課題として別Issueで追跡することを提案する。#351のマージをブロックする
    ものではない（#351のコード・UI-B-06本体いずれも原因ではないため）。

#### 完了条件との照合

- 要件1（実マウスでの200ms超維持・タイル→ポップオーバー移動で閉じない・
  二重強調しない・解除で固着しない・「⋯」強調）: **すべて満たす**
- 要件2（UI-B-05を実際に実行して通過）: **満たす**（3/3）。UI-B-06は
  #351非依存の既存flakyを観測したが、#351の合否判定には影響しない
- 前回QAで不合格だった二重強調（強調タイルが1でなく2）は解消済み

#### 申し送り

- `docs/PLAN.md` のチェックボックス更新は統括が行う（QAでは更新しない）。
- UI-B-06の併走時flakyは別Issue化の判断を統括に委ねる。
