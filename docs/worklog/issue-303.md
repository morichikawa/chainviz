...
### 2026-07-13 差し戻し修正着手前の方針確認（collector）

reviewer の指摘・修正指示をそのまま採用する。実装に落とす際の具体的な関数構成を
以下に記録する。

- `applyTransaction` の分岐条件を `tx.blockHash !== undefined` から
  `tx.status !== "pending"` に変更する。`evictExcessPendingTransactions` は
  既に `status === "pending"` で判定しているため、これで両者の判定基準が
  一致する。
- `tx.status !== "pending"`（included/failed）の枝の中で、まず
  `tx.blockHash === undefined` かどうかを見る。真の場合はアダプタ契約違反の
  不正入力とみなし、`applyNodeInternals` と同じ形式（`console.error` +
  具体的な識別子を含める）でログし、空配列を返して捨てる。`blockHash` が
  あれば従来どおり block 存在の入口ガード（`this.entities.get(tx.blockHash)`
  が `kind === "block"`）を適用する。
- `tx.status === "pending"` の枝は変更しない（従来どおり `applyKeyed` の後に
  `evictExcessPendingTransactions` を呼ぶ）。
- この変更は、アダプタが実際に生成する正常入力（included/failed は必ず
  blockHash あり、pending は必ず blockHash なし）に対して分岐結果が完全に
  一致するため、既存の正常系テストへの影響はない見込み。差分が出るのは
  「included/failed だが blockHash が undefined」という現状到達不能な
  不正入力のみ。
- テストは新規ケースとして、この不正入力を `applyTransaction` に渡し、
  (1) 返り値が空配列、(2) `hasTransaction` が false のまま、(3) pending
  としてもカウントされない（`evictExcessPendingTransactions` の対象に
  ならない）、(4) `console.error` が呼ばれる、の4点を検証する。既存の
  `store-transaction-retention.test.ts` に追記する（「入口ガード」の関心事の
  一部であり新規ファイルを立てるほどの分量ではないため）。
- `docs/ARCHITECTURE.md` §10.4 の入口ガードの説明を、「included/failed の
  判定は `blockHash` の有無ではなく `status` で行う。`status` が
  included/failed なのに `blockHash` が無い場合はアダプタ契約違反としてログし
  捨てる」旨に更新する。

### 2026-07-13 差し戻し修正の実装（collector）

- 担当: collector
- ブランチ: issue-303-transaction-retention
- 内容: reviewer の指摘どおり、`packages/collector/src/world-state/store.ts` の
  `applyTransaction` の分岐条件を `tx.blockHash !== undefined` から
  `tx.status !== "pending"` に変更した。これで `evictExcessPendingTransactions`
  の判定基準（`status === "pending"`）と一致する。
  - `tx.status !== "pending"`（included/failed）の枝に入った際、
    `tx.blockHash === undefined` であれば、アダプタ契約違反の不正入力として
    `console.error` で tx hash を含めてログし、空配列を返して捨てるように
    した（`applyNodeInternals` と同じ「境界でログして捨てる」形式）。
    `blockHash` があれば従来どおり block 存在の入口ガードを適用する。
  - `tx.status === "pending"` の枝は変更していない。
  - この変更により、旧実装で「pending 分岐に紛れて pending cap にも block
    連動の退去にも掛からず無制限蓄積する」可能性があった不正入力
    （status が included/failed なのに blockHash が undefined）は、
    取り込まれずログ付きで破棄されるようになった。
- テスト: `store-transaction-retention.test.ts` に新規 describe ブロック
  「malformed input: status is included/failed but blockHash is missing」を
  追加し、以下を検証する4ケースを実装した。
  1. included かつ blockHash なしの tx が取り込まれず、tx hash を含む
     `console.error` が呼ばれること
  2. failed かつ blockHash なしの tx についても同様
  3. この種の不正入力を大量（300件）に流しても pending cap
     （`PENDING_TX_RETENTION = 256`）の枠を消費しないこと（もし旧実装のまま
     なら、この不正入力は pending 扱いされず無制限に蓄積する）
  4. 同じ hash の tx が後で正しい blockHash 付きで再配信されれば、通常どおり
     取り込まれること（恒久的な取りこぼしにはならない）
  - 回帰確認: これらのテストを追加前（修正前のコード、`tx.blockHash` 判定の
    まま）に対して実行すると、ケース1・2・3が実際に失敗することを確認した上で
    修正を適用し、再実行して全て通ることを確認した。
- `docs/ARCHITECTURE.md` §10.4 を更新し、included/failed の判定基準が
  `blockHash` の有無ではなく `status` であること、および `blockHash` が
  欠落した不正入力をログして捨てる旨を追記した。
- ビルド・テスト: `pnpm --filter @chainviz/collector build` および
  `pnpm --filter @chainviz/collector test` が通ることを確認した
  （57 test files / 1412 tests。差し戻し前の 1408 + 新規 4）。
- 実機確認（既存の正常系への非退行確認）: 稼働中の `profiles/ethereum`
  Docker スタックに対し、他ブランチの collector プロセスとポートが衝突
  しないよう `CHAINVIZ_COLLECTOR_PORT=4100` / `CHAINVIZ_PROXY_PORT=4101` で
  collector を起動し、WebSocket 経由で `runWorkbenchOperation`
  （`transfer`）を実行した。tx が pending → included へ正しく遷移し、送金元
  ウォレットの `recentTxHashes` にも反映されることを確認した
  （`console.error` は出力されず、正常入力に対する挙動が変わっていないことを
  確認）。検証後、この確認用 collector プロセスのみ停止した。


### 2026-07-13 再レビュー（reviewer）: 合格

- 担当: reviewer（横断レビュー・静的整合性）
- ブランチ: issue-303-transaction-retention
- 判定: **合格**（前回差し戻した判定基準の食い違いが解消されたことを確認）。

#### 前回指摘（判定基準の食い違い）の解消確認
- `applyTransaction` の分岐が `tx.blockHash !== undefined` から
  `tx.status !== "pending"` に変更され、`evictExcessPendingTransactions` の
  判定基準（`status === "pending"`）と一致した。両者が同じ基準を使うことで、
  「status が included/failed なのに blockHash が undefined」の不正入力が
  pending 分岐へ紛れ込む経路が塞がれた。
- 不正入力は `console.error`（tx hash と status を含む具体的なメッセージ）で
  ログしたうえで空差分を返して破棄する。`applyNodeInternals` の防御スタイルと
  一致しており、握りつぶし（catch して無言・汎用メッセージへのすり替え）には
  該当しない。破棄する理由はコード内コメントと ARCHITECTURE.md §10.4 の両方に
  明記されている。

#### テストコードの質
- `store-transaction-retention.test.ts` に追加された4ケースは、実際の欠陥
  シナリオを検証している。特に「不正入力を300件流しても pending cap の枠を
  消費せず store に0件のまま」というケースは、旧実装の無制限蓄積バグを
  直接検出する（実装担当が回帰確認済み）。ハッピーパスのなぞりではない。
- included/failed の双方・ログ内容（hash 含有）・正しい blockHash 付き
  再配信での取り込み回復まで網羅しており、境界を押さえている。

#### ビルド・lint・テスト（リポジトリ全体）
- `pnpm build` 成功。`pnpm lint` クリーン。`pnpm test` 全 green
  （shared 4 test files / collector 57・1412 tests / e2e 13 / frontend
  143・2120 tests）。collector のログ出力はエラー系を検証するテストの
  想定内 stderr で、失敗ではない。

#### コミット粒度・docs
- 差し戻し対応の4コミット（fix / test / ARCHITECTURE.md 更新 / worklog）は
  それぞれ単一の関心事に分かれており、Conventional Commits に準拠する。
- ARCHITECTURE.md §10.4 は status ベースの判定基準・不正入力のログ付き破棄を
  正しく反映している。

#### 軽微な気づき（差し戻しには当たらない）
- `evictExcessPendingTransactions` の docstring が pending tx を
  「`blockHash` なし」と旧来の枠組みで説明している箇所が残る（実際の
  フィルタは `status === "pending"`）。正常入力では両者が等価なため機能上の
  問題はないが、判定基準を status に統一した今回の変更の趣旨からは表現が
  やや古い。次に同ファイルへ触れる機会があれば揃えるとよい（今回の合格を
  妨げるものではない）。

- QA（実 Docker 環境での動作検証）は次工程の chainviz-qa に委ねる。

### 2026-07-13 QA（実機検証）: 合格

- 担当: qa（実 Docker 環境での動作検証）
- ブランチ: issue-303-transaction-retention
- 判定: **合格**。`docs/PLAN.md` の該当項目の完了条件（送金操作による tx の
  正常な取り込み・ブロック退去に連動した tx の退去）を満たしていることを、
  実際に環境を動かして確認した。

#### 検証環境
- `profiles/ethereum` の Docker スタック（既に稼働中のものを再利用。チェーンは
  約 2 秒に 1 ブロックのペースで進行しており、検証開始時点で 500 番台まで
  到達済み）に対し、`pnpm dev:up` で collector（ws://127.0.0.1:4000）と
  frontend（http://127.0.0.1:5173）を起動した。collector の dist は本検証の
  対象コミットで再ビルドしてから起動した（`a0a5a71..HEAD` の差分は worklog
  のみでコード差分なしを確認済み）。
- WebSocket 経由でスナップショット・差分を受信するクライアントスクリプトで
  疎通・状態を確認した。frontend は HTTP 200・`<title>chainviz</title>` を配信。

#### 確認できたこと
1. 疎通: collector からスナップショットが仕様どおりの形（entities/edges）で
   届き、以降 diff が流れることを確認した。
2. 送金の非退行（tx pending → included・ウォレットカード反映）: workbench
   （`chainviz-ethereum/workbench`）に対し金額の異なる `transfer` を 5 回実行
   したところ、5 件すべて成功し、各 tx が `pending -> included` へ遷移、
   送金元ウォレットの `recentTxHashes` に 5 件すべて反映された。
3. ブロック退去に連動した tx の同時退去（#298 の保持窓との連動）: 約 95 秒間
   diff を監視したところ、
   - block の保持件数は終始 32 件で一定（min=max=32、最大 span 31）で、
     `BLOCK_RETENTION=32` により有界であることを確認。観測期間中に 49 個の
     block が窓落ちで `entityRemoved` された。
   - included tx が窓落ちする際、その tx は対応する block と**同一の diff の
     中で** `entityRemoved` された（block と別々の diff に分かれて orphan として
     残るケースは 0 件）。
   - 「included/failed tx が store に在る ⇔ その block が store に在る」の
     不変条件は監視期間を通じて破れなかった（違反 0 件）。
   - 監視開始時点で store 内の tx は 1 件のみ（先に作成した 5 件のうち 4 件は
     既に対応 block の窓落ちで退去済み）であり、tx が無制限に蓄積せず
     block 保持窓に連動して有界に保たれていることを確認した（本 Issue の主目的）。
4. collector ログにエラー・警告なし（同一内容の送金を重複送信した際の
   `already known`（同一 nonce）を除く。`malformed transaction observation` の
   `console.error` は正常入力では出力されず、不正入力ハンドリングが通常運転に
   影響しないことも確認）。

#### 検証中に遭遇した環境要因（#303 とは無関係。参考記録）
- 検証開始直後、`runWorkbenchOperation`（transfer）が collector 経由の
  `docker exec` で "current working directory is outside of container mount
  namespace root -- possible container breakout detected"（runc 1.3.4 の
  CVE-2024-21626 緩和）で失敗した。原因は稼働中スタックが別 worktree
  （wt-issue-301）の `profiles/ethereum/contracts` を bind mount していて、
  その mount ハンドルが古くなり workbench コンテナ（WorkingDir=`/contracts`）
  の exec が弾かれていたこと。workbench コンテナのみ再起動したところ復旧し、
  以降の送金は正常に動作した。これは #303 のコード変更（store.ts の tx 保持）
  とは無関係な実行環境（マウント状態）の問題である。
