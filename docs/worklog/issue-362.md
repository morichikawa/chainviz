...
### 2026-07-18 Issue #362 差し戻し対応の再確認（reviewer）

- 担当: reviewer
- ブランチ: issue-362-sidepanel-resize
- 内容: 前項の差し戻し指摘1件（`docs/ARCHITECTURE.md` §12.2 の
  範囲外値の扱いの記述）に対する修正コミット 09bcf43 の目視確認。
- 確認結果: 差分は指摘した1文のみ（他ファイル・他行への変更なし）。
  修正後の記述「読み込み時は非数などの壊れた値は既定値へフォール
  バックし、範囲外の値は現在の [最小, 最大] 範囲へクランプする」は
  `loadSidePanelWidth` の実挙動（非有限値→既定420、範囲外の有限値→
  クランプ。テストで固定済み）と一致。コミットメッセージも
  Conventional Commits 形式・単一関心事で規約適合。
- 判定: **合格**。実装・テスト・docs の全項目で差し戻し事項なし。
  push / PR 作成 / マージは統括に委ねる。

### 2026-07-18 Issue #362 実機QA検証（qa）

- 担当: qa
- ブランチ: issue-362-sidepanel-resize
- 内容: 実ブラウザ（Playwright/chromium）で幅リサイズの実挙動を検証した。
  稼働中の共有Ethereumスタック（12秒スロットでブロック進行を確認。
  0x2b5→0x2b8）を冪等に再利用し、E2Eは専用ポート（collector 4125 /
  vite 5275）で隔離実行した。既存のdev collector(4000)・dev vite(5173)は
  停止・変更していない。
- 検証環境の注意点: この環境のPlaywrightブラウザは `libnspr4.so` 等の
  共有ライブラリが未導入で、そのままでは chromium が起動できず
  `error while loading shared libraries: libnspr4.so` で失敗する。
  `/home/zoe/chrome-deps/root/usr/lib/x86_64-linux-gnu` を
  `LD_LIBRARY_PATH` に追加することで起動できた。UI層E2E（`test:e2e:ui`）は
  pre-pushフック（`lint && build && test`）の対象外であり、CI相当の
  自動実行がされないため、UI E2Eを回す際はこのライブラリ手当てが必要。
- 検証結果（すべて完了条件を満たす）:
  1. `packages/e2e/src/ui/side-panel-resize.spec.ts`（UI-PANEL-01）:
     通信ログパネルを開き、ハンドルを左へ150pxドラッグ→幅が+100px超
     広がることを確認。リロード後も同じ幅で再表示（localStorage
     `chainviz.sidePanel.width.v1` 永続化）を確認。**合格**（1 passed, 1.8s）
  2. キーボード操作: ハンドルにフォーカスし←キーで420→444（+24）、
     →キー2回で444→396（-48）。24px刻み・←広く/→狭くを確認
  3. 最小/最大クランプ: viewport 1280px で→連打→300pxで下限クランプ、
     ←連打→1152px（=1280×0.9）で上限クランプ。ハンドルの
     `aria-valuemin=300` / `aria-valuemax=1152` も一致
- reviewer申し送り2点の実機確認（いずれも再現した）:
  - 右ボタンドラッグ: ハンドル上で右ボタンを押して150px移動すると、
    パネル幅が実際に150px変化した（`handlePointerDown` が
    `event.button` を見ないため右ボタンでもドラッグが開始する）。ただし
    薄いハンドル(8px)上で右ドラッグする操作自体が稀で、通常のコンテキスト
    メニュー操作と競合する程度。体験上の実害は小さい
  - ドラッグ中のテキスト選択: 通常の左ボタンで右方向（狭める向き）に
    ドラッグすると、ポインタが本文上を通過してパネル本文テキストが
    178文字選択された（ドラッグ中の `user-select` 抑止が無いため）。
    リサイズ後にテキストがハイライトされたまま残る。通常操作（左ボタンで
    狭める）で発生するぶん、右ボタン件より目につきやすい軽微な見た目の
    粗さ。ただし機能自体（幅変更・永続化）は正しく動作する
- 判定: **合格**。Issue #362 の完了条件（ドラッグでの幅リサイズ・
  最小300/最大90vwのクランプ・リロード後の永続化・キーボード操作・
  a11y）はすべて実機で満たしている。
- 差し戻し: なし。申し送り2点は本Issueの完了条件を損なわないため
  差し戻し対象外。ただしドラッグ中テキスト選択の抑止（`resizing` 中に
  `user-select: none`）と右ボタンドラッグのガード（`event.button !== 0`
  で無視）は、実機で軽微ながら再現するUX上の粗さのため、frontend担当の
  別Issueとして分離することを推奨する。
