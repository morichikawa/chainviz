...
### 2026-07-17 Issue #313 レビュー（静的整合性・テストの質）

- 担当: reviewer
- ブランチ: issue-313-glossary-panel
- 判定: **合格**（軽微な docs 齟齬1件のみ。下記「指摘」参照。実装・テストの
  差し戻しは不要）

#### 確認したこと

- **境界の遵守 / shared の範囲**: `packages/shared`・`packages/collector` への
  変更は無し（`git diff main..HEAD` で確認。設計どおり frontend 単独）。
  チェーン固有語彙の漏れも無し。パネルは「読み込み済みの glossary 全件」を
  汎用に表示する作りで、プロファイル名のハードコードは無い
- **ダングリングガード修正の正しさ**: `SidePanelHost.tsx` の
  `dangling = view?.kind === "contractSource" && contract === undefined` は
  正しい。修正前は `view !== null && contract === undefined` で、glossary
  view では `contract` が常に `undefined` になるため開いた瞬間に自動クローズ
  される自己矛盾があった。修正後も contractSource 側の防御は維持されている
  ことを、`SidePanelHost.glossary.test.tsx` の cross-kind テスト（glossary →
  参照切れ contractSource で正しく閉じる / world state 変動・エンティティ
  消失を跨いで glossary が開いたまま）が固定している。tester が「修正前に
  戻すと11件全て失敗する」検出力確認を実施済み（本ファイル前節）で、
  「意味のないテスト」ではないことも担保されている
- **エラー握りつぶし**: 新規・変更コードに catch 節は1つも無い（diff を
  機械的に確認）。意図的な no-op フォールバックは2箇所
  （`useOptionalSidePanel` による Provider 外での no-op、`Canvas` の
  `onLayerFilterChange` 省略時の noop）で、いずれも理由がコメントに明記
  されており CLAUDE.md の運用ルールに適合
- **固定値の前提条件記録**: ポップオーバー定義文の6行クランプ
  （`styles.css` `.glossary-popover__definition`）は、根拠（260px幅での
  実測・line-clamp 非対応環境でのフォールバック）がコード内コメントと
  本ファイル §3.7-1 の両方に記録済み。ジャンプ時ハイライトは新規定数を
  作らず `NEW_ARRIVAL_HIGHLIGHT_DURATION_MS` を再利用（役割が同じ）
- **テストコードの質**: 純関数（glossarySearch）・コンポーネント配線
  （GlossaryPanelView / SidePanelHost）・実 App 経由の配線
  （App.glossaryPanel.test.tsx）の3層で関心が分割されており、モックは
  ResizeObserver スタブと scrollIntoView の一時スタブ（jsdom 未実装の補完）
  に留まる。異常系（空 glossary・参照切れ termKey/relatedTerms・未知 layer
  値・Provider 外レンダー）・境界値（空白のみクエリ・1文字クエリ・
  フィールド全体一致・グループ内順序保持）を実質的に検証している
- **i18n**: 新規9キーすべて `{ja, en}` 両方あり（`i18n.test.ts` が非空も
  検証）。glossary YAML・パーサの変更は無し
- **ビルド・lint・テスト**: リポジトリ全体で `pnpm lint` / `pnpm build` /
  `pnpm test` すべて成功（frontend 167ファイル・2412テスト）
- **コミット粒度**: 6コミット（UX設計 docs / パネル本体 feat /
  GlossaryTerm クリック連携 feat / 実装 docs / テスト強化 / テスト docs）で
  1変更1コミットを満たす。コミットメッセージも Conventional Commits 準拠
- **CONCEPT.md との整合**: 「用語集パネル」構想（一覧・検索・レイヤーへの
  ジャンプ）を満たす。「キャンバス上の該当要素へジャンプ」はレイヤーレンズ
  連動で例示（peer → B層）を満たし、個別要素へのパンは見送り（§3.5 に
  理由記録済み）で問題無い
- tester 申し送りの a11y 問題（Space の preventDefault 欠落）は Issue #353
  として別途起票済み（本 PR のスコープ外）であることを統括依頼文で確認

#### 指摘（軽微・非ブロッキング）

1. `docs/ARCHITECTURE.md` §12.2 のコード例コメントが
   「#313 実装時に `{ kind: "glossary"; termKey?: string }` を…追加する想定」
   のままで、本ブランチで実装済みの現状と齟齬がある（sync-docs 観点）。
   `SidePanelView` のコード例に glossary kind を実装済みとして反映する
   1行程度の docs 修正をマージ前に行うことを推奨（対応は統括の判断に委ねる）

### 2026-07-17 Issue #313 QA検証
...
frontend をモックモード（`VITE_COLLECTOR_URL` 未設定、`pnpm --filter
@chainviz/frontend dev --port 5199`）で起動し、Playwright（`@playwright/test`
同梱の chromium-1228。この worktree では libnspr4.so 等が不足するため
`LD_LIBRARY_PATH=/home/zoe/chrome-deps/root/usr/lib/x86_64-linux-gnu` で
共有ライブラリを解決。issue-125.md 記載の既知回避策）で実ブラウザを操作し、
実際の DOM・振る舞いを確認した。全項目を自動操作で再現・確認できた
（スクリーンショット不要の範囲で完結）。

#### 確認できたこと（依頼の観点1〜5に対応）

1. 用語集パネルの開閉・検索・一覧（観点1）
   - ヘッダーの「用語集」ボタン（`glossary-open-button`）でパネルが開き、
     検索欄に自動フォーカスが当たる。ボタンの `aria-pressed` が開閉に連動
   - A〜D層の4グループがすべて描画され、用語行は 37 件（worklog 記載の
     用語数と一致）
   - 検索: 「mempool」で 4 件に絞り込み、日本語 UI のまま英語名「Validator」
     で 2 件ヒット（ja/en 両引き）、無一致クエリで空表示、クリアで 37 件へ
     復帰
   - 行クリックでアコーディオン展開し、クランプ無しの定義全文が表示される
     （本 Issue の起点「長い定義文が読み切れない」の解消を実際に確認）

2. 用語アンカーからの起動（観点2）
   - インラインの `glossary-term-*` アンカー（読み込み時に 35 個描画）を
     クリックすると、その用語（例: workbench）を展開状態でパネルが開く
   - キーボード（アンカーにフォーカス→Enter）でも同じく該当用語
     （例: container）で開く
   - Esc でパネルが閉じる

3. Issue #321（コントラクトソース表示）との排他制御（観点3）
   - コントラクトカードの「ソースコードを見る」で contractSource パネルを
     開いた状態から用語アンカーをクリックすると、glossary パネルへ置き換わる
     （両方同時に出ない）
   - 逆方向（glossary 表示中に contractSource を開く）でも glossary が閉じて
     contractSource へ置き換わる
   - glossary パネルを開いたまま 4 秒（モック差分は 3 秒間隔なので 1 ティック
     以上の world state 更新を跨ぐ）待っても、パネルは閉じずに開いたまま
     （ダングリングガード修正が実挙動として効いていることを確認）

4. パネル内ジャンプ・レイヤーレンズ連動（観点4・CONCEPT.md 整合）
   - 展開行の関連用語チップ（`glossary-panel-related-*`）をクリックすると、
     対象用語がパネル内で展開・スクロールされる（例: workbench →
     container）
   - 展開行のレイヤーチップ（`glossary-panel-layer-chip`）をクリックすると
     チップが active（`aria-pressed=true`）になり、キャンバスのレイヤー
     レンズ（Issue #299）が該当層に切り替わる。CONCEPT.md「用語集パネル」の
     「用語から関連するレイヤーへジャンプ（例: peer → B層をハイライト）」を
     レンズの dim で満たしている
   - ポップオーバー側（§3.7 の共存仕様）: 用語ホバー/フォーカスで出る
     ポップオーバーの定義文に CSS `line-clamp: 6` が効き、末尾に
     「クリックで用語集を開く」フッターが出て、関連用語が生キーではなく
     用語名で表示されることを確認

5. PLAN.md 完了条件（観点5）
   - #313 は PLAN.md バックログの単一チェックボックス項目。CONCEPT.md
     「用語集パネル」構想（全用語一覧・検索・レイヤーへのジャンプ）を満たす
     実装であることを実挙動で確認した。チェックボックスは実装担当が既に
     チェック済み

#### 補足・申し送り

- ステップ10以降の運用ルール「UIに見える機能には SCENARIOS.md への UI
  シナリオ追記 + Playwright テストを含める」に対し、本 Issue（バックログの
  単一項目）では PLAN.md にその明示チェックボックスが無い。QA 手動検証は
  合格だが、用語集パネルの UI シナリオを `packages/e2e/SCENARIOS.md` +
  Playwright へ恒久追加するかは統括の判断事項として申し送る（QA 合格の
  ブロッカーではない）。
