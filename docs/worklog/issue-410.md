...
### 2026-07-23 Issue #410 QA再検証結果（合格）

- 担当: qa
- 検証ブランチ: issue-410-operation-panel-tooltip-suppression（origin の最新
  コミット fbff4c2 を detached HEAD で取得して検証）。前回未達だった受け入れ
  条件4（ヘッダー「ワークベンチ」ラベルの用語解説ポップオーバーが操作パネルを
  覆う）への差し戻し対応を実機で再確認した。
- 検証環境: 依存を pnpm install 済み。profiles/ethereum の Docker スタックが
  起動しておりチェーンが進行していることを確認した（eth_blockNumber がホスト
  RPC 8545 経由で 0xa9=169 → 0xab=171 と増加。ワークベンチコンテナから reth1
  への cast block-number も 169 を返し RPC 疎通を確認）。フロントエンドは
  pnpm --filter @chainviz/frontend exec vite（VITE_COLLECTOR_URL 未設定＝
  モックデータ、実際には 5299 が使用中のため 5300 で起動）で配信し、Playwright
  (chromium、LD_LIBRARY_PATH=/home/zoe/chrome-deps/root/usr/lib/x86_64-linux-gnu
  で共有ライブラリ補完)で実操作した。

#### 受け入れ条件1〜4の通し再確認（すべて合格）

ワークベンチカード（workbench-alice）の操作を実行ボタンをクリックして操作
パネル（送金タブ）を開いた状態で、以下を実操作で確認した。

- 条件1: 操作パネルが開いた瞬間、ActionHint 予告ツールチップが消える
  （.action-hint__popover のDOM要素数 0）。デグレなし。
- 条件2: 操作パネルが開いている間、カード本体（.infra-card__name）を
  ホバーしても InfraPopover が表示されない（[data-testid^=infra-popover-]
  のDOM要素数 0）。デグレなし。
- 条件3: 上記に連動して InfraPopover 内の用語解説ポップオーバー
  （glossary-popover-rpc-endpoint）も表示されない（DOM要素数 0）。デグレなし。
- 条件4（今回の修正対象）: 操作パネルを開いた状態でヘッダーの「ワークベンチ」
  ラベル（glossary-term-workbench）にカーソルを乗せても、用語解説ポップ
  オーバー（glossary-popover-workbench）が表示されない（DOM要素数 0）。
  この状態で金額入力欄（operation-transfer-amount）への click が成功し、
  値「1.5」を実際に入力できた（前回はここでポップオーバーに覆われ click が
  4秒でタイムアウトして失敗していた）。スクリーンショットでも操作パネル
  （送金フォーム、金額欄に 1.5 入力済み）がカード右側に完全表示され、
  ラベルの用語解説ポップオーバーに覆われていないことを確認した。

#### 抑制が条件付きであること（ラベル自体を壊していない）の確認

修正が「操作パネルを開いている間だけ」の条件付き抑制であり、ヘッダーラベルの
用語解説機能そのものを無効化していないことを別途確認した。

- 操作パネルを閉じた状態でヘッダー「ワークベンチ」ラベルをホバーすると、
  用語解説ポップオーバーが通常どおり表示される（glossary-popover-workbench
  のDOM要素数 1）。
- 続けて操作パネルを開いてから同ラベルをホバーすると表示されない（0）。
- すなわち suppressed 条件（entity.kind === "workbench" && operationPanelOpen）
  が意図どおり機能し、パネル開閉に追従して表示/抑制が切り替わる。

#### 判定

- 受け入れ条件1・2・3: 合格（今回の修正でのデグレなし）。
- 受け入れ条件4: 合格（前回未達だったヘッダーラベル経由の重なりが解消。
  パネル表示中はラベルのポップオーバーが抑制され、金額入力欄を実際に
  クリック・入力できる）。
- 総合判定: 合格。Issue #410 の受け入れ条件をすべて満たしている。
- コミット・push・PR作成・マージ・Issueクローズは行っていない（統括が内容を
  確認のうえ実施する）。
