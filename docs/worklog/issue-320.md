...
### 2026-07-16 QA検証（検証大地）

結果: **合格**（Issue本文「期待する対応」を満たす）。

検証環境と制約:

- 他worktree（issue-325）が同一Dockerデーモンと dev ポート（4000/4001/
  5173）を使用中だったため、`scripts/dev-up.sh` は使わず、他セッションに
  影響しない独立ポート（5173ではなく5199）で frontend の vite dev サーバー
  をモックモード（`VITE_COLLECTOR_URL` 未設定）で起動して検証した。
  `docker compose` は実行していない（共有スタックには触れていない）。
- モックモードでは `MOCK_ALICE_RECENT_TX_LIMIT = 20` により Alice の tx 履歴が
  3秒間隔で最大20件まで積み上がる。これを使い、実ブラウザ（Chromium /
  puppeteer-core によるヘッドレス駆動）でホバー・スクロール・ホイールを
  実操作して確認した。collector 側の `MAX_WALLET_RECENT_TX_HASHES = 32`
  への引き上げそのものは、実チェーンを起動できなかったため live での
  32件到達までは未確認（ユニットテスト＋レビューで確認済み）。フロント側の
  「保持分を全件描画する」挙動は下記のとおり実ブラウザで確認できており、
  collector が32件保持すればそのまま32件表示される配線であることは
  コード上も確認した。

実ブラウザで確認した項目（Alice ウォレットカードをホバーして WalletPopover
を開いた状態、tx が8件に達した時点で測定）:

- tx 一覧が6件で切れず全件（8件）描画される。`.wallet-popover__tx-list` は
  `max-height: 220px` / `overflow-y: auto` / `scrollbar-width: thin` が実際に
  効いており、`scrollHeight 321 > clientHeight 220` でスクロール可能。
- 実際のマウスホイール操作でリスト内容がスクロールした（`scrollTop 0 → 60`）。
- スクロール中もポップオーバーは開いたまま（`.wallet-popover` が残存）。
  WalletPopover は WalletCard の React 子（portal で body 直下に描画）であり、
  React のファイバーツリー上はカードの子であるため、ポップオーバー上へ
  マウスを移してもカードの `onMouseLeave` は発火せず閉じない挙動を実測確認。
- ポップオーバー上のホイールがキャンバスのズームを誤発火しないことを、
  イベント到達性の対比で確認: `.react-flow` ルート要素にキャプチャ段の
  wheel リスナを仕込み、ポップオーバー上でホイールしたときの到達回数=0、
  同じホイールをキャンバスの空きペイン上で行ったときの到達回数=4（両者とも
  document へは到達しており、ホイールイベント自体は発火している）。
  ポップオーバーは portal で `.react-flow` の外（body 直下）に描画されるため、
  ホイールが React Flow のズームハンドラに一切到達しない。
- 見出しに件数表示「直近の tx（8件）」が出る（`wallet.recentTxCount` /
  `format()`。日本語で確認。英語 "Recent tx (N)" は i18n ユニットテストで確認済み）。
- カード面の tx チップは従来どおり6件のまま（`.wallet-card__tx-chips` の
  チップ数=6）。ポップオーバー（全件）とカード（6件）で表示件数が異なる
  ことを同一カードで同時に確認した。

差し戻しなし。統括の合流・マージ作業に進んでよい。
