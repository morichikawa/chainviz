...
### 2026-07-09 QA検証記録(qa)

- 担当: qa
- ブランチ: issue-200-ui-cmd-scenarios
- 環境: 稼働中の chainviz-ethereum スタックを再利用(検証開始時ブロック高約600、
  稼働19分の若いスタックのため Issue #229 の長時間稼働 stall には該当しない)。
  Playwright chromium の共有ライブラリは scratchpad 展開済みの nss/nspr を
  `LD_LIBRARY_PATH` で補完して実行した。

#### 実施内容と結果

1. **UI層E2Eフルラン(`pnpm test:e2e:ui`)**: 16件すべて green(所要約1分)。
   内訳は今回の UI-CMD-01〜07(7件)と既存の UI-CONN-01 / UI-A-01〜05 /
   UI-B-01〜03(9件)。UI-CMD-01(ゴースト2枚→実体2枚)、UI-CMD-06(同名2回で
   `-2` 付き2枚共存)、UI-CMD-07(削除後ウォレットがオーファン表示で残存)を
   含め全件パス。

2. **プロトコル層E2E(`pnpm test:e2e`)**: 6ファイル18件すべて green
   (所要約173秒)。削除対象だった WS テストの整理後の姿として
   `commands.test.ts` は2件(PROTO-CMD-01 ブロック追従 / PROTO-CMD-02 compose
   ノード削除拒否)、`error-paths.test.ts` は4件になっており worklog 記載の
   削減(6→2、6→4)と一致。PROTO-CMD-01(自己完結化したブロック追従)は
   65.6秒で完走し、若いスタックでは stall しないことを実測で確認
   (Issue #229 の再現無し)。残りのテスト(reconnect / a-b-layer / d-layer /
   collector-port-collision)も引き続き green で、削除による穴は無い。

3. **実ブラウザ相当での目視確認(スクリーンショット)**: 一時的な
   スクショ取得 spec を用いて実際の画面遷移を撮影し目視確認した(検証後に
   spec は削除、追加したノードコンテナも後始末してスタックを元の compose
   7コンテナに戻した)。
   - ノード追加: 押下直後にゴーストカード(半透明)2枚が出現し、数秒後に
     実カード(reth + beacon3)へ切り替わり、追加ノードがブロック同期
     (95/780 → 191/783 → 351/792 と進行)する様子を確認。
   - ワークベンチ同名2回追加: 別々のウォレットカードを伴う2枚のワークベンチが
     共存することを確認。
   - 削除: ワークベンチカードは消え、付随するウォレットカードは
     オーファンバッジ付きで残存する(CONCEPT.md の設計どおり)ことを確認。

#### 判定

**合格**。Issue #200 本文および依頼された検証項目1〜3をすべて満たしている。
差し戻し事項なし。push / PR作成 / マージ / Issue クローズは統括に委ねる。
