...
### 2026-07-17 UI-LOG-02再検証合格・UI-LOG-04フレーキー対応（frontend）

- 担当: frontend
- ブランチ: issue-317-comms-log-panel
- 経緯: 上記 UI-LOG-02 の修正を QA が実 Docker スタックで再検証し、
  3回中3回とも安定して合格した。ただし再検証中に別のテスト
  `UI-LOG-04` が3回中1回フレーキーに失敗することが判明した。QA の
  切り分けにより、フィルタ機能自体（製品コード）は正しく動作している
  （QA の自前環境で13回試行しすべて正常）ことを確認済みで、原因は
  テスト側にあると特定された。

#### 問題

`UI-LOG-04` は「操作」以外の5カテゴリのチップを for ループで高速連続
クリックした直後に、非操作エントリが0件になっていることを検証していた。
各クリックが実際に反映される（React の状態更新・再描画）前に次のクリック
へ進んでしまうことがあり、まれにチップの反映を取りこぼしたまま次の
検証（エントリ件数チェック）に進んで失敗していた。

#### 修正内容

`packages/e2e/src/ui/comms-log.spec.ts` のチップ off ループで、各クリック
の直後に `aria-pressed="false"` になったことを明示的に待ってから次の
チップへ進むよう変更した（`CommsLogFilterBar.tsx` は `aria-pressed={active}`
をチップに持たせているため、この属性で反映済みかどうかを判定できる）。
取りこぼしが起きた場合はこの `expect().toHaveAttribute()` の時点で
明示的に失敗するようになり、後続の「エントリ件数が0件」という検証が
別の理由（取りこぼし）で紛れて失敗する状態を防ぐ。再度 on に戻す方の
ステップ（139行目付近）は単発クリック + 通常の `toBeVisible()` ポーリング
であり、高速連続クリックの問題は無いため変更していない。
...
### 2026-07-17 UI-LOG-04 追加フレーキー対応（frontend）

- 担当: frontend
- ブランチ: issue-317-comms-log-panel
- 経緯: 前節（UI-LOG-03）と同じ QA 再検証ラウンドで判明した、UI-LOG-04
  の別原因のフレーキー対応。

#### 問題

UI-LOG-04 はブロック蓄積待ちのみを前提にしており、操作カテゴリの
エントリを明示的に発生させていなかった。以前合格していたのは、
UI-LOG-02 が作ったワークベンチのポーリング（レシート待ちの
`eth_getBlockByNumber` 等）が偶然この待機窓に入っていたためで、
UI-LOG-04 単体としては操作エントリの存在を保証していなかった。QA の
切り分けにより、フィルタ機能自体（非操作カテゴリが0件になる検証）は
毎回合格しており製品側は正しいことを確認済み。

#### 修正内容

UI-LOG-02 と同様に `addWorkbenchAndGetWallet` + `submitTransfer` で送金
操作を実行し、操作カテゴリのエントリ数が増えたことを `expect.poll` で
確認してから、カテゴリチップの絞り込みに進むよう変更した。ワークベンチ
のラベルは UI-LOG-02 と衝突しないよう `comms-log-filter-sender` とした
（各テストは独立した Playwright コンテキストで実行されるため実害は
無いが、ログ・トレースで区別しやすくする意図）。
...
### 2026-07-17 UI-LOG-04 根本修正（ワークベンチ操作依存の排除）（frontend）

- 担当: frontend
- ブランチ: issue-317-comms-log-panel
- 経緯: UI-LOG-01/02/03 は安定して合格したが、UI-LOG-04 は対症療法
  （送金の実行タイミングをずらす対応を2回）を重ねても5回目の QA 再検証で
  失敗した。統括からの指示により、今回は対症療法ではなく問題の構造自体を
  無くす修正を行う。

#### 問題の本質

UI-LOG-04 は UI-LOG-02 の後に実行される。UI-LOG-02 が残した managed
ワークベンチ（`comms-log-sender`）に UI-LOG-04 自身が追加する
`comms-log-filter-sender` が加わり、managed ワークベンチが2台になる。
カードのグリッド配置はエンティティ数に対して決定的なため、この2台分
だけカードのグリッドが伸び、静的ワークベンチのカードがキャンバス上の
オーバーレイ（通信ログパネル・p2p-legend・ツールバー等。テスト実行順・
ビューポートの微妙な条件次第でどのオーバーレイに当たるかが変わる）の
裏に押し込まれ、操作ボタンがクリックできなくなる。これは「送金操作の
ためにキャンバス上のカードをクリックする」という UI-LOG-04 のテスト
構造そのもののレイアウト脆弱性であり、送金の実行タイミング・パネルの
開閉順序をどう変えても、カードの位置がオーバーレイの裏に回り込む条件
自体は消えないため収束しなかった。

#### 修正内容（案A: ワークベンチ操作を要しないカテゴリでの検証に変更）

UI-LOG-04 が検証すべきなのは「フィルタが表示を絞り込む」「フィルタは
表示だけに作用し蓄積は継続する」という挙動であり、絞り込む対象カテゴリ
自体は何でもよい。操作カテゴリの記録・検証は UI-LOG-02 が既にカバーして
いるため、UI-LOG-04 では対象を「操作（RPC）」から、チェーン進行だけで
キャンバス操作なしに決定的に蓄積される「ブロック」に変更した:

1. 「ブロック」以外の全カテゴリチップ（`operation`/`internal`/`tx`/
   `peer`/`environment`）を off にする
2. 表示されるエントリが全てブロックカテゴリになることを確認する
3. off にした「内部API」カテゴリを再度 on に戻すと再び表示されることを
   確認する（蓄積自体は保たれている）

これによりワークベンチカードのクリック自体が不要になり、カード位置・
オーバーレイ依存が完全に消える。`addWorkbenchAndGetWallet` /
`submitTransfer` / `STATIC_WORKBENCH_ID` / `OPERATION_EFFECT_TIMEOUT_MS`
への依存は UI-LOG-04 から無くなった（UI-LOG-02 は引き続き使用するため
import からは削除していない）。

`packages/e2e/SCENARIOS.md` の UI-LOG-04 の文面（「操作カテゴリになる」
という記述）もこの変更に合わせて更新した（対象カテゴリを block に、
確認するカテゴリを internal に変更）。

なお、通信ログパネルを開いている間は裏に位置するカードの操作ボタンが
押せなくなるという UX 上の懸念自体は、実利用でも起こりうる脆弱性として
QA から改めて共有されている。既知の「操作パネルはみ出し問題」
（`packages/e2e/src/ui/support/operations.ts` の `OPERATION_PANEL_VIEWPORT`
コメント参照）と同種の課題で、本 Issue のスコープ外のため今回は対応
していない（統括側でフォローアップ Issue 化を検討）。
...
### 2026-07-17 Issue #317 QA検証（qa）

- 担当: qa
- ブランチ: issue-317-comms-log-panel
- 判定: 合格

#### 実施した検証（実Dockerスタックに対して）

Ethereumプロファイルのdocker composeスタックを起動し（reth1でブロック進行を
実測確認）、UI層E2E（`pnpm test:e2e:ui comms-log.spec.ts`）と、自前collector
+viteをつないだ実チェーンでのライブ操作の両方で確認した。

- UI-LOG-01〜04をクリーンな環境で複数回実行し、全て安定して合格することを
  確認した（4/4合格を3回）。途中1回だけ2テストが失敗したが、これは別エージェント
  （chainviz-detective）が同じ共有Dockerスタックに`det-test`コンテナを追加し
  チェーンを再起動していた並行干渉が原因で（UI-LOG-02はカードグリッドのずれで
  静的ワークベンチの操作ボタンがパネル裏に回り込み、UI-LOG-04は`canvas-toolbar-
  comms-log`ボタン自体が現れずタイムアウト）、干渉のないクリーンな環境では
  再現しないことを確認済み。
- ライブ確認（自前collector+vite、実チェーン）で以下を目視・DOM検証した:
  - パネルの開閉とトグルのaria-pressed。
  - 6カテゴリ全て（operation/internal/block/tx/peer/environment）が実際に時系列で
    蓄積される（operation/txはE2E・トレース、他4つはライブで確認）。
  - ブロック伝播の文言が「ブロック #N を受信」「最初に受信」で送信経路を断定
    しない。ピアは「A ⇄ B / ピア接続が確立」で方向を断定しない。
  - カテゴリフィルタで絞り込み中も裏側でログ蓄積が継続する（off中に増えた分が
    再表示で反映される）。フィルタ純関数は13回の制御試行でも取りこぼしゼロ。
  - glossary⇄commsLogの直接遷移で排他制御が機能する（commsLog内の用語アンカー
    からglossaryパネルへ置換、side-panelは常に1枚、commsLog再オープンで蓄積が
    保持される）。レビュー担当の申し送り#1の未テスト経路を実機で確認した。

#### QA差し戻しで駆動した修正

初回検証で実DockerのUI層E2Eが複数箇所で失敗したため、実装担当（frontend）へ
差し戻して以下を修正した。いずれも製品コードではなくE2Eテストの構成上の問題で、
通信ログ機能そのものは正しいことを別途実証済み。

- UI-LOG-02（07155f1）: 「操作カテゴリの先頭エントリがeth_sendRawTransaction」
  という順序前提の誤り。送金後のレシート待ちポーリング（eth_getBlockByNumber等）
  が新しいタイムスタンプで先頭に来るため決定的に失敗していた。存在確認へ緩和。
  eth_sendRawTransaction自体はトレース上8件記録されており、記録漏れではない
  ことを確認済み。
- UI-LOG-03（eb834a2）: 「一覧の絶対先頭がblock」という断定を、ブロックカテゴリ内
  の相対順序比較へ緩和。ブロック生成直後の内部API観測がブロックより新しい
  タイムスタンプで割り込むため（レビュー担当の申し送り#3の失敗モードが顕在化）。
- UI-LOG-04（d664cd7・e321182・e1e40f9・d91f7cb）: フィルタチップの高速連続
  クリックでの取りこぼし、操作エントリの存在保証が無かった点、送金操作のために
  キャンバス上のカードをクリックする構造がオーバーレイ（通信ログパネル・
  p2p-legend・ツールバー）とのカード位置の重なりで繰り返し失敗した点。最終的に、
  フィルタ検証の対象をワークベンチ操作不要なブロック/内部APIカテゴリへ変更する
  根本修正（d91f7cb、SCENARIOS.mdも更新）で収束した。操作カテゴリの記録・検証は
  UI-LOG-02が引き続きカバーしている。

#### 注意点（次の担当・統括へ）

- UI層E2E（test:e2e:ui）は実Dockerスタックを共有するため、他のエージェント
  （例: chainviz-detective）が同じスタックにコンテナを追加・チェーンを再起動
  すると、カードのグリッド配置がずれてワークベンチ操作系のテストが失敗したり、
  collector/frontendの初期化が乱れたりする。共有スタックを使う検証は他エージェント
  の並行作業と競合しないよう注意すること。
- 「通信ログパネルやp2p-legend等のオーバーレイの裏に来たキャンバス上のカードは
  操作ボタンをクリックできない」というフロントのUX脆弱性が、今回のE2E反復で
  繰り返し表面化した。#317のブロッカーではないが、フォローアップIssueの候補
  として統括へ共有済み。
- Playwrightのchromiumは実行環境にlibnspr4.so等が無いため、事前展開済みの
  ライブラリ（~/chrome-deps）へLD_LIBRARY_PATHを通して実行した。
