...
- 担当: qa（実機検証）
- ブランチ: `issue-229-investigate-backfill-stall`
- 判定: **合格**（元Issueの「長時間稼働スタックでPROTO-CMD-01が誤って失敗する」
  は実機で解消を確認した）

検証環境:

- 稼働中の `chainviz-ethereum` スタック（compose 起動、稼働約4時間）を再利用。
  検証開始時点のブロック高は約 7261（reth1）。detective の分析では高さ約 5000
  （稼働約 2.8 時間）を超えると旧条件（head への完全追従）が構造的に必ず失敗
  するため、この状態は元Issueの再現条件を満たす。
- 別プロセスの dev collector がポート 4000（WS）/ 4001（ロギングプロキシ）を
  専有中だったため、Issue #254 の回避策として環境変数 `CHAINVIZ_PROXY_PORT=4124`
  を付与して e2e ハーネスの子 collector を起動した。

実施内容と結果:

1. 修正版 PROTO-CMD-01 の実機実行（完了条件1・2）:
   `CHAINVIZ_PROXY_PORT=4124 pnpm -C packages/e2e exec vitest run src/commands.test.ts`
   を高さ約 7261 の長時間稼働スタックに対して実行。**2 テスト全て pass**。
   ブロック追従テスト（addNode → 最低進行量到達 → removeNode）は 37.8 秒で完了
   （テストファイル全体 50.6 秒）。
   - この間、addNode で reth4 + beacon4 が追加され、追加 reth が最低進行量
     （`MIN_PROGRESS_BLOCKS=300`）到達まで進行し、removeNode で後片付けされる
     ことを確認。テスト後に reth4 / beacon4 が消え、既存スタック（reth1/reth2
     等）が健全に稼働継続していることも docker ps で確認した。
   - 旧条件（head=7261 への完全追従）であれば、追加ノードの開始高さ約 95 から
     gap 約 7166 を実測バックフィル速度 約 10 ブロック/秒で追いつくのに約 716 秒
     を要し、固定上限 `maxTimeoutMs=540_000`（9 分）を超えてタイムアウト失敗
     していたはずの状況。実際に 37.8 秒で pass したことで、head 完全追従ではなく
     最低進行量を条件にした修正が誤タイムアウトを解消していることを実機で確認
     した。detective が高さ 9285 で観測した実失敗（経過 541011ms でタイムアウト）
     と整合する再現条件下での検証である。

2. stall 検出（回帰検出力）の確認（完了条件3）:
   - stall 判定ロジック（`CatchUpMonitor`）は本修正で一切変更されていない
     （`waitForBlockCatchUp` の停止検出パスはそのまま利用）ことをコードで確認。
   - `catch-up-min-progress.unit.test.ts`（7件）/
     `catch-up-min-progress.edge.unit.test.ts`（13件）を実行し **20件全 pass**。
     高さが停止したまま（高さ 0 固定・非ゼロ固定・進行後凍結・head フォール
     バック時の 0 固定）の各パターンで、目標に関わらず有限時間（stall 判定
     45 秒相当）で失敗することが合成クロックで検証されていることを確認した。
   - 実機で EL 間 P2P を意図的に破壊しての stall 再現は行っていない。これは
     共有中の長時間稼働スタックを down -v で作り直す破壊的操作が必要で、
     dev collector・統括が利用中の共有リソースを一方的に壊すことになるため
     見送った。停止検出ロジックが本変更で不変であること・上記ユニットテストの
     合格・既存コメントの「reth-node.sh の該当フラグを外して実際に失敗する
     ことを確認済み」の記録により、回帰検出力は維持されていると判断した。

3. Issue #254 の影響確認（完了条件4）:
   回避策なし（`CHAINVIZ_PROXY_PORT` 未指定）で同じテストを実行したところ、
   dev collector が 4001 を専有しているため子 collector が起動に失敗し、
   `Error: connect ECONNREFUSED 127.0.0.1:4123` で 2 テストとも skip（Failed
   Suite）になることを実機で再現・確認した。これは #229 とは無関係で Issue
   #254（未解決、OPEN）そのもの。`CHAINVIZ_PROXY_PORT=4124` の付与で回避でき、
   その状態で #229 の検証（上記1）は正常に pass した。#254 は #229 の検証を
   直接妨げるものではないが、dev collector 稼働中に素の `pnpm test:e2e` を
   叩くと失敗するため、恒久対応（ハーネスが子 collector へ重複しないプロキシ
   ポートを明示的に渡す）が引き続き必要。

結論: 元Issue（長時間稼働スタックで PROTO-CMD-01 が誤って失敗する）は実機で
解消を確認した。差し戻しなし。
