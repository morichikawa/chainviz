...
### 2026-07-06 Issue #123 QA検証: 合格

- 担当: qa
- ブランチ: issue-123-ux-design-node-addition
- 方法: このworktreeからdocker composeへの破壊的操作は一切行わず、
  frontendをモックデータで起動して(`pnpm --filter @chainviz/frontend dev`、
  ポート5176、VITE_COLLECTOR_URL未設定=モック)Playwrightで実際に操作・
  スクリーンショット確認した。
- モックのコマンドレイテンシは既定0msで、仮カード(ゴースト)が同一Reactコミット
  内で実カードに置換され描画されないため、レイテンシ2500msのモックでAppを
  マウントする使い捨ての検証用エントリを一時的に作成して仮カードの描画を
  観測した(確認後に削除済み。コミット対象外)。実docker環境ではコンテナ起動に
  数秒かかるため、この遅延分だけ仮カードが実際に表示される。
- PLAN.md完了条件「ノード/ワークベンチを追加する前後で『どこに追加され、
  何と繋がるか』がユーザーに伝わるようになっていること」を、以下のとおり
  確認した。

検証項目と結果:
1. ボタンホバー時の予告ツールチップ(§4-1): 「+ ノードを追加」ホバーで
   「フォロワーノード(reth + beacon のペア、カード2枚)を起動します。
   chainviz-reth-1 と chainviz-lighthouse-1 を入口(ブートノード)に既存
   ネットワークへ参加し、同期後は他のノードとも自動で繋がります」が
   表示される。英語表示でも `{elBoot}`/`{clBoot}` がcontainerNameに
   置換されて表示されることを確認(EN: "...through chainviz-reth-1 and
   chainviz-lighthouse-1 as bootnodes...")。「+ ワークベンチを追加」も
   同様に「RPC 呼び出しは chainviz-reth-1 に送られ...」を確認。
2. 仮カード2枚化+接続予定エッジ(§4-2): 「+ ノードを追加」クリック直後、
   「新しいノード (reth) / 起動中… chainviz-reth-1 と接続予定」
   「新しいノード (beacon) / 起動中… chainviz-lighthouse-1 と接続予定」の
   ゴースト2枚と、それぞれからブートノードへの接続予定エッジ(点線)2本が
   描画されることを確認。
3. 実カード到着後の新着強調・既存カード不動(§4-3, §4-4): 実カード
   (chainviz-reth-follower-1 / chainviz-beacon-follower-1)が仮カードと
   同じグリッド位置に到着し、既存カード(lighthouse-1 / reth-1 / reth-2 /
   workbench-alice)が一切移動しないことを確認。新着の2枚だけが発光
   (infra-card--new)し、5秒経過後に発光が消えることを確認。到着直後は
   「P2P接続を確立中…」の接続確立中エッジが表示され、ADD_NODE_PEER_
   CONNECT_DELAY_MS(4000ms)経過後に実PeerEdge(緑実線)へ切り替わって
   確立中エッジが消えることを確認。
4. ワークベンチ追加(§4-4): 追加時にゴーストへ「起動中… 操作先:
   chainviz-reth-1」の予告が出ること、常設の操作先エッジがワークベンチ→
   操作先ノード(reth-1)へ描画されること(Alice分1本→Zoe追加後2本)、
   カードのホバーポップオーバーに「操作先ノード chainviz-reth-1」欄が
   出ることを確認。追加ワークベンチも空きスロットに配置され既存カードは
   不動。
5. フォールバック/通常表示(§4-5): 実際のユーザー向け設定(ルートURL・
   0レイテンシモック)で初期スナップショットのカード6枚が正常描画され、
   ブートノードバッジ2件が表示され、起動時・操作時ともにpageerror/
   console.errorが1件も出ないことを確認。レビュー2回目で指摘された
   b-network.yamlのbootnode二重キーによる起動時クラッシュが解消されて
   いること(アプリが正常に起動すること)を、実際の画面表示で確認した。
- 判定: 完了条件を満たしている。合格。
- 実施しないこと: git push / PR作成 / マージ / Issueクローズは統括に委ねる
  (QA担当は実行しない)。PLAN.mdの該当チェックボックスにはチェックを
  付けた。
