...
### 2026-07-16 Issue #322 再レビュー(reviewer)

- 担当: reviewer
- ブランチ: issue-322-slot-time-and-indicator-e2e
- 判定: **最終合格**(実装コードは前回レビューで合格済み。今回は統括による
  docs 再構成のみを再確認した)

#### 確認した項目

- 前回差し戻しの 3 点はすべて解消:
  1. `docs/ARCHITECTURE.md`: `e34c420` 相当のコミット(`b2c3583`)を取り込み済み。
     旧「slot ごと(約 2 秒)」「slot 2 秒 × スクレイプ 3 秒」等の 4 箇所が
     12 秒前提へ更新されている。残る「2 秒」への言及(1313 行・1963 行)は
     「2 秒だった当時は〜」という改訂経緯の説明であり、実装との矛盾は無い。
     §8.6 には「実測は 2 秒当時のもの。#322 の QA で再実測して更新すること」
     という申し送りも明記されている
  2. `docs/CONCEPT.md`: `7acad0b` 相当のコミット(`a0a54f7`)を取り込み済み。
     旧決定(slot time 短縮)の直後に改訂注記(2026-07-16、Issue #322。
     ユーザーフィードバックを受け 12 秒へ戻す・#343 のインジケータで補う・
     tx 契機のブロック生成は不採用の理由付き)があり、決定事項リスト側にも
     撤回の相互参照が付いていて、経緯が矛盾なく読み取れる
  3. `docs/worklog/issue-322.md`: 見出し → designer 設計メモ全文(§1〜§11)→
     tester 記録 → node-env 記録 → レビュー記録 → 統括の再構成記録、の
     正しい時系列順に再構成されている。元ブランチとの diff で、差分が
     「tester 記録・レビュー記録・再構成記録の追記のみ」であることを確認
     (設計メモ本文の欠落・断片の重複は無い)。`docs/worklog/issue-343.md` は
     元ブランチと完全一致、`docs/WORKLOG.md` の #322(designer)・#343 の
     索引行も取り込み済み
- `git merge-base --is-ancestor main HEAD` で main 取り込み済みを確認
- `pnpm lint` / `pnpm build` / `pnpm test` 全体通過。テスト件数は
  collector 1458 件・e2e 171 件・frontend 2220 件・shared 64 件で前回と一致
- 軽微指摘だった「マージコミット `bdc6b43` への PLAN.md 更新の同居」は
