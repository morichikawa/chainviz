...
#### docs追従（2026-07-11・collector担当）

レビューで指摘された `docs/ARCHITECTURE.md` の追従漏れに対応した。

- 8.3節のポート割り当て一覧に、本 Issue で新規追加したロギングプロキシ
  ポート衝突テスト用ポート（4210/4211）を追記した。
- 「e2e の各 collector はロギングプロキシに WebSocket ポート +1 を既定で
  使う」という規約を新規の箇条書きとして追記し、現行の使用ポート対
  （4123/4124、4125/4126、4199/4200、4210/4211）を列挙した。
- 「新しく固定 WebSocket ポートを追加する際は、その値の +1 が既存の
  いずれの WebSocket ポートとも重複しないか確認すること」という注意点を
  同じ箇条書きに含め、worklog だけでなく恒久ドキュメント側にも残した。
- コード変更は無し。`docs/` のみの変更のため `pnpm build` / `pnpm test`
  の再実行は不要と判断した（実装・テストは変更していない）。

#### 再レビュー（2026-07-11・レビュー担当）

- 判定: **合格**。前回の差し戻し事項（1点）はコミット `04db04e` で
  過不足なく解消された。

確認した内容:

1. **指摘への対応内容**: `docs/ARCHITECTURE.md` 8.3節のポート割り当て
   一覧に、(a) 新規テストのポート（ロギングプロキシポート衝突テスト
   4210）の追記、(b) 「e2e の各 collector はロギングプロキシに
   WebSocket ポート +1 を既定で使う」規約の箇条書き（現行の使用ポート対
   4123/4124・4125/4126・4199/4200・4210/4211 の列挙を含む）、
   (c) 「新しく固定 WebSocket ポートを追加する際は +1 が既存の
   WebSocket ポートと重複しないか確認する」という注意点、のすべてが
   追記されている。前回指摘した内容と過不足なく一致する。
2. **記述と実装の一致**: 追記されたポート番号を e2e ソースの実際の値と
   照合した（`collector.ts` の既定 4123 / `+1`、
   `playwright-global-setup.ts` の 4125、
   `collector-port-collision.test.ts` の 4199、
   `collector-proxy-port-collision.test.ts` の 4210/4211）。すべて一致。
3. **変更範囲**: コミット `04db04e` は `docs/ARCHITECTURE.md` と
   本 worklog への追記のみで、コード変更なし。docs のみの変更のため
   ビルド・テストの再実行が不要という実装担当の判断は妥当
   （前回レビューでリポジトリ全体の build / lint / test green を
   確認済みで、その後コードに変更はない）。
4. **コミット粒度・メッセージ**: 1コミットに docs 追従とその作業記録が
   まとまっており、単一の関心事として妥当。Conventional Commits 準拠。

その他の観点（実装・テスト・境界の遵守など）は前回レビューで合格済みで
あり、以降コードに変更が無いことを `git log` で確認したため再確認は
省略した。

#### QA検証記録（2026-07-11・QA担当）

- 判定: **合格**（Issue #254 の完了条件「dev collector 稼働中は pnpm
  test:e2e が起動不能」は実機で解消を確認）。ただし別要因による失敗が
  1件あり（後述、#254 とは無関係）。

実施環境:

- ブランチ `issue-254-e2e-port-collision-guard` の worktree で検証。
- `packages/shared`・`packages/collector` を再ビルドしてから実施
  （e2e は worktree のビルド済み `packages/collector/dist/index.js` を
  子プロセスとして起動するため）。
- Docker の Ethereum スタックは稼働中。検証時点でチェーン高は約 8193、
  検証終盤で約 8539 まで進行（4 時間以上稼働）。
- **dev collector が別プロセス（`/home/zoe/workspace/chainviz` の
  `packages/collector/dist/index.js`、pid 1753413）として稼働し、既定の
  ロギングプロキシポート 4001 を専有している状態**を実測で確認
  （`ss -ltnp` で 4000/4001 を listen）。これは元 Issue の再現条件その
  ものにあたる。

シナリオ1（dev collector 稼働中＝元 Issue の再現条件で `pnpm test:e2e`）:

- 実行前に `ss` で 4001 が dev collector に専有されていることを確認した
  うえで `pnpm test:e2e`（全 e2e スイート）を実行した。
- なお最初の起動試行は、同一ホストで別の `pnpm test:e2e`（PID 2629334）が
  実行中でホスト排他ロック（Issue #64 の仕組み）を保持していたため
  起動できなかった。ロック機構が期待どおり「先着1本に制限」しており、
  誤って多重起動しないことも副次的に確認できた。先行実行の終了を待って
  から再実行した。
- 結果（所要 623 秒）: テストファイル 7 件中 6 件が green、テスト 14 件中
  13 件が green。
  - `collector-proxy-port-collision.test.ts`（本 Issue の回帰、ロギング
    プロキシ側だけのポート衝突）: **green**。
  - `collector-port-collision.test.ts`（Issue #64 回帰、WebSocket 側の
    ポート衝突）: **green**。
  - `error-paths.test.ts`（harness 経由で既定の `startCollector()`＝
    WS 4123 / proxy 4124 を使う経路）: **green**。従来の実装なら
    dev collector が 4001 を専有しているこの状況で collector 子プロセスが
    EADDRINUSE で即死し、e2e が起動不能になっていた経路。今回は正常に
    起動・完走した。
  - `a-b-layer.test.ts` / `d-layer.test.ts` / `reconnect.test.ts`: すべて
    green（各テストが collector 子プロセスを正常に起動できている）。
- 以上より、dev collector が 4001 を専有している状態でも e2e の各
  collector 子プロセスは（`port + 1` の専用ロギングプロキシポートを使う
  ため）4001 と衝突せず正常に起動・完走することを実機で確認した。
  **元 Issue の「起動不能」は解消している。**

唯一の失敗（#254 とは無関係・別要因）:

- `commands.test.ts > addNode > 最重要: 追加した reth が既存チェーンに
  ブロック追従する（0 のままにならない）` が
  `waitForBlockCatchUp` の全体タイムアウト（540000ms）超過で失敗
  （高さ 5501 / ターゲット 8193）。
- これはポート衝突とは無関係。当該テストの collector 子プロセスは正常に
  起動し、実際にブロック高 5501 まで追従できていた（＝ `startCollector`
  は成功している）。失敗したのは、新規追加した reth が長時間稼働
  （4 時間以上・履歴 8000 超）のチェーンの head 高までを固定タイムアウト
  内に同期しきれなかったためで、collector 起動の問題ではない。
- 原因はブランチが main より 58 コミット遅れており、main 側で既に入って
  いる修正 `ec97931`（feat(e2e): head到達ではなく最低進行量で追従完了を
  判定するヘルパーを追加）/ `d724122`（fix(e2e): PROTO-CMD-01の合格条件を
  head完全追従から最低進行量に変更）を、この検証対象ブランチが取り込んで
  いないこと。`d724122` のコミットメッセージにも「稼働約2.8時間を超えた
  スタックに対してPROTO-CMD-01が構造的に必ずタイムアウトしていた」と
  明記されており、今回観測した失敗（4 時間以上稼働で必ずタイムアウト）と
  一致する。main の合否基準（最低進行量で判定）では通る。

シナリオ2（dev collector 停止状態での回帰確認）について:

- dev collector（pid 1753413）は別セッションの稼働環境である可能性が高く、
  検証のために停止すると他の作業を妨げるおそれがあるため、あえて停止
  させての実行は行わなかった（並行して動いている別プロセスに手を入れない
  という運用方針に従った）。
- 回帰の観点は、より条件の厳しいシナリオ1（4001 が専有された状態）で
  既定ポートを使う `error-paths.test.ts` を含む 6/7 ファイルが green で
  あることで実質的にカバーされている。dev collector が居ない状態は
  4001 が空くだけで e2e 側の使用ポート（4124/4126/4200/4211）は変わらず、
  シナリオ1 の真部分集合にあたるため、#254 の修正に関して差は生じない。

差し戻し要否:

- **Issue #254 の修正自体に差し戻しは不要**（完了条件を満たしている）。
- addNode 追従テストの失敗は #254 とは別要因（ブランチが main 遅れで
  未取り込みの修正 `ec97931`/`d724122`）であり、collector 実装担当への
  差し戻し対象ではない。マージ前にこのブランチへ main を取り込む
  （merge/rebase）ことで解消する見込み。統括へ「マージ前に main を
  取り込んでから最終確認する」ことを推奨として申し送る。
