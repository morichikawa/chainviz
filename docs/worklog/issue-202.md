...
### 2026-07-09 Issue #202 レビュー記録(reviewer 合格)

- 担当: reviewer(査読)
- ブランチ: issue-202-ui-err-multi-scenarios(mainからの9コミット、HEAD 59ee341)

#### 確認内容と結果(合格)

- **ビルド・lint・テスト**: `pnpm build` / `pnpm lint` / `pnpm test`
  (shared 58 / e2e 77 / collector 1103 / frontend 1372)がすべて成功。
- **§8.4規約との対応**: 6シナリオ(UI-ERR-01〜04・UI-MULTI-01/02)とも
  `test()` タイトルが「<シナリオID>: <タイトル>」でSCENARIOS.mdの見出しと
  1対1に対応し、前提・操作・確認の各箇条書きが `test.step` として
  実装されていることを突き合わせで確認した。
- **移行元WSテストの同等性**: `reconnect.test.ts` から削除された2 describe
  (「再接続時のスナップショット整合性」「複数クライアント同時接続時の
  差分配信」)は、`multi-client.spec.ts` のUI-MULTI-02(切断前の不在確認→
  実WebSocket切断→別クライアントでの追加→再接続後スナップショットでの
  出現確認)・UI-MULTI-01(Aの追加がBへブロードキャスト→Bの削除が両方へ
  反映)で同等以上に検証されている。操作コマンドの種類(addNode→
  addWorkbench)の差は、移行元自身のコメントどおり差分/スナップショット
  生成経路が共通のため本質でない。「残す」指定のPROTO-ERR-04/05
  (接続シーケンスのタイミング異常系)は無変更で残っていることを確認した。
- **collector-registry.ts の設計**: globalSetup/ワーカーが別OSプロセスで
  動くというPlaywrightの制約に対するファイルベース受け渡しは妥当。
  固定パスの衝突は `acquireE2eLock()`(test:e2e/test:e2e:ui共用ロック)+
  `workers: 1` + `fullyParallel: false` で直列化されており、ユニット
  テストは per-test 一意パスを使うため実行時パスに触れない。エラーの
  握りつぶしは無し(clearの unlink 冪等catch・readの「未登録扱い」catch
  はいずれも意図がコメントで明示され、意味論として妥当。multi-clientの
  afterAll `.catch(() => {})` もIssue #238への申し送りコメント付き)。
- **固定値の前提条件**: GHOST_DISAPPEAR_TIMEOUT_MS(60s+10s)・
  OVER_BALANCE_AMOUNT_ETH(999999999999 ETH)とも、値の根拠と見直し条件が
  コード内コメントと本worklogの両方に明記されており、運用ルールを満たす。
- **テストの質**: `collector-registry.unit.test.ts`(26ケース)は破損
  ファイル・型不一致・空ファイル・非オブジェクトJSON・PID死亡(spawnSync
  でreap済みPIDを取得しPID再利用の偽陽性を回避)・SIGTERM→SIGKILL切替の
  境界を実質的に検証しており、実装の詳細をなぞるだけの無意味なテストは
  見当たらない。
- **コミット粒度**: 9コミットとも1関心事(レジストリ機構/各spec/WSテスト
  削除/SCENARIOS更新/worklog/テスト強化/その記録)に閉じており、
  Conventional Commits形式に適合。
- **docs整合**: PLAN.mdのチェック・WORKLOG.md索引・SCENARIOS.mdの
  棚卸し表(移行済み(#202で削除))とも実装と一致。UI-CMD-07の確認文言
  (ウォレットカードはオーファン表示で残る)は本ブランチの差分には
  含まれない(マージベース時点で確定済み)が、CONCEPT.md「ノード/
  ワークベンチを削除したときの過去データの扱い」の決定と一致している
  ことを念のため確認した。

#### 軽微な指摘(差し戻し不要。次回以降の参考)

- `connection-errors.spec.ts` のローカル関数 `anyGhostCard` は
  `ui/support/operations.ts` が同名関数をexport済みのため、本来は
  importで足りる(重複定義)。動作・可読性への実害は無い。
- `collector-registry.ts` のPIDベース停止には、受け渡しファイルが
  異常終了で残った場合のPID再利用(別プロセスへのSIGTERM)という理論上の
  窓がある。`e2e-lock.ts` の既存の生死判定と同じ限界であり、globalSetupが
  毎回上書き登録すること・ロックによる直列化で実質的に緩和されている。
- mainが本ブランチ分岐後に3コミット(docs: #232/#233バックログ追記)
  進んでいる。変更行は重ならないためクリーンにマージできる見込みだが、
  マージ時にPLAN.md/WORKLOG.mdの自動マージ結果を一応確認すること。

コード変更は行っていない(worklogへの本記録の追記のみ)。

### 2026-07-10 Issue #202 QA検証記録(合格)
...
稼働中の chainviz-ethereum スタック(compose 定義どおりの7コンテナ:
reth1/reth2/beacon1/beacon2/validator1/validator2/workbench、動的追加
ノード無し)を再利用した。Playwright の chromium 実行に必要な
システムライブラリ(libnspr4 / libnss3 / libasound2t64)がホストに未導入
だったため、`apt-get download` で該当 deb を取得しユーザー領域へ展開して
`LD_LIBRARY_PATH` を通す形で実行した(sudo 不可のため)。

#### 実施内容と結果

1. **UI 層 E2E フルスイート(`pnpm test:e2e:ui`、29 テスト)を4回実行した。**
   - 1回目・2回目: 26 passed / 2 failed / 1 did not run。失敗したのは
     新規6シナリオではなく既存の UI-C 系2件(UI-C-05 token-balance・
     UI-C-02 wallet-balance)で、両回とも同じテストが同じ位置で再現的に
     失敗した。新規6シナリオ(UI-ERR-01〜04・UI-MULTI-01/02)は全4回とも
     green。
   - 原因を切り分けた結果、**前セッションの中断された実行が残した停止済み
     コンテナ `chainviz-ethereum-e2e-ui-c-token-recipient-1`(Exited、私の
     実行開始より前から存在)が環境に残っていた**ことが根本原因と判明した。
     タスク冒頭の `docker compose up -d` は compose 定義の7コンテナを立て
     直すだけで、動的追加されたワークベンチコンテナは削除しないため、この
     残骸が生き残っていた。UI-C-05 が同名ラベル `e2e-ui-c-token-recipient`
     のワークベンチ追加を要求すると、collector がラベル重複を検知して
     `e2e-ui-c-token-recipient-2` と付番するため、テストが待つサフィックス
     無しのカード ID が見つからず失敗していた(失敗時スナップショットで
     バッジは「接続済み」、実際に表示されていたカードは末尾 `-2` 付き)。
   - UI-C 系2件を単独実行(`playwright test token-balance wallet-balance`、
     #202 の新規テストを一切含まないフレッシュな collector)したところ、
     残骸コンテナが存在する状態では **UI-C-05 が単独でも同様に失敗し、
     UI-C-02 は単独では成功**した。これにより、UI-C 系の失敗は #202 の
     新規テスト(collector の kill/再起動)とは無関係な、環境汚染に起因する
     既存 UI-C テストの脆さであることを確定した。フルスイートでの UI-C-02
     の失敗は UI-C-05 失敗後に collector が「切断」状態に至った巻き添え
     (失敗時スナップショットで「切断」「表示するコンテナがありません」を
     確認)。
   - 残骸コンテナ(`e2e-ui-c-token-recipient-1` および付番された
     `-2-N`)を削除して環境をクリーン(7コンテナ)に戻した状態で
     3回目・4回目を実行したところ、**いずれも 29 passed(所要 3.6〜3.7分)**。
     実行後の残骸コンテナも無く、7コンテナのまま。新規6シナリオを含め
     全シナリオが安定して green になることを確認した。
2. **プロトコル層 E2E(`pnpm test:e2e`)を実行し、6ファイル13テストすべて
   green** を確認した。`reconnect.test.ts` は残す指定の PROTO-ERR-04/05
   の2テストのみになっており(移行対象2件は削除済み)、両方とも green。
   削除した2件の機能は UI-MULTI-01/02 でカバーされている。
3. **ブラウザ相当(実 chromium)での目視確認**(手動 collector[port 4200]
   + vite[port 5300] を立て、スクリーンショット取得):
   - collector プロセスを実際に SIGKILL すると接続ステータスバッジが
     「切断」へ変わり(カードは最終スナップショットのまま残る)、collector
     再起動後にリロードすると「接続済み」に戻り7ノード分のカードが再表示
     されることを確認した(UI-ERR-01 相当)。
   - 送金フォームの**金額**に `abc` を入力すると、送信ボタンが無効化され
     インラインのバリデーションエラーが表示され、コマンドは送信されない
     ことを確認した(UI-ERR-03 相当)。
   - 2つのブラウザコンテキストを開き、一方(A)でワークベンチを追加すると
     もう一方(B)にもカードとウォレットカードが出現し、B で削除すると
     両方から消えることを確認した(UI-MULTI-01 相当)。

#### 完了条件の判定

Issue #202 の完了条件(6シナリオが green・移行元 WS テスト削除後の穴が
無い・プロトコル層が引き続き green)を**満たしている(合格)**。新規6
シナリオはクリーン環境・汚染環境いずれの実行でも一貫して green であり、
#202 の変更に起因する不具合は検出しなかった。

#### 検証中に判明した申し送り(#202 の完了条件には影響しない)

- **既存 UI-C テスト(#201 由来)の環境汚染への脆さ**: 停止済みの動的
  ワークベンチコンテナが残っていると、同名ラベルの付番ズレで UI-C-05 が
  再現的に失敗する。`docker compose up -d` だけでは動的コンテナが消えない
  ため、フルスイート実行前には動的追加コンテナ(`e2e-*` / 過去ラベル)の
  掃除が必要。これは #238 の対応候補「各 spec の後片付けの頑健化」と
  関連する。本 Issue の範囲外だが、UI 層 E2E をクリーンに回すための前提
  として記録しておく。
- 今回の4回の実行では、クリーン環境での2回はいずれも #238 のカスケード
  失敗(collector クラッシュ)を再現しなかった。汚染環境での2回で見えた
  UI-C-02 の「切断」失敗は上記のとおり UI-C-05 の巻き添えであり、
  #238 が報告する「他 spec 実行中の原因不明クラッシュ」とは別系統。
