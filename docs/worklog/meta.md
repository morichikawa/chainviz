...
### 2026-07-08 ステップ10のIssueリンク追記(docs/PLAN.md)のレビュー(reviewer)

- 担当: reviewer(査読)
- ブランチ: main(未コミットの docs/PLAN.md 変更をレビュー)
- 対象: ステップ10(E2EテストのPlaywright移行)の7チェックボックスへの
  Issueリンク(#197〜#203)追記と、milestone #9 へのリンク追加。
- 確認内容と結果(合格):
  - Issueの実在と対応: #197〜#203 の7件すべて OPEN で存在し、milestone
    「Step 10: E2EテストのPlaywright移行」(milestone #9)に紐付いている
    ことを `gh issue view` / `gh api` で確認。各チェックボックスの記述と
    Issueタイトル・本文の内容が1対1で対応している。
  - 記述形式の一貫性: `[#番号](URL)` をチェックボックス本文の次行に
    インデントして置く形式で、ステップ9(#183〜#191)と同一。milestone
    リンクの `GitHub: [milestone](.../milestone/9)` もステップ1〜9と
    同一形式。未チェックの項目に先行してリンクを付ける運用もステップ9
    (#186〜#189, #191)で既に行われており前例と整合。
  - milestone URL: milestone/9 が実在し(open, open_issues=7)、URLも一致。
  - 依存関係: #199〜#202 の本文に「Depends on #197, #198」、#203 の本文に
    「Depends on #188, #189, #197, #198」が明記されており、PLAN.md の
    「ステップ9の #188/#189 の実装完了が前提」という記述とも整合。
  - ラベル: #198 が frontend、他6件が collector。e2e系Issueに collector
    ラベルを付ける運用は既存の #191 と同じで前例に整合。
- 注意点(指摘ではなく申し送り): ワーキングツリーには CLAUDE.md の
  未コミット変更(自律進行の許可範囲の追記。セッション開始時点から存在)が
  同居している。docs/PLAN.md のコミット時は「1つの変更内容 = 1コミット」の
  原則に従い、CLAUDE.md とは別コミットに分けること。
- docs のみの変更(コード変更なし)のため、CLAUDE.md の例外規定により
  chainviz-qa は省略。

### 2026-07-09 PLAN.mdバックログへのIssue #232/#233追加のレビュー(reviewer 合格)

- 担当: reviewer(査読)
- ブランチ: issue-232-233-backlog(コミット 80d0440)
- 対象: Issue #201(C層シナリオのPlaywright実装)の作業中に見つかった
  別問題2件(#232: 確定時のコントラクトへのパルス/フラッシュがアドレス
  表記の食い違いで発火しない、#233: UI-CMD系Playwrightテストの
  afterAllクリーンアップが競合状態で無効化されうる)を `docs/PLAN.md` の
  バックログセクションへ追記する変更。
- 確認内容と結果(合格):
  - 差分の範囲: `git diff main` で `docs/PLAN.md` のみ +6行。他ファイル
    への意図しない差分の混入は無い。ワーキングツリーもクリーン。
  - Issueとの対応: #232/#233 とも OPEN で実在し、GitHub上のIssueタイトル
    とPLAN.mdの記述が完全一致(改行による折り返しを連結すると同一文字列)。
    ラベルは #232 が frontend、#233 が collector で内容と整合。
  - 書式の一貫性: 既存のバックログ項目(#221/#223/#224 等)と同じ
    `- [ ]` + 6スペースインデントの折り返し + 次行に `[#番号](URL)` の
    リンクという形式で揃っている。追記位置もバックログ末尾で既存の
    並び(起票順)と整合。
  - コミット粒度: main..HEAD は1コミットのみで、docsへの記録という
    1つの関心事に閉じている。コミットメッセージも Conventional Commits
    形式(docs:)に適合。
- docs のみの変更(コード変更なし)のため、CLAUDE.md の例外規定により
...
### 2026-07-16 docs/PLAN.md バックログのIssue #322更新・#343追加のレビュー

- 担当: reviewer
- ブランチ: docs-issue-322-343-backlog
- 内容: Issue #322(slot time 12秒化)の設計完了と #343(ブロック生成
  インジケータ)への分割に伴う `docs/PLAN.md` バックログ節の更新
  (packages/* のコード変更なし)をレビューした。結果は合格。
  - 設計メモ(`docs/worklog/issue-322.md`・`issue-343.md`、ブランチ
    `issue-322-slot-time-and-indicator` 上)と PLAN.md の記述
    (values.env の3変数変更・genesis再生成・#286閾値据え置き・
    分割理由・#322先行マージ推奨)の一致を確認した
  - GitHub の Issue #322(分割判断のコメントあり)・#343 本文とも
    整合していることを確認した
  - 体裁(6スペースインデント・補足の括弧書き・リンク行・並び順)は
    既存項目と一貫している
- 決定事項・注意点(いずれも修正必須ではない軽微な表現の指摘):
  - 「3変数を12に変更」は厳密には `SLOT_DURATION_MS` が 12000 になる
    ため「12秒相当に変更」がより正確
  - 「E2Eの固定待ち時間2箇所を修正」は設計メモの「修正必須2箇所」に
    対応するが、実際のE2E変更範囲は他に定数追従3箇所・コメント更新
    2箇所・slot-duration出所の新設を含む。詳細は Issue/worklog に
    委ねる要約として許容した
  - GitHub の Issue #322 のタイトルは分割前のまま(インジケータを含む)
    なので、統括の判断でタイトル更新を検討するとよい
...
### 2026-07-18 docs/PLAN.mdのIssue #364・#216チェック漏れの修正

- 担当: 統括
- ブランチ: docs-plan-md-issue364-fix
- 内容: 「今あるissue全部片づけて」の再指示を受けてオープンIssueを
  棚卸しした際、既にマージ・クローズ済みのIssue #364(サンプルコントラクト
  のトークンシンボル変更)のチェックボックスが`docs/PLAN.md`上で未完了
  (`[ ]`)のまま残っていることを発見した。実際のPR(#384)・Issue状態
  (CLOSED)とは矛盾する。
- 原因の実測: 当初「巻き戻り」(一度`[x]`になったものが他ブランチのマージで
  戻った)と推測したが、reviewerの指摘を受け`git log --all -S '- [x] サンプル
  コントラクトのトークンシンボル' -- docs/PLAN.md`で全履歴を検証したところ、
  このチェックボックスが`[x]`になったコミットは本修正が最初だった(PR #384の
  マージコミット時点でも`[ ]`のまま)。**実態は「#364完了時にチェックボックス
  更新を含め忘れたままマージされた」という更新漏れであり、巻き戻りではない**。
  同様の手法でIssue #216(beacon/rethペア制約の疑問。2026-07-10に「現状維持が
  妥当」で決着しCLOSED済み)も同じ更新漏れが見つかったため、あわせて修正した。
- 対応: #364はPR #384の内容(案B・命名変更のみ・CVZDEMO/CVNDEMO・影響範囲の
  表記統一)に基づきチェックボックスを`[x]`に修正し完了内容の要約を実態に
  合わせて更新。#216もチェックボックスを`[x]`に修正し`docs/worklog/issue-216.md`
  への参照を追記した。
- 注意点: 教訓は「マージ時に他のチェックボックスが巻き戻っていないか確認する」
  ではなく、「各Issueの完了PRに`docs/PLAN.md`のチェックボックス更新コミットが
  実際に含まれているかを、マージ前にレビューで確認する」こと。

### 2026-07-26 CLAUDE.mdへの運用ルール2件の追記(統括)

- 担当: 統括（指揮統）
- ブランチ: main（直接コミット。理由は下記「注意点」参照）
- 内容: ユーザーから2件の指摘・依頼を受け、`CLAUDE.md`に運用ルールを2コミット
  に分けて追記した。
  1. マージ時のObsidian知見記録ルールの明文化: グローバル設定
     （`~/.claude/CLAUDE.md`）には元々Obsidian vault `llm_knowledge`への
     記録方針があったが、chainviz側のCLAUDE.mdには「統括がマージ時に
     worklogを確認して記録する」という運用の明記が漏れており、実際に本
     セッションで#408〜#430まで多数のIssueをマージしながら一度も記録して
     いなかったことが発覚した。ユーザーからの質問（「オブシディアンは
     更新されてるの？」）で発覚し、正直に未実施を認めた上で明文化した
  2. git worktreeの後片付けルール追記: ユーザーが`/home/zoe/workspace/`
     直下で`chainviz-wt-113`等、`git worktree add`由来と見られる空の
     残骸ディレクトリ（中身がほぼ空、gitのworktree登録からは外れている）
     を発見し、原因調査と再発防止を依頼された。調査の結果、5つとも
     `git worktree list`に未登録・中身は`packages/`や`profiles/`の空フォルダ
     殻のみで、`git worktree remove`を経ずに中身だけ`rm -rf`等で消された
     残骸と推測された。zoe所有の4件はこのセッションで削除、root所有の
     `chainviz-wt-issue56`はsudoパスワードが必要でユーザーに手動削除を
     依頼した
- レビュー: `chainviz-reviewer`が2コミットの差分・技術的正確性
  （`git worktree remove`/`--force`/`prune`の実際の挙動を検証環境で再現して
  確認）・既存文体との整合を確認し合格。コード変更を伴わないため`chainviz-qa`
  は省略（CLAUDE.mdの既存の例外規定に準ずる判断）
- 注意点（reviewerからの指摘）:
  - `git worktree remove --force`は「未コミットの変更が残っている場合」に
    必要で、ディレクトリが既に消えている場合は素の`remove`でも成功する。
    CLAUDE.md本文の記述は誤りではないが「常に`--force`が必須」と読める
    余地があり、将来より精度を上げる余地がある
  - `git log --oneline main -- CLAUDE.md`を辿ると、CLAUDE.md自体の運用
    ルール改訂はこれまでも一貫して`main`への直接コミットで行われてきた
    確立済みの慣行だった（機能開発Issueのような「Issueごとにブランチを
    切る」対象ではなく、開発ガイド自体の自己言及的な改訂という性質の
    違いによるものと考えられる）。ただしこの例外はCLAUDE.md本文には
    明文化されておらず、将来誤読・拡大解釈されるリスクがreviewerから
    指摘されている。明文化の要否はユーザー・統括の今後の判断に委ねる
