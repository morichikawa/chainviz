...
### 2026-07-18 docs/PLAN.mdのIssue #364チェックボックス巻き戻りの修正
...
- 内容: 「今あるissue全部片づけて」の再指示を受けてオープンIssueを
  ...
  (CLOSED)とは矛盾しており、他の完了済みIssue(#346/#351/#352/#366/#369/
  #373)は正しく`[x]`だったため#364のみの局所的な巻き戻りと判明。
  おそらく#364マージ後に並行して分岐していた別ブランチ(#385等)が
  #364マージ前のPLAN.mdを基点にしており、そのauto-mergeで巻き戻った
  ものと推測される(原因の完全な特定はしていない)。
- 対応: PR #384の内容(案B・命名変更のみ・CVZDEMO/CVNDEMO・影響範囲の
  表記統一)に基づき、チェックボックスを`[x]`に修正し完了内容の要約を
  実態に合わせて更新した。
- 注意点: 今後、複数ブランチが並行してPLAN.mdを編集する運用では、
  マージ時に「他のチェックボックスが巻き戻っていないか」の確認を
  習慣化する余地がある。
