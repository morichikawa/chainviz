...
### 2026-07-19 Issue #401 差し戻し対応の再確認（reviewer）

- 担当: reviewer
- ブランチ: issue-401-hash-computation-viz
- 判定: **合格**
- 確認した内容（コミット `6ee3d1a`）:
  - `keccak256.boundary.test.ts` 50行目の生NUL文字がエスケープ
    シーケンス `"\u0000"` に置き換えられ、ファイルがテキスト判定に
    戻ったこと（`file` で UTF-8 text、`git diff main --numstat` で
    行単位の差分表示、制御文字の残存ゼロを実測）
  - 副次的修正: 合成文字/分解文字の比較テスト（34-35行目）も
    `"\u00e9"` / `"e\u0301"` のエスケープ表記に変更されている。
    生リテラルはエディタ等のUnicode正規化で同一バイト列に潰れて
    テストが壊れる恐れがあり（実際に修正作業中に発生したとの報告）、
    エスケープ化は根治として適切
  - 当該ファイルのテスト 6/6 pass・eslint クリーンを自分でも実行して
    確認。作業ツリーもクリーン
  - コミットは単一関心事・Conventional Commits 形式
  - 注記: コミット `6ee3d1a` 自体の diff は「Binary files differ」と
    表示されるが、これは修正前のblobがバイナリ判定だったためで不可避。
    以後の差分はテキストとして表示される
- これで静的レビューの指摘事項はすべて解消。Issue #401 は reviewer
  として合格とする。残る完了条件は chainviz-qa による実ブラウザでの
  UI-HASH-01 実行を含む動作検証

### 2026-07-19 Issue #401 最終QA検証結果（合格）

- 担当: qa
- ブランチ: issue-401-hash-computation-viz
- 判定: **合格**（完了条件をすべて満たしている）
- 検証環境: 稼働中の共有 Docker スタック（chainviz-ethereum、reth block が
  118→140 と進行中であることを確認）を再利用。E2E 排他ロック
  （/tmp/chainviz-test-e2e.lock）は未保持、UI 層 E2E 用ポート（collector
  4125 / vite 5275）も未使用で、他エージェントが同スタックで E2E を
  走らせていないことを確認してから着手した。
- 実施した検証:
  1. **E2E `UI-HASH-01` を実 Docker + 実ブラウザで実行 → 通過**。
     `LD_LIBRARY_PATH=/home/zoe/chrome-deps/root/usr/lib/x86_64-linux-gnu`
     を付与して `pnpm test:e2e:ui -g UI-HASH-01` を実行し、chromium が
     起動して 1 passed（51.9s）。入口ボタン→3ブロック全有効→#1データ
     編集で#2のみ無効・#3は有効のまま→#2つなぎ直しで#2有効・#3無効
     （連鎖修復）→#3つなぎ直しで全有効+まとめメッセージ→resetで初期
     データ・全有効に戻る、という一連の操作が実ブラウザで通ることを
     確認した。実装担当・reviewer が実行できていなかった完了条件の
     残項目が解消。
  2. **手動での実ブラウザ操作確認**。worktree の vite dev server を別ポート
     （5399、既存 collector 4000 に接続）で起動し、実際に画面を操作して
     以下を目視・スクリーンショットで確認した:
     - チェーンリボンカードの「ハッシュのしくみを試す」導線
       （testid `chain-ribbon-hash-demo-open`）からサイドパネル
       「ハッシュのしくみ」が開く。
     - 初期状態は3ブロックとも「有効」（緑バッジ）で連結線が通り、
       #2の親ブロックのハッシュ（0x4e0e47…fd7c）が#1の導出ハッシュと
       一致、#3の親ハッシュ（0x1cc331…fd4a）が#2と一致していることが
       目で見比べられる。各ブロックは「格納情報の枠 → keccak256 で
       ハッシュ化の処理帯 → 枠の外にこのブロックのハッシュ」という
       配置で、自分のハッシュは自分の中に格納されず次に渡ることが
       レイアウトで表現されている。
     - #1のデータを「Alice → Bob: 999 ETH」に書き換えると#1の導出
       ハッシュが 0x5c5c5d…eee0 に即座に変わり、**直後の#2のみが赤枠+
       「無効: 親ブロックのハッシュと食い違っています」バッジ+「親ハッシュを
       つなぎ直す」ボタン**になり、#3は「有効」のまま（設計どおり直後
       1ブロックのみ無効化）。無効表示は色だけでなく文言も併記されている。
     - パネル末尾に「どこで計算されるか」（実行クライアントが計算・各ノードが
       再計算検証・collector は転記のみ）と簡略化注記（RLP への言及）が
       表示される。
     - ページエラー（pageerror）は一度も観測されなかった。
  3. **用語集 `hash` エントリの表示確認（実ブラウザ）**。用語集パネルを開き
     「ハッシュ」で検索、`glossary-panel-term-hash` を展開すると、定義文全体
     （「指紋」「keccak256」への言及・砂場デモへの案内を含む）が正しく表示され、
     関連用語チップ「ブロック」「トランザクション（tx）」も表示された。
- 完了条件との照合:
  - チェーンリボンからハッシュ改ざんデモパネルを開ける: 満たす
  - 編集で直後1ブロックのみ無効化（色+バッジ+文言）: 満たす
  - 「親ハッシュをつなぎ直す」による連鎖修復の体験: 満たす（E2E で検証）
  - reset で初期状態に戻る: 満たす（E2E で検証）
  - 用語集 hash エントリの表示: 満たす
  - E2E `UI-HASH-01` の実ブラウザ実行: 満たす（通過）
- 差し戻しなし。docs/PLAN.md の該当チェックボックス更新は統括に委ねる。
