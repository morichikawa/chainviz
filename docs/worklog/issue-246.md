...
### 2026-07-11 Issue #246 レビュー

- 担当: reviewer
- ブランチ: issue-246-validator-service-detection（先端 2f55115 時点）
- 判定: **差し戻し（軽微・docs の1箇所のみ）**

#### 確認した内容（問題なし）

- 境界の遵守: `isValidatorService` は Ethereum アダプタ内
  （`packages/collector/src/adapters/ethereum/targets.ts`）に閉じており、
  `packages/shared`・frontend にチェーン固有語彙は漏れていない。
  サービス名の命名規則への依存（新チェーン追加のたびに衝突リスクを
  持ち込む要因）を取り除く方向の変更で、「チェーンプロファイル単位で
  増やす」原則にも沿う
- `ROLE_LABEL` の付与経路: 静的コンテナは
  `profiles/ethereum/docker-compose.yml` の全ノード系サービス
  （reth1/reth2=execution、beacon1/beacon2=consensus、
  validator1/validator2=validator、workbench=workbench）に設定済み。
  動的コンテナは `node-lifecycle.ts` の `nodeLabels()`（execution|consensus
  のみ。addNode は VC を作らない）・`workbenchLabels()`（workbench 固定）が
  必ず付与する。ラベル欠落時は `false` → `p2pRole: "peer"` で、
  `MANAGED_LABEL`/`P2P_ROLE_LABEL` と同じ「省略・想定外 = 安全側」の流儀に
  一貫している
- `p2pRole` 導出（`index.ts` の `toEntity`）: 差分を確認し、ロジックは
  無変更・コメントのみ更新であることを確認（worklog の説明どおり）
- テストの質: `targets.test.ts` はラベル厳密一致の契約（一致で true、
  欠落・他ロール・near-miss 11件で false、サービス名非依存の双方向確認）を
  固定しており、将来 `.includes` やトリム・大文字小文字無視へ緩めた場合に
  検出できる。`index.test.ts` は統合レベルで #246 の回帰（"tx-validator1"
  という名前の execution ノードが peer のまま）・大文字非正規化・ラベル無し
  の互換をカバー。回帰テストが旧実装で実際に失敗することの確認も worklog に
  記録済み
- エラー握りつぶし・環境状態依存の固定値: 該当なし（純粋な述語関数）
- ビルド・lint・テスト: リポジトリ全体で `pnpm lint` / `pnpm build` /
  `pnpm test` すべて成功（shared 62・collector 1167・frontend 1732・
  e2e 97 テスト）
- コミット粒度: 4コミット（fix=実装+対応テスト、docs=設計メモ・記録、
  test=境界値強化、docs=強化記録）。Conventional Commits 準拠・
  1変更1コミットの規約どおり

#### 指摘事項（要修正）

1. `docs/ARCHITECTURE.md` の `p2pRole` フィールドのコメント（109〜110行目
   付近）に「Ethereum アダプタは compose サービス名に "validator" を含む
   コンテナを "none" と判定」という旧実装の記述が残っている。今回の変更で
   判定材料は `com.chainviz.role` ラベルの厳密一致に変わったため、この
   記述を実装に合わせて更新すること（sync-docs の観点。なお 118 行目以降の
   `nodeRole` の記述は現状と整合しており修正不要）

### 2026-07-11 Issue #246 レビュー指摘対応（docs追従）

- 担当: collector
- ブランチ: issue-246-validator-service-detection

レビューで指摘された `docs/ARCHITECTURE.md` の `p2pRole` フィールド
コメントを修正した。旧実装の記述「Ethereum アダプタは compose サービス名に
"validator" を含むコンテナを "none" と判定」を、現在の
`isValidatorService`（`packages/collector/src/adapters/ethereum/targets.ts`）
の実装に合わせて「Ethereum アダプタは `com.chainviz.role` ラベルの値が
厳密に "validator" と一致するコンテナを "none" と判定」に更新し、
Issue #246 で旧実装（compose サービス名への部分一致、Issue #214）から
変更した経緯が分かるようコメントに追記した。コード自体の変更はない。

- 検証: docs のみの変更のため `pnpm build` / `pnpm test` の再実行は
  不要（変更前後でロジック・テストに影響なし）。差分箇所を目視で
