...
- 担当: qa
- ブランチ: issue-236-transfer-address-validation
- 判定: **合格**（元Issueの「不正なアドレス形式でも送金操作が送信できてしまう」
  が実際に解消されていることを実機で確認）
- 検証方法: frontendをモックモード（`VITE_COLLECTOR_URL`未設定で
  `resolveDefaultClient`が`isMock=true`となりモックデータで起動）で
  vite dev serverを立て、実ブラウザ（Playwright chromium）から
  モックの静的ワークベンチ`workbench-alice`の操作パネル→送金タブを
  開いて、宛先・金額欄への入力と送信ボタンの活性/エラー表示をDOMの
  実挙動として確認した。既存のUI層E2E（`form-validation.spec.ts`）は
  UI-ERR-03を金額側の不正入力に差し替えており宛先アドレス検証を
  カバーしていなかったため、本Issue用に宛先バリデーションを直接
  操作する検証を行った。全19チェック項目がpass。
- 確認した挙動:
  1. 不正な宛先アドレスの各パターンで、送信ボタンが無効化され、
     `operation-transfer-to-error`のインラインエラー
     「0xで始まる40桁の16進数のアドレスを入力してください（例: 0x1234…）」
     が表示されることを確認した（元Issueの再現手順そのもの）:
     - 短すぎる（`0x123`）
     - `0x`プレフィックス無しの40桁16進
     - 非16進文字を含む（`0xZZZZ...`）
     - 39桁16進（1桁不足）
     - 41桁16進（1桁超過）
  2. 宛先が空欄の間はエラー文言を表示せず、送信ボタンのみ無効化される
     （未入力とバリデーションエラーの区別が金額欄と同じ挙動）ことを確認。
  3. 不正な宛先（`0x123`）を有効なアドレス
     （`0x000000000000000000000000000000000000dEaD`）に訂正すると、
     エラー文言が消え送信ボタンが有効化されることを確認。
  4. 回帰: 有効な宛先 + 不正な金額（`abc`）では送信ボタンが無効のまま
     で、かつ宛先側のエラー（`operation-transfer-to-error`）は誤って
     出ないことを確認（アドレス検証が金額検証と独立して機能する）。
  5. 回帰: 有効な宛先 + 有効な金額（`3.14`）で送信ボタンが有効化され、
     クリックすると操作が発行され操作パネルが閉じる（通常の正常な送金
     操作が従来どおり機能する）ことを確認。
- 完了条件との照合: 元Issue（不正なアドレス形式でも送金操作が送信できて
  しまう）が実機で解消されていることを確認した。完了条件を満たしている。
- 差し戻し: 無し。
- 補足（環境メモ／実装への指摘ではない）: 本検証はworktreeが一度
  クリーンアップされた後に再作成し（`git worktree add`で
  `issue-236-transfer-address-validation`を再チェックアウト後、
  `pnpm install`）、モックモードのvite dev serverに対して実施した。
  Playwright同梱のchrome-headless-shellが実行ライブラリ
  （libnspr4/libnss3/libasound2）を欠いていたため、フルchromium
  （`chromium-1228`）を`LD_LIBRARY_PATH`で対応ライブラリを補って
  起動した。いずれも検証環境固有の事情で、実装・成果物への指摘ではない。
