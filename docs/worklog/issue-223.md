...
### 2026-07-11 レビュー指摘への対応(designer)

- 担当: designer
- 内容: 差し戻し2件を修正した
  1. `packages/collector/README.md` の環境変数一覧に
     `CHAINVIZ_COLLECTOR_PORT` を追記し、「主な環境変数」を「環境変数
     (全 5 つ)」に改め、各変数の用途を一言ずつ添えた。「主な」という
     含みのある書き方は「READMEを実態の正とする」目的と相性が悪いため、
     全数列挙であることを明示する書き方に変えた
  2. `docs/ARCHITECTURE.md` §1冒頭を「4パッケージ(shared / collector /
     frontend / e2e)に分割し、shared の型を残る 3 パッケージから参照する」
     に修正した(e2e も package.json で `@chainviz/shared` に依存している
     ことを確認済み)
- 決定事項・注意点: collector が読む環境変数はレビューと同じく
  `grep -rhoE "CHAINVIZ_[A-Z_]+"` で全数確認し、5 つで一致した

### 2026-07-11 再レビュー(reviewer)

- 担当: reviewer
- 内容: 前回差し戻し2件の対応(コミット `dc20c3c`)を確認し、**合格**と
  判定した
  1. `packages/collector/README.md` の環境変数一覧: 実装を
     `grep -rhoE "CHAINVIZ_[A-Z_]+"` で再確認し、collector が読む環境変数
     は5つで README の列挙と完全に一致した。各変数の用途説明も実装
     (`src/index.ts` のコメント・使用箇所)と齟齬なし。「主な」を
     「全 5 つ」に改めた判断も README を実態の正とする目的に適う
  2. `docs/ARCHITECTURE.md` §1: 「4パッケージに分割」は直下のツリーと
     一致。「shared の型を残る 3 パッケージから参照」も、collector /
     frontend / e2e の各 `package.json` が `@chainviz/shared` に依存して
     いることを確認した
- ビルド・lint・テスト: 前回レビューから差分は markdown のみで
  `packages/*` のコードに変更はない。`pnpm lint` の通過を再確認した
- 決定事項・注意点: 申し送りのあった本ファイルの prettier 非準拠
  (行折り返し)は、markdown が品質ゲート対象外のため対応不要との判断を
  追認する
