...
### 2026-07-19 Issue #402 最終QA検証結果(合格)

- 担当: qa
- ブランチ: issue-402-signature-verification-viz
- 判定: 合格。実装担当への差し戻しは無し。
- 検証環境: 実Docker Composeスタック(profiles/ethereum)。他エージェントの
  使用は無し(検証開始時、全コンテナは Exited 状態で稼働中のcompose
  プロジェクトは無かった)。Playwrightのchromiumは
  `LD_LIBRARY_PATH=/home/zoe/chrome-deps/root/usr/lib/x86_64-linux-gnu`
  を付与することで起動できた(素の状態では共有ライブラリ不足で起動不可を
  再現)。
- 実施内容と結果:
  1. `pnpm test:e2e:ui -g UI-SIG-01` を実Docker環境で実行し合格
     (1 passed, 約1.1分)。globalSetupが実チェーン起動・collector起動・
     vite dev起動を行い、chromiumで送金フォーム入口→改ざん→攻撃者
     再署名(なりすまし不成立)→Alice再署名→リセットの一連が通ることを
     確認した。
  2. 送金フォーム(transferタブ)下部の「署名と検証のしくみを試す」
     リンクからサイドパネル(kind signatureDemo)が開き、初期状態が
     「有効: 復元されたアドレスが送信者と一致」であることを実機で確認。
  3. 内容編集による署名の即時変化: 上ゾーン(ワークベンチ)の金額を
     編集すると署名データが別の値へ即座に変化することを確認
     (title属性の全文hexが `0x006f79dee082af906231d0…` から
     `0x007f98a8683e231c3cfde1…` へ変化。さらに宛先編集でも再度変化)。
     追従して検証は「有効」のまま。
  4. 下ゾーン「届いた内容」の金額改ざんで「無効: 復元されたアドレスが
     送信者と一致しません」に変わり、2つの再署名ボタンが出現することを
     確認。
  5. 「攻撃者の鍵で署名し直す」で、署名は数学的に正しくなるが依然
     「無効」のまま(復元アドレスが攻撃者のものになりなりすまし不成立)、
     攻撃者再署名の結果メッセージが表示されることを確認。
  6. 「Aliceが署名し直す(正しく送り直す)」で「有効」に戻り、攻撃者
     再署名ボタンが消えることを確認。
  7. 「最初に戻す」で届いた内容(金額=1)・バッジ(有効)が初期状態に
     戻ることを確認。
  8. 用語集attestationの表示: デモ末尾のattestationアンカー
     (glossary-term-attestation)をクリックするとglossaryパネルが開き、
     attestation行が展開表示され、定義文(「Attestation（証明）」/
     「バリデーターが…投票する証明」)が表示されることを実機で確認。
  9. txライフサイクルポップオーバー末尾の入口は、ボタンが
     `sidePanel.open({ kind: "signatureDemo" })` に配線されており
     (test-id tx-lifecycle-sig-demo-open-<hash>)、コンポーネントテスト
     TxLifecyclePopover.sigDemoEntry.test.tsx でカバーされている。
     完了条件は「txライフサイクル末尾 または 送金フォーム送信ボタン
     直前」のいずれかから開ければよく、後者を実機確認済みのため充足。
- 補足: 上記3・8の追加確認は一時的な検証用Playwright spec(実行後削除)で
  実機確認した。作業ツリーへの変更は残していない(git status クリーン)。
- 次の担当への申し送り: 合格。統括はマージ・PLAN.mdチェックボックス更新・
  Issueクローズを行う。
