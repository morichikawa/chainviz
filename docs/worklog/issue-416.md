...
### 2026-07-25 Issue #416 QA検証（qa）

- 担当: qa
- 検証対象: `origin/issue-416-eclipse-attack-demo`（441df4a「docs: Issue #416の
  レビュー結果(合格、占有率算出の重複を修正)を記録」）を専用worktreeに
  detached HEAD でチェックアウトして検証した。commit・pushは行っていない
- 判定: **合格**（Issue #416 の完了条件をすべて満たしている）

#### 検証環境

- ノード環境: `profiles/ethereum` の docker compose 環境（reth1/2・beacon1/2・
  validator1/2・workbench）が稼働中の状態を使用。`eth_blockNumber` が
  0x52b → 0x52c と進行していること（slot 12秒でブロックが進み続けている
  こと）を実測で確認した
- collector: 稼働中のプロセス（ポート4000）に接続。#416 は collector を
  変更しないため main 相当のビルドで検証した。WebSocket に接続して
  snapshot を受信し、entities 43件・peer エッジ2本（reth1↔reth2、
  beacon1↔beacon2）が流れていることを確認した（peerエッジが1本も無いと
  `PeerNetworkLegend` 自体が非表示になるため、入口ボタン検証の前提として
  先に確認した）
- frontend: 検証用worktreeで `pnpm install` → shared の build 後、
  `VITE_COLLECTOR_URL=ws://localhost:4000` で vite dev をポート5199で起動。
  モックではなく実 collector 接続（ヘッダーが「接続済み」表示）
- ブラウザ: ホストの Chromium が起動できないため、Docker の
  `mcr.microsoft.com/playwright:v1.61.1-jammy`（`--network host`）から
  Playwright で実ブラウザを操作し、DOM の実測値とスクリーンショットを取得した

#### 実機で確認した項目と結果

1. **入口**: 画面右下のP2Pネットワーク凡例に「eclipse攻撃のしくみを試す」が
   表示され、クリックでサイドパネル（タイトル「eclipse攻撃のしくみ」）が
   開く。ボタンにフォーカスして Enter でも開けることを確認した
2. **初期状態**: 8スロットすべてが正規ピア（aria-label「正規ピア」・グリフ
   ✓・青系チップ）。占有率テキストは「攻撃者ピア: 0 / 8（0%）」、メーターの
   塗り幅0%、バッジ「ネットワーク全体と一致しています」、見ているチェーンは
   正規3件（#1 Alice → Bob: 3 ETH / #2 Bob → Carol: 1 ETH /
   #3 Carol → Dave: 2 ETH）、警告文なし、被害ノードのclassは
   `eclipse-demo__victim`（--eclipsed なし）
3. **1クリック=1スロットの固定順置換**: 「攻撃者ピアを追加」を8回押し、毎回
   DOM を取得して確認した。置換はスロット0（12時）から7（10:30）へ時計回りに
   1つずつ進み、各段階で直前に置換されたスロットにだけ flash クラスが付く。
   占有率テキストとメーターの塗り幅は 0% → 13 → 25 → 38 → 50 → 63 → 75 →
   88 → 100% と推移した
4. **7/8 では正規のまま**: 攻撃者7個の時点でバッジ・チェーン内容・警告文・
   被害ノードのoutlineがいずれも初期状態と同一。偽ブロックの文言は画面上に
   1つも現れない
5. **8/8 で実際に切り替わる**: 8個目を押した瞬間にバッジが「攻撃者だけが
   見せている内容です」、チェーン内容が偽3件（#1 Alice → 攻撃者: 50 ETH /
   #2 攻撃者 → 攻撃者: 999 ETH / #3 Carol → 攻撃者: 50 ETH）へ入れ替わり、
   警告文が出現し、被害ノードのclassに `--eclipsed` が付いて枠が緑から赤へ
   変化した。「攻撃者ピアを追加」は disabled になり文言が「すべてのスロットが
   攻撃者に占められました」へ変わる。8/8到達後にさらに押しても100%のまま
   変化しない
6. **リセット**: 「リセット」で占有率0%・全スロット正規・正規チェーン・
   警告文なしの初期状態に戻る。キーボード（Tabでリセットへ移動 → Enter）でも
   同じ結果になった
7. **開き直すと初期化**: 8/8まで進めた状態で Esc でパネルを閉じ、再度入口から
   開くと「攻撃者ピア: 0 / 8（0%）」から始まることを確認した
8. **色以外の手がかり**: `html { filter: grayscale(1) }` を適用した状態で
   7/8・8/8のスクリーンショットを取得し、正規=✓/攻撃者=! のグリフで各スロットが
   判別できること、占有率テキスト・バッジ文言・警告文がすべてテキストとして
   読めることを目視で確認した。各スロットの aria-label（正規ピア/攻撃者ピア）
   と占有率テキストの `aria-live="polite"` も DOM 上で確認した
9. **実データからの独立性**: 8/8の状態で30秒待機し、実チェーンのブロックが
   #1478 → #1480 と進み続ける一方、砂場の状態は 8/8 のまま保持され、P2P凡例の
   実ピア接続数（実行1・コンセンサス1）も変化しないことを確認した。砂場が
   実データに影響せず、実データの更新で砂場の進捗が巻き戻ることもない
10. **用語アンカー**: フッターの「ノード発見」「ブートノード」にホバーすると
    定義ポップオーバーが出る。クリックすると用語集パネルが該当用語で開く
    （既存の `GlossaryTerm` の仕様どおりサイドパネルが用語集に切り替わるため
    砂場は閉じる。ハッシュ/署名デモと同じ挙動であり問題なし）
11. **英語表示**: ヘッダーの English に切り替えると、凡例の入口
    （Try how eclipse attacks work）・パネル内の全文言・8/8時の警告文・
    偽ブロックの文言がすべて英語化されることを確認した
12. **レイアウト**: 1024x700 のビューポート（サイドパネル幅420px・内容幅391px）
    でもミニ包囲グラフ（280x280）が収まり横スクロールが発生しない
    （scrollWidth === clientWidth）。全操作を通じて console error・pageerror は
    0件だった

#### 完了条件との照合

- 「サイドパネルからeclipse攻撃シミュレーションを開始・操作できる」: 満たして
  いる（上記1・3・6）
- 「実際に動く簡略化アルゴリズムで結果が決まる（演出のみのフェイクにしない）」:
  満たしている。7/8では切り替わらず8/8でのみ切り替わる境界が実機で観測でき、
  占有率メーターの塗り幅・テキストも 0/8〜8/8 の各段階で占有率の計算値と
  一致していた（上記3・4・5）
- 「対応するユニットテストがある」: 実装・tester・reviewer の記録どおり存在する
  （静的な合格確認は reviewer の担当範囲であり、QAでは再実行していない）
- 「`pnpm lint && pnpm build && pnpm test` が通る」: reviewer が確認済み

ARCHITECTURE.md §17.5.3 の要求（占有率100%到達時点で見え方が実際に切り替わる／
メインキャンバスのP2Pエッジの視覚言語を踏襲／攻撃者ピアは既存配色と衝突しない
専用色／中心の被害ノードを取り囲むミニグラフ）も実画面で満たされていることを
確認した。

#### Issue #413 との共存（依頼された確認項目）

- 現時点の `PeerNetworkLegend.tsx` には #413 の `eclipseAttack` 用語アンカーは
  無く（#413未マージ）、#416 の入口ボタンのみが存在する。既存の `discovery`
  用語アンカー（Issue #124 のヒント文中）とは干渉していない
- `origin/issue-413-attack-glossary-foundation` の同ファイルの差分を確認すると、
  #413 はヒント文 `<p>` の直後に用語アンカー行（class
  `p2p-legend__hint--attack`、testid `p2p-legend-eclipse-hint`）を追加する。
  #416 はまさに同じ位置に `<button>` を追加しているため、`git merge-tree` で
  試すと `PeerNetworkLegend.tsx` は content conflict になる（`docs/WORKLOG.md`
  も同様にconflict）。ランタイムの干渉ではなくマージ時のテキスト衝突であり、
  両者は独立した兄弟要素なので「ヒント文 → #413の用語アンカー行 → #416の入口
  ボタン」の順に並べるだけで解決できる
- 共存レイアウトが崩れないことを確認するため、#413相当の用語アンカー行を
  ブラウザ上でDOMに注入した状態のスクリーンショットも取得した。凡例は縦積みの
  ブロック要素なので行が1本増えるだけで、入口ボタンとの重なりは起きない
  （用語アンカー行の下端795px・ボタン上端801px）

#### 所見（いずれも差し戻し不要。統括の判断に委ねる）

1. **e2eシナリオが無い**: 同型の先行砂場である #401（UI-HASH-01 /
   `hash-chain-demo.spec.ts`）・#402（UI-SIG-01 / `signature-demo.spec.ts`）は
   いずれも `packages/e2e/SCENARIOS.md` への追記と Playwright スペックを
   伴っている。#416 にはどちらも無い。Issue #416 の完了条件にも #414/#415 の
   完了条件にも e2e は書かれていないため未達とは判定しないが、3つの砂場
   （#414/#415/#416）で揃えるかどうかは統括が方針を決めるとよい（今回のPRで
   追加する／3砂場まとめて別Issueにする、のいずれか）
2. **ARCHITECTURE.md 側の記述粒度**: 先行2砂場は §15/§16 に実装後の仕様
   （状態モデル・導線）が書かれているのに対し、eclipse砂場は §17.5.3 の設計
   時点の記述（スロット数は「例8個」、詳細はUX設計に委ねる）のままで、確定した
   固定8スロット・初期満員モデル・スロット順・`--attacker-peer` 変数などは
   `docs/worklog/issue-416.md` にしかない。整合を取るなら §17.5.3 への追記を
   検討する余地がある
3. 追加ボタンが disabled になった瞬間にフォーカスが body へ外れる。次の Tab で
   リセットボタンに到達できるためキーボード操作は詰まらない（tester も確認
   済み）が、厳密には disabled 化と同時にリセットへフォーカスを移すほうが
   親切。既存UIにも同種の箇所があるため本Issue固有の問題としては扱わない
4. tester・reviewer が記録した入れ子 `role="img"` のa11y所見について、実画面で
   「色以外の手がかり（グリフ・占有率テキスト・バッジ・警告文）で状態が区別
   できる」というUX設計の要件自体は満たされていることを確認した。スロット個別の
   aria-label が支援技術に届くかは実装依存であり、改善を将来の課題とする判断は
   妥当と考える

#### 次の担当（統括）への申し送り

- 完了条件を満たしており合格と判定する。`docs/PLAN.md` の該当チェックボックス
  （#416）は実装担当が既にチェック済みで、QAとして追加すべきチェックは無い
  （他の完了済み項目と同様の要約文の追記は、マージ時の統括の作業に委ねる）
- commit・push・PR作成・マージ・Issueクローズはいずれも行っていない
- #413 とのマージ時コンフリクト（`PeerNetworkLegend.tsx` /
  `docs/WORKLOG.md`）は上記のとおり解決方針が単純なので、どちらを先にマージする
  場合でも要素の並べ替えのみで解消できる
