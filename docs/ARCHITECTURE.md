...
- `subscribeBlocks` は起動時に一度だけ対象を列挙するため、addNode で後から
  ...
  （＝色分けの対象外。ブロック伝播パルスが動的ノードに乗らないのと同根の
  既存ギャップ）。本 Issue では「空文字列 = 未観測」の縮退として受容し、
  購読の動的追従は別 Issue として起票する。
...
- **included / failed tx（`blockHash` を持つ）は、対応する block の store 内
  存在に連動させる**。block 集合は既に番号窓（`BLOCK_RETENTION = 32`）で
  閉じているため、これに従わせるだけで included/failed tx も自動的に有界に
  なる（tx 自体に番号を持たせる shared 型変更は不要）:
