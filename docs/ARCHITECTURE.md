...
## 9. チェーンリボン（ブロックの連なり表示。Issue #298）
...
### 9.1 データフロー: 新しい観測・新しいスキーマは増やさない
...
### 9.2 collector 側: store のブロック保持に窓を設ける
...
- tx（`TransactionEntity`）も store で無制限蓄積している点は本 Issue の
  範囲外（ブロックと違い表示が「直近 N 件」に閉じないため別途設計が要る）。
  バックログ Issue として起票して扱う
