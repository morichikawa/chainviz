...
- この実測は slot time 2 秒当時のもの。slot time を現実の 12 秒へ戻す
  Issue #322 により、tx 取り込み・ブロック出現を待つテストの所要時間が
  伸びる（見積もり +2〜5 分。`docs/worklog/issue-322.md` §6.5）。#322 の
  QA で再実測し、この節の値を更新すること。あわせて e2e が前提とする
  slot time は `profiles/ethereum/values.env` を単一の出所として
  `helpers/` のモジュールが読み取る（テストコード内の重複定義を禁止）
