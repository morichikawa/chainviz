...
- **Engine API は EL（reth）側 = 受け手のメトリクスで観測する**（決定）。
  ...
  （先回りしない）
  - **（Issue #420 で状況更新）** 上記は Engine API（CL→EL）観測に関する
    Phase 5 当初の判断であり、「lighthouse 側のメトリクスは不要」を
    恒久的な結論として扱っているわけではない。Issue #420 で validator
    client（VC）自身の活動（ブロック提案・attestation）を可視化する要望が
    生まれ、VC 側の Prometheus メトリクス（既定 5064 番ポート）を新規に
    有効化する判断に至った（§7.6.12）。両者は別々の駆動ペア（CL→EL /
    validator→CL）を観測する独立した決定であり、本項の Engine API の
    観測方法（EL 側で観測する）自体は変更していない
...
- EL ノードの InfraPopover に 1 行追加: ラベル「txpool」（GlossaryTerm:
  ...
  省略されていれば行ごと出さない
- **カード面にも常時表示する（Issue #408 で変更）**。当初（本節初稿時点）は
  「カード面には出さない。数字2つが常時貼り付いてもチェーンが動いている
  限りほぼ0〜数件で変化に乏しく、ノイズになる」という判断だったが、
  Issue #408 の UX 再検証で、ノードカード本体（常時見える面）に mempool の
  手がかりが一切無く、ホバーして InfraPopover を開かない限り「このノードが
  mempool を持つこと」自体に気づけないという問題が実際に確認された。
  値の変化に乏しいという当初の懸念よりも、「各ノードが個別に mempool を
  持つ」という Ethereum の基本構造が伝わらないことの方が学習用途では
  問題が大きいと判断し、カード面での常時表示（pending 0 件のときも表示）に
  方針を変更した。既存の `infra-card__badge--bootnode` と同格の小さな
  バッジとして `InfraNodeCard` のサブタイトル直下に追加する
  （`packages/frontend/src/entities/InfraNodeCard.tsx`）。ラベル・値の
  文言・GlossaryTerm 連携は InfraPopover の txpool 行と同じものを再利用する
...
2. **パルスは 1 観測 1 本**（7.6.4。メソッド別に分けない。内訳はエッジ
   ホバーで見せる）
3. **カード常設はバックフィル進行の 1 行のみ**（7.6.5。同期ステージ一覧は
   ポップオーバーのみ。txpool 内訳は当初ポップオーバーのみだったが、
   Issue #408 でカード面にも常時表示するバッジを追加した。7.6.6 参照）
...
- **活動パルスは流さない**（本節初稿時点の判断）。VC→beacon の呼び出し
  活動を観測する経路が無い（§7.3。collector は validator 起点の
  `nodeLinkActivity` を配信しない）ため、エッジは常設リンクのみ。
  ポップオーバーの「直近の呼び出し」セクションも出さない（実際には VC は
  スロットごとに beacon を呼んでいるのに、観測していないだけで「最近の
  呼び出しはありません」を常時出すと「呼び出しが無い」という誤った事実を
  伝えてしまうため、セクションごと隠す）
  **その後の状況（Issue #420）**: 「バリデーターの動き自体をもっと
  可視化したい」というユーザー要望を受け、VC 自身の Prometheus メトリクス
  （`--metrics`、既定 5064 番ポート）を新規に有効化してスクレイプする
  方針（§7.6.12）に転換し、上記の制約を解消した。validator→beacon にも
  活動パルスを流し、ポップオーバーの「直近の呼び出し」セクションも
  表示する（下表・§7.6.12 を参照。下記の「役割の組」表・InfraPopover の
  行は現行の内容に更新済み）
- **文言は役割の組で切り替える**: エッジポップオーバーの見出し・説明・
  ...
  | validator → consensus | 内部リンク（Beacon API） | `beacon-api`（新設） | ~~表示しない~~ → **表示する**（Issue #420、§7.6.12） |
...
- **用語解説**: `glossary/ethereum/terms/d-internal.yaml` に `beacon-api` を
  ...
  **chainviz では**: validator カードから beacon カードへ引かれる内部
  ...
  対象
  - **（Issue #420 で文言更新）** 上記の「chainviz では」の記述（呼び出しの
    観測が無くパルスが流れない旨）は本節初稿時点のものであり、§7.6.12 の
    とおり VC メトリクスの観測経路が新設されたことで成立しなくなった。
    `glossary/ethereum/terms/d-internal.yaml` の `beacon-api` 定義文
    （ja/en 双方）から「呼び出しの観測経路が無いためパルスは流れない」
    という一文を削除し、パルスが流れる旨（§7.6.12 の文言）に更新すること

#### 7.6.12 validator 自身の活動パルス（Issue #420）

「バリデーターの動き自体をもっと可視化したい」というユーザー要望への対応。
§7.6.11（Issue #285）は validator→beacon の常設リンクだけを描き、活動パルスは
「観測経路が無い」ことを理由に意図的に流さなかった。本節はその制約を解消する
設計で、**設計担当（designer）が4案（Beacon API相関／VCメトリクス新規スクレイプ／
両者併用／VCログのtail解析）を比較し、ユーザーが「案B（VCメトリクス新規
スクレイプ）」を選択した**（比較の詳細・実測ログは `docs/worklog/issue-420.md`）。

**ユーザーとの前提合意**: 「バリデーター内部で何をやっているか」への期待値は、
以下の限界とすり合わせ済み。案B（他の案でも同様）で実現できるのは
①職務の種類（提案／証明）という大分類、②その職務が実施されたかどうかの確認、
③（メトリクスに含まれれば）処理所要時間の代表値、の3点までであり、
「対象決定 → ブロック構築 → 署名 → 送信」のようなリアルタイムのステップ単位の
内部処理描写はいずれの案でも実現できない（外部観測から復元するには lighthouse
自体のソースコード計装や構造化ログ解析が要り、§7.1「データ源は Prometheus
メトリクスのみ」の決定と衝突するため不採用）。

**採用案の要旨**: reth の Engine API 観測（§7.2、Issue #184/#185）と対称的な
構造で、lighthouse VC 自身の Prometheus メトリクス（`--metrics`、既定
ポート 5064）を新規に有効化してスクレイプする。VC は「自分が鍵を管理する
validator の活動」だけを自分のメトリクスとして公開するため、reth の
Engine API 観測が抱えなかった新しい対応付け問題は生まれない（下記
「validator_index → VC コンテナの対応付け問題」参照）。

**node-env の変更**:

- `profiles/ethereum/scripts/lighthouse-vc.sh` の `exec lighthouse vc \` に
  `--metrics --metrics-address 0.0.0.0 --metrics-port 5064` を追加する
  （既定の `--metrics-address` は `127.0.0.1` で、コンテナ IP 経由で到達する
  collector からは届かないため `0.0.0.0` への上書きが必須。`reth-node.sh` の
  `--metrics 0.0.0.0:9001` と同じ理由）
- **`docker-compose.yml` の変更は不要**。`ports:` へのホスト公開は行わない
  （reth の 9001 番と同じ扱い。collector はホスト上のプロセスから Docker
  ブリッジネットワーク上のコンテナ IP へ直接到達できるため、ホスト公開は
  Beacon API の 5052 番のように「ホストから手動で curl したい」利便性目的の
  場合のみ必要であり、collector の到達性には無関係。§7.2 の reth の実装
  前例のとおり）。validator1/validator2 サービス定義には現状 `ports:` が
  無く、今回もそのままでよい

**validator_index → VC コンテナの対応付け問題（調査時点で未解決だった論点）
の決定**: **対応付けは不要、と結論づけた**。調査時点（案の比較段階）では
「案A（Beacon API 相関）」を採った場合にのみ必要になる問題として書かれていた
（Beacon API の `proposer_index`/`liveness` はチェーン全体の validator_index
空間で答えが返るため、これを `generate-genesis.sh` の連続range分割
（node0=0..31, node1=32..63）で「どの VC コンテナの管轄か」に変換する処理が
要る）。採用した案Bでは、collector は各 VC コンテナ自身の `/metrics` を
個別にスクレイプする（reth の `executionMetricsTargets`/`beaconTargets` と
同じ「対象コンテナへ直接 GET する」構造）ため、**validator_index を扱う場面
自体が発生しない**。VC の Prometheus カウンタ（`vc_signed_beacon_blocks_total`
等。下記参照）は実測（ソース確認、後述）で validator_index やバリデーター
公開鍵のラベルを一切持たない、VC プロセス単位の集計値であることを確認して
おり、これが「対応付け不要」という結論を裏付ける。したがって
`generate-genesis.sh` の range 分割ロジックや環境変数経由の対応付けを
collector に新規に持たせる変更は行わない（先回りしない）。

**collector の変更（`packages/collector/src/adapters/ethereum/`）**:

reth の D層実装（`reth-metrics-client.ts` / `reth-metrics.ts` /
`reth-metrics-tracker.ts` / `reth-node-internals.ts`、Issue #184/#185）と
対称的な構造を踏襲できることを確認した。新設するファイル:

- `vc-metrics-client.ts`: `VALIDATOR_METRICS_PORT = 5064` の定数と、
  `VcMetricsClient`（`getText(url): Promise<string>`）インターフェース +
  `createFetchVcMetricsClient(timeoutMs = 3000)` ファクトリ。実装は
  `reth-metrics-client.ts` とほぼ同一になる想定だが、既存コードの
  「IO境界ごとに専用インターフェースを切る」流儀（`eth-rpc-client.ts` /
  `eth-ws-client.ts` / `reth-metrics-client.ts` 冒頭コメント参照）に
  そろえ、reth 用と共用せず別インターフェースとして新設することを推奨する
  （多少の重複より、対象ごとに独立した DI・テストの差し替え口を保つ
  ことを優先する、という既存方針の踏襲。この点は実装コストとのトレード
  オフであり、最終判断は collector 担当の裁量とする）
- `vc-metrics.ts`: `prom-text-parser.ts`（reth 側と共用。チェーン非依存の
  汎用パーサーであり変更不要）を使い、VC 固有のメトリクス名を解釈する
  純粋関数。下記「実装時に確定したメトリクス名」のとおり、職務種別
  （提案/証明）ごとのカウンタと status ラベルから
  `RawValidatorDutyCounter { method: string; count: number; sumSeconds?:
  number }` を構築する（`method` は Prometheus のメトリクスファミリー名と
  status ラベル値を `:` で連結した生の識別子。例:
  `"vc_signed_beacon_blocks_total:success"`。reth の
  `RawEngineCallCounter.method` が生の JSON-RPC メソッド名をそのまま
  持つのと同じ「生の識別子をそのまま持ち、解釈はフロントが担う」流儀）
- `vc-metrics-tracker.ts`: `VcMetricsTracker` クラス。ノード（VC）id ×
  method ごとに前回の累積値を保持し、増分を計算する（`RethMetricsTracker`
  と同一のアルゴリズム: 初回観測はベースラインのみ記録、カウンタリセット
  検知、増分ゼロは出力しない）。**設計判断**: このアルゴリズム自体は reth
  固有の語彙を一切含まない汎用の「Prometheus カウンタの差分計算」であり、
  `RethMetricsTracker` をリネーム・汎化した共通クラス（例:
  `PrometheusCounterTracker`）を reth・VC 両方から使う実装でもよい。
  ほぼ同一のロジックを持つ2ファイルを個別にメンテする重複コストと、
  IO境界と同様に「対象ごとに独立させる」一貫性のどちらを取るかは
  collector 担当の裁量とする（データフロー・スキーマに影響しないため
  designer としてはどちらでも問題ない判断)
- `vc-node-internals.ts`: `pollVcNodeInternals(client, target, tracker):
  Promise<InternalCallStats[] | undefined>`。`reth-node-internals.ts` の
  `pollRethNodeInternals` と同型（取得・パース失敗時は stableId と実際の
  エラーを `console.error` に残し `undefined` を返す）だが、**`internals`
  （`NodeInternals` パッチ）は返さない**。VC の同期状態・mempool 相当の
  内部状態表示は今回のスコープに含めない（`NodeEntity.internals` の新規
  フィールドは追加しない。後述「今回のスコープ外」参照）

`targets.ts` への追加:

- `ValidatorMetricsTarget { stableId: string; metricsUrl: string }` と
  `validatorMetricsTargets(observations)`。選別基準は既存の
  `isValidatorService(obs)`（`com.chainviz.role === "validator"`、Issue
  #246）+ `obs.ip` が取れること。`executionMetricsTargets` と同型

`index.ts`（`EthereumAdapter`）への配線:

- 既存の `subscribeNodeInternals`/`pollNodeInternalsOnce` の同じ周期ループに
  相乗りさせる（Issue #274 で beacon 側の同期状態観測を同じループに
  相乗りさせたのと同じ設計判断。新しいループ・新しい定数は増やさない）。
  `validatorMetricsTargets(observations)` を毎 tick 取り直し、
  `trackedNodeInternalsIds`/`trackedBeaconSyncIds` と同様に
  `trackedValidatorInternalsIds` で観測から消えた VC の
  `VcMetricsTracker.forgetNode()` を呼ぶ
- 新設する `pollOneValidatorInternals(target, observations, obsByStableId,
  handlers)`: `pollVcNodeInternals` を呼び、`calls.length === 0` なら何もしない。
  1件以上あれば `NodeLinkActivity` を組み立てて `handlers.onLinkActivity`
  へ渡す:
  - `fromNodeId = target.stableId`（VC 自身。`NodeLinkActivity.fromNodeId`
    は「駆動する側（`drivesNodeId` を持つ側）」であり、validator が
    `drivesNodeId` を持つ側なので validator 自身が `fromNodeId` になる。
    reth の実装で `fromNodeId` を `beaconStableIdForExecution()` で
    “逆引き” していたのは、reth 側からスクレイプしているのに `fromNodeId`
    は駆動する側＝beacon にする必要があったため。VC 側は自分自身の
    メトリクスをスクレイプしており `fromNodeId` = 自分自身なので逆引きは
    不要という違いがある）
  - `toNodeId`: 既存の `beaconStableIdForValidator(validatorObs,
    observations)`（Issue #285 で実装済み。新規実装不要）で解決する。
    解決できない場合は reth の実装と同じく配信せず、その旨をログに残す
    （黙って握りつぶさない。CLAUDE.md）
  - `observedAt = this.now()`

**実装時に確定したメトリクス名（暫定。実装時に実機で再確認すること）**:
lighthouse 公式リポジトリ（`sigp/lighthouse`、`stable` ブランチ）の
`validator_client/validator_metrics/src/lib.rs` /
`validator_client/lighthouse_validator_store/src/lib.rs` /
`validator_client/validator_services/src/attestation_service.rs` を
ソースレベルで確認した（実行中の validator を複製起動する実験は
二重署名リスクを避けるため行わず、公開ソースの確認で代替した。reth の
とき（Issue #185）と異なりドキュメント調査に留める運用にした理由は
`docs/worklog/issue-420.md` 参照）:

- **ブロック提案**: `vc_signed_beacon_blocks_total{status="..."}`
  （`IntCounterVec`）。VC がブロックへの署名を試みるたびに増える。
  `status` ラベルの値は `success` / `slashable`（二重署名保護が拒否）/
  `same_data`（同一データへの再署名） / `unregistered`（管理外の
  validator）のいずれか。**ラベルに validator_index・公開鍵は含まれない**
  （VC プロセス単位の集計。上記「対応付け不要」の根拠）
- **証明（attestation）**: `vc_signed_attestations_total{status="..."}`
  （同上、`status` ラベルの値も同じ4種）
- **所要時間（代表値、取れれば）**:
  - ブロック提案: `vc_block_signing_times_seconds`（ラベル無しの
    Histogram。`_sum`/`_count` から区間平均を出す。reth の summary 型
    カウンタと同じ「`_sum`/`_count` を読む」パース手法がそのまま使える。
    TYPE 判定は reth の "summary" 限定ではなく "histogram" も許容する
    よう緩めること）
  - 証明: ブロック提案と対称な単一の署名時間ヒストグラムは無い
    （`vc_signing_times_seconds{type=...}` は signer backend
    （`local_keystore`/`web3signer`）別の集計で、職務種別では分かれて
    いない）。最も近い代替は
    `vc_attestation_service_task_times_seconds{task="attestations_http_post"}`
    （証明の提出 HTTP 呼び出しの所要時間。署名処理そのものではなく
    提出全体の時間である点に注意）。latency が取れない/近似精度が
    低いと判断される場合は `latencyMs` を省略してよい
    （`InternalCallStats.latencyMs` は元々 optional で「観測できた場合
    のみ」の契約）
- **今回のスコープに含めない候補**（先回りしない。将来 Issue で必要になれば
  同じ仕組みで拡張できる）: `vc_signed_aggregates_total`（アグリゲーター
  職務）、`vc_signed_sync_committee_*_total`（sync committee 職務）、
  `vc_validators_enabled_count`/`vc_validators_total_count`（このVCが
  管理する validator 数。`NodeEntity.internals` への追加候補だが今回は
  ユーザー要望のスコープ外）
- 実装時は reth のとき（`docs/worklog/issue-185.md`）と同じ方法論、
  すなわち実機の `docker exec <validator コンテナ> lighthouse vc --version`
  等でイメージのバージョンを確認したうえで、**安全な方法**（例:
  一時的に `profiles/ethereum` を一度 `docker compose down` してから
  `--metrics` を有効化した状態で `up` し直す。既存の稼働環境を複製
  しての並行起動はしない）で実測し、上記の名前・ラベル・型が実際の
  `/metrics` 出力と一致するかを確認してから確定させること

**フロントエンドの変更（`packages/frontend/src/`）**:

- `chain-profiles/ethereum/internalLinkKinds.ts`: `VALIDATOR_TO_CONSENSUS`
  の `showsActivity` を `false` → `true` に変更する（他のフィールドは
  変更不要）
- 新設 `chain-profiles/ethereum/validatorApiMethodLabels.ts`:
  `nodeInternals.ts` の `ENGINE_API_METHOD_LABELS`/
  `describeEngineApiMethod` と同型の前方一致マッピング（1ファイル1責務の
  ため別ファイルを推奨）。初稿:

  | 前方一致 | ja | en |
  | --- | --- | --- |
  | `vc_signed_beacon_blocks_total` | ブロック提案の署名 | Sign proposed block |
  | `vc_signed_attestations_total` | 証明（attestation）の署名 | Sign attestation |

  一致しないメソッドは生名のみで表示する（`describeEngineApiMethod` と
  同じフォールバック）
- `entities/internalLinkActivity.ts` の `formatInternalCallEntry` は現状
  `describeEngineApiMethod` だけを呼んでいるため、`method` の前方一致で
  reth 用テーブルと validator 用テーブルの両方を試す（例:
  `describeEngineApiMethod(method) ?? describeValidatorApiMethod(method)`）
  よう更新する。両テーブルの前方一致プレフィックス（`engine_*` /
  `vc_signed_*`）は名前空間が重ならないため、単純な2段フォールバックで
  誤マッチの心配はない
- `internalEdge.recentCalls` / `internalEdge.noRecentCalls` /
  `internalEdge.latency`（既存 i18n キー）はメソッド非依存の汎用文言の
  ため**新規 i18n キーは不要**。`InternalLinkEdgePopover` /
  `internalLinkEdgesToFlowEdges` 等の共通コンポーネント・データ変換も
  `drivesNodeId` を機械的に扱う既存実装のまま無変更で動く
  （§7.6.11 で既にそのように設計済み）
- `glossary/ethereum/terms/d-internal.yaml` の `beacon-api` 定義文
  （ja/en）を更新する（上記 §7.6.11 の当該箇所を参照。「呼び出しの
  観測経路が無いためパルスは流れない」を削除し、パルスが流れる旨に
  書き換える）

**作業分担・Issue の粒度**: 対象範囲は node-env（スクリプト1行程度の変更・
compose 変更なし）・collector（reth の前例を踏襲する新規ファイル4つ +
`targets.ts`/`index.ts` への配線）・frontend（`showsActivity` の1行変更 +
ラベルテーブル新設 + フォーマッタの2段フォールバック化 + glossary 文言更新）
であり、**1つの Issue（#420）のまま実装しきれる粒度**と判断する
（Issue #412 のように独立して価値を持つ複数機能へ分割する必要はない。
本Issueは「validatorの活動パルスを流す」という単一の user-facing な成果に
向けた3パッケージ分担であり、Issue #285・#187・#274 と同種の「1Issue・
複数パッケージ」の形）。依存関係:

- node-env の変更は他パッケージから独立して着手・マージできる（先に
  終わっていないと着手できない関係はない）。ただし **collector の
  実環境での動作確認（E2E・QA）には node-env の変更が先に反映されている
  必要がある**（`--metrics` が無効なままだと `vc-metrics-client.ts` は
  接続不能になる）
- collector の実装（parse/tracker/wiring）はテスト用のモック
  `VcMetricsClient`（reth 側 `reth-node-internals.test.ts` 等が
  `RethMetricsClient` をモックしているのと同型）で単体テスト可能なため、
  node-env の変更を待たずに並行着手できる
- frontend の変更（`showsActivity` 反転・ラベルテーブル・フォーマッタ）は
  `NodeLinkActivity`/`InternalCallStats` の型そのものは既存のままであり、
  モックデータ（`method: "vc_signed_beacon_blocks_total:success"` 等の
  文字列）で単体テスト・Storybook的な確認が可能なため、collector の
  実装を待たずに並行着手できる
- 3パッケージとも Issue #408 と同じ運用（同一 Issue 番号のブランチ
  `issue-420-validator-activity-visualization` を軸に、並行作業する
  パッケージごとに `-collector`/`-frontend` サフィックスの作業ブランチを
  切って cherry-pick で合流）を推奨する。E2E での最終確認（QA）は
  3パッケージすべてがマージされた状態で行う
...
- タイルのポップオーバーは、他カード（ウォレット/コントラクト/インフラ
  ノード等）のポップオーバーと同じく、ホバー対象要素（タイル div）の
  React ツリー上の子として描画する（`{hovered && <Popover />}` をタイル
  div の内側に置く）。`PopoverPortal` は実 DOM を `document.body` 直下へ
  portal するが、React はイベント合成をこの JSX 上の親子関係で行うため、
  こう並べることでポップオーバーへのホバーがタイルへのホバーの延長として
  扱われる（隙間を越えてポップオーバーへ移動しても閉じない）。以前は
  Fragment で兄弟として描いており、この恩恵を受けられずタイルを離れた
  直後に閉じてしまっていた（Issue #351。docs/worklog/issue-351.md）。
  加えて、表示窓の凍結条件（上記）にはタイルのポップオーバー開閉状態も
  含める必要がある（タイル div の mouseleave 自体は隙間へ出た瞬間に
  即座に発火するため、それだけを見ると凍結が一瞬外れる）
...
### 10.6 ホバー連動ハイライトの E2E 検証戦略（Issue #388）

UI-B-06（`packages/e2e/src/ui/chain-ribbon.spec.ts`）は「実送金 → ブロック
取り込み待ち → ホバー」を直列に行うため、待機中にチェーンが進行して対象
ブロックが表示窓（直近 8 タイル）から流れ出る競合と本質的に隣り合わせに
なる（表示窓の凍結はホバー開始後にしか効かない）。この競合を構造的に
避けるためのテスト戦略:

- **tx チップに `data-block-hash` を計装する**: ウォレットカードの tx チップ
  （`WalletCard.tsx` の `TxChip`）に `data-block-hash={tx.blockHash}` を
  出す（pending 中は `blockHash` が undefined なので属性ごと出ない）。
  `data-parent-hash` / `data-connected-to-previous` と同じ「表示テキスト
  （短縮 hash）からは逆引きできない完全な hash を e2e/テスト専用に露出する」
  用途。テストはこれで「送金した tx がどのブロックに入ったか」を
  ホバーなしで即座に特定できる
- **識別ベースの検証にする**: 従来の「チップにホバーして強調された
  タイルを探し、その testid を読んで正方向の対象を決める」という
  逆引きの往復をやめ、`chain-ribbon-tile-<blockHash>` を直接特定して
  「対象タイルが表示窓内に見えること」を明示的に待ってからホバーする
  （UI-B-05 が #351 で `data-parent-hash` による識別ベース検証へ移行した
  のと同じ教訓）
- **ホバーは実マウス座標を使わない**: チップ・タイルへのホバーは
  `dispatchHover`（`mouseover` の直接 dispatch）、解除は対になる
  `dispatchUnhover`（`mouseout` の直接 dispatch）で行う。実マウス座標に
  紐づくホバーは、負荷時の頻繁な再描画で要素がポインタの下から動くと
  `mouseleave` が発火して強調が落ちる（UI-B-06 併走 flaky の一因）。
  合成イベントによるホバー状態はポインタ位置に依存しないため、再描画では
  落ちない。ビューポート内へ収めるための Fit View クリックもホバーの
  経路からは不要になる（React Flow のキャンバスに
  `onlyRenderVisibleElements` は設定しておらず、ビューポート外のノードも
  DOM に描画される前提）
- **タイミング前提**（固定値ルール）: included チップが観測できた時点で、
  対象ブロックはリボンの最新タイルとして届いている（collector は block の
  entityAdded を対応する tx の included 更新より先に配信する。§10.4 の
  観測順序）。以後タイルが窓から流れ出るまで約 7 スロットの猶予があり
  （最小構成 slot=2 秒でも約 14 秒）、識別ベース + dispatch 化した
  ホバー開始までの操作は数秒で完了する。表示件数 8 を減らす変更をする
  ときはこの前提を見直すこと

なお UI-B-05 は #351 の要件（実マウス軌跡でのポップオーバー維持検証）を
担うため、意図的に実マウス操作のまま残す。
...
- 行クリックで from のウォレットカード（ノード id = アドレス）へパンする
  ...
  存在しない tx の行はパンしない
- **ノード別実数の行もクリック可能にする（Issue #408 で追加）**。クリック
  すると対応するノードカード（`InfraNodeCard`）へキャンバスがパンし、
  `ContractListPanel` の行クリックと同じく対象カードを一時的に強調表示する
  （`jumpHighlightNodeId` の対象型を `InfraNodeCard` の React Flow ノード
  type にも拡張。強調は一定時間で解除される）。ノード別行は
  `buildMempoolNodeEntries` が既存の `rfNodes`（キャンバス上に実在する
  ノード）から直接組み立てるため、tx 行のように「対応カードが存在せず
  パンできない」ケースは発生しない
...
- リサイズした幅は kind（contractSource / glossary / commsLog）に
  ...
  `chainviz.lang` と同系のパターン）。読み込み時は非数などの壊れた値は
  既定値へフォールバックし、範囲外の値は現在の [最小, 最大] 範囲へ
  クランプする。保存失敗は握りつぶしてログのみ残す（`layoutStore` と
  同じ防御的パターン）
- **本文の文字サイズもユーザーが変更できる**（Issue #377）。ヘッダーの
  タイトルと閉じるボタンの間に「A− / 現在値 / A+」のステッパーを置き、
  幅と同様 kind によらない共通のスカラー1値として
  localStorage キー `chainviz.sidePanel.fontScale.v1` に永続化する
  （`side-panel/sidePanelFontScale.ts`）。選べる倍率は 0.85 / 1.0 /
  1.15 / 1.3 / 1.5 の5段階（既定 1.0）で、端では該当ボタンを disabled
  にする。中央の現在値表示（例: `100%`）はクリックで既定倍率へ戻す
  リセットボタンを兼ねる。倍率はルート要素の CSS カスタムプロパティ
  `--side-panel-font-scale` として渡し、`side-panel__body` 配下の本文
  セレクタ側で `font-size: calc(Npx * var(--side-panel-font-scale, 1))`
  に変換する（ヘッダーのクロームは拡大対象外）
...
`catalog.json` のエントリは `token` の代わりに **`nft: { symbol: "CVNDEMO" }`** を
...
- **ウォレットカード**（保有の視点）: トークン残高チップ列の下に
  ...
  `ownerAddress` 照合（大文字小文字無視）で導出し、「CVNDEMO #1」形式の
...
## 15. 「ハッシュのしくみ」デモ（改ざん体験の砂場。Issue #401）

「ブロックのハッシュがどう計算され、なぜ改ざんが検知されるのか」を実際に
手を動かして理解できる、実チェーンから完全に独立した学習用の砂場。UX設計の
全文（動機・操作フロー・文言案）は `docs/worklog/issue-401.md` を参照。

### 15.1 データフロー: 実チェーン・`packages/shared` への依存なし

- サイドパネルの新しい kind `"hashChainDemo"`（`side-panel/sidePanelView.ts`）
  として実装する（§12.2 の汎用サイドパネル機構に相乗り）。対象を指す
  データを持たない点は `commsLog` と同じで、`SidePanelHost` のダングリング
  ガード（§12.2）の対象外
- 中身（`crypto-demo/HashChainDemoView.tsx`）は完全にフロント内で閉じた
  疑似データ（`crypto-demo/hashChainDemo.ts`）を扱う。`BlockEntity` 等の
  ワールドステートのエンティティとは一切関係せず、`packages/shared` ・
  collector の変更は無い
- 疑似ブロックは3個（#1〜#3）固定。各ブロックは
  `{ number, storedParentHash, data }` を持ち、ハッシュ自体は state に
  持たず `deriveBlockHash()` で都度導出する（`番号|親ハッシュ(記録値)|
  データ` を UTF-8 連結して keccak256。実ブロックの RLP エンコードは
  再現しない簡略化で、パネル内に注記する）
- keccak256 の計算は監査済み・依存ゼロの `@noble/hashes`
  （`crypto-demo/keccak256.ts` の薄いラッパー経由）で実際に行う。記号的な
  図解ではなく本物の計算をすることが、この体験の核（「1文字変えると別の
  値になる」雪崩効果を実際に確かめられる）
- パネルは開くたびに `createInitialHashChainDemoState()` から始まる（閉じたら
  state を破棄。学習デモは毎回同じ起点が明快という判断）

### 15.2 連鎖修復の仕組み: ハッシュはブロック自身のフィールドだけで決まる

- ブロックの `data` を編集すると、その**直後の1ブロックだけ**が無効になる
  （`isBlockValid`: 自身の `storedParentHash` が直前ブロックの現在の導出
  ハッシュと一致するか）。編集した本人より2つ先のブロックは、直後の
  ブロックが実際にハッシュを変えるまでは無効にならない
- 「親ハッシュをつなぎ直す」（`relinkBlock`）は対象ブロックの
  `storedParentHash` を直前ブロックの現在のハッシュへ書き換える。これで
  対象ブロックは有効に戻るが、`storedParentHash` が変わったことで対象
  ブロック自身の導出ハッシュも変わるため、**次のブロックがまだ無効の
  まま**残る。1回の relink ごとに「無効の先頭」が1つずつ後ろへ進む
  （改ざんを隠すには後続をすべて作り直す必要がある、という連鎖修復の
  体験）
- 3ブロックすべてが有効な状態に戻ると、「1台のマシンの中では辻褄を
  合わせられるが、実際のネットワークでは他ノードが元のチェーンを持ち、
  各ブロックには提案者の署名と検証（attestation）が必要」というまとめを
  表示する（Issue #402 署名・検証デモへの概念的な橋渡し。機構的なリンクは
  張らない）
- 「全部つなぎ直せた」まとめメッセージの表示条件は「一度でも編集/relink
  操作をした後に全ブロックが有効」（View 側のローカル state。pure logic
  側の `isFullyRepaired` だけでは初期状態＝無操作でも真になるため区別できない）

### 15.3 導線

- チェーンリボンカード（§10）の subtitle 行の行末に常設の入口ボタン
  （`chain-ribbon-hash-demo-open`）を1つ置く（タイルが0件でも常に表示される）
- タイルホバーのポップオーバー（`ChainRibbonPopover`）末尾にも文脈導線の
  ボタンを1つ置く
- どちらも `useOptionalSidePanel()` を使う（`GlossaryTerm` と同じパターン。
  `SidePanelProvider` の外でレンダーされる既存の単体テストを壊さないため）
- 用語集 `hash` エントリ（`glossary/ethereum/terms/c-transaction.yaml`。
  §5）の定義文中でこの砂場に言及する。用語集からパネルへの機構的な
  ジャンプは作らない（既存のパネル間ジャンプ導線に無い経路のため、今回は
  文章での案内のみ）
- `ChainRibbonPopover` の「ハッシュ」「親ブロック」ラベルに
  `GlossaryTerm termKey="hash"` を付ける
- keccak256 という規格自体の説明は、`hash`（ハッシュという一般概念）とは
  別に独立した用語集エントリ `keccak256` を新設する（同じく
  `c-transaction.yaml`。出力長・SHA-3 との違い・イーサリアムでの用途を
  扱う。Issue #406）。処理帯のアルゴリズム名の行（`hashDemo.compute`）に
  `withTermAnchor` でアンカーを付ける

### 15.4 共通骨格（Issue #402 との共有の想定）

`crypto-demo/` フォルダに、暗号デモ共通のパターン（編集できる入力 →
実アルゴリズム名を明示した処理帯 → 導出値 → 検証バッジ → 改変の影響が
伝播する様子 → リセット → 「実際のネットワークでは誰がやるか」の説明）を
体現する部品を置く。Issue #402（署名・検証の可視化）は同じフォルダに
並べる想定だが、今回はハッシュデモが実際に使う部品だけを作る（共通化の
ための推測的な props は先に作らない）。サイドパネルの kind もデモごとに
分ける（`"hashChainDemo"` / `"signatureDemo"` 等）。

処理帯は「実アルゴリズム名の行」（用語集アンカー付き）と「その入力 `x`
の中身を明示する行」の2行構成にする（Issue #406）。`f(x)` / `f⁻¹(x)` の
括弧トークンと同様、`x =` も i18n を経由しないハードコードの記号として
扱い、対応する文言キー（`hashDemo.computeInput` /
`sigDemo.computeInput.sign` / `sigDemo.computeInput.verify`）には本文
（項目名の連結）だけを持たせる。x の中身の説明は学習内容そのものであり
補足ではないため常時表示にし、処理帯コンテナ自体は `aria-hidden` に
しない（`f(x)` 等の装飾トークン単体のみ `aria-hidden` を残せる）。

## 16. 「署名と検証のしくみ」デモ（Issue #402）

「何が署名で、何が検証という操作・処理なのか」を実際に本物の署名・
公開鍵復元（ecrecover）を計算しながら理解できる、§15 と同じ流儀の学習用
砂場。UX設計・実装設計の全文は `docs/worklog/issue-402.md` を参照。

### 16.1 スコープ: EOA の tx 署名とその検証（ecrecover）に絞る

Ethereum の「署名」「検証」には複数の意味があるが、このデモは **EOA の
tx 署名と、それを ecrecover で検証する部分**だけをインタラクティブに扱う。
理由: ワークベンチの送金操作は既にこれを毎回行っており、その裏側を開けて
見せる位置づけが最も学習の接続が良く、かつフロント内の砂場で本物の計算
として完結できる（観測できないものを観測したふりをする必要がない）。

以下の2つは説明テキスト＋用語集のみで扱い、インタラクティブには扱わない
（chainviz は validator の投票内容やブロック検証プロセス自体を観測しない
設計のため。§7.6.11 と同じ判断）:

- PoS バリデーターの attestation（`glossary` に新設。§5 参照）
- Engine API 経由の CL→EL ブロック妥当性検証（既存の `engine-api` 用語へ
  接続するのみ）

### 16.2 データフロー: 実チェーン・`packages/shared` への依存なし

- サイドパネルの新しい kind `"signatureDemo"`（`side-panel/sidePanelView.ts`）
  として実装する（§12.2 の汎用サイドパネル機構に相乗り。§15 の
  `"hashChainDemo"` と同型で、対象を指すデータを持たずダングリングガード
  の対象外）
- 中身（`crypto-demo/SignatureDemoView.tsx`）は完全にフロント内で閉じた
  疑似データ（`crypto-demo/signatureDemo.ts`）を扱う。`TransactionEntity`
  等のワールドステートのエンティティとは一切関係せず、`packages/shared`・
  collector の変更は無い（collector が実 tx の `from` を ecrecover で検証
  する案は今回のIssueでは採らず、引き続き RPC 報告の `from` をそのまま
  信頼する）
- 秘密鍵は砂場専用の使い捨て固定鍵（Alice役・攻撃者役の2つ）。ラベル
  文字列（`"chainviz:sigdemo:alice"` 等）を keccak256 した値を秘密鍵として
  使うことで、乱数ではなく決定的に導出された「実ウォレットとは無関係の
  鍵」であることをコード上も明確にする（`crypto-demo/signatureDemo.ts`）
- 署名・検証の計算は監査済みの `@noble/curves`（secp256k1。
  `crypto-demo/secp256k1.ts` の薄いラッパー経由）で実際に行う。署名対象の
  メッセージハッシュは `from|to|amount` を UTF-8 連結して keccak256 する
  簡略化（実際の RLP / EIP-155 エンコードは再現しない。パネル内に注記）
- パネルは開くたびに `createInitialSignatureDemoState()` から始まる
  （§15 と同じ判断。閉じたら state を破棄）

### 16.3 状態モデルと操作フロー

- 状態は「ワークベンチが署名して送った時点の内容と使った鍵」（`sent`）と
  「ノード側で見えている内容」（`received`）の2つを持つ。導出値
  （署名データ・復元アドレス・有効/無効）は state に持たず、都度計算する
- 上ゾーン（ワークベンチ）の編集は常に Alice 自身の操作という前提のため
  `signedBy` を `"alice"` に固定し、`sent`・`received` の両方を同じ内容に
  更新する（＝本人が署名し直して送り直した状態。有効なまま追従する）
- 下ゾーン（ノードに届いた内容）の編集は `received` だけを更新する
  （通信途中の改ざんの想定）。復元アドレスが `from`（常に Alice の
  固定アドレス）と食い違い、無効になる
- 無効な状態でのみ「攻撃者の鍵で署名し直す」「Alice が署名し直す」の
  2つの再署名操作を提示する。前者は署名自体は数学的に正しくなるが
  復元されるのは攻撃者のアドレスで依然無効（なりすまし不成立）、後者は
  有効な状態に戻る。両操作とも `sent = { content: received, signedBy:
  <鍵> }` という同じ形の遷移で表され、個別の特殊分岐は無い

### 16.4 導線

- tx ライフサイクルポップオーバー（§6.11）末尾に文脈導線ボタンを1つ置く
  （「署名」が1段目として見えている、まさにその場）
- 送金フォーム（操作パネルの transfer タブ）下部・送信ボタンの直前にも
  同文言の小リンクを置く（「あなたが今からする送金の裏側」という最も
  文脈の強い導線）
- 用語集 `signature` エントリ（`glossary/ethereum/terms/c-transaction.yaml`）
  の定義文中でこの砂場に言及する。用語集からパネルへの機構的なジャンプは
  作らない（§15 と同じ判断）
- デモ末尾の「ほかの『検証』」説明文の直後に、`attestation` / `engine-api`
  を指す短いラベルチップ（`GlossaryTerm` でラップ）を置き、Issue #124
  「アンカーの無い用語を作らない」を満たす
- keccak256（§15.3 で新設した用語集エントリ）へのアンカーは、アドレス
  導出の注記（`sigDemo.addressNote`）と、署名側・検証側それぞれの処理帯の
  x 行（`sigDemo.computeInput.sign` / `.verify`）の計3箇所に付ける
  （Issue #406）

## 17. 攻撃手法の理解（Issue #412）

「ブロックチェーンに対する一般的な攻撃手法を理解できるようにする」という
要望への答え。実装案の比較・攻撃手法ごとの向き不向きの検討経緯は
`docs/worklog/issue-412.md`・GitHub Issue #412 本文を参照。

### 17.1 全体構成: 「土台」+「3つの独立シミュレーション砂場」

対象とする攻撃手法6種類（51%攻撃・ダブルスペンド・リオーグ・
フロントランニング・ロングレンジ攻撃・eclipse攻撃）すべてに共通の土台
として、案1（グロッサリー解説 + 既存可視化との紐付け）を敷く。そのうえで、
実チェーンでの再現が原理的に困難、または今の開発用ネットワークの規模では
説得力が出ない3手法（51%攻撃・ロングレンジ攻撃・eclipse攻撃）にのみ、
案3（§15/§16 と同型のフロント内独立シミュレーション砂場）を追加する。

リオーグ・ダブルスペンド・フロントランニングは今回は解説のみに留め、
シミュレーション砂場は作らない（別 Issue として後日検討。理由は §17.7）。

- **土台（全6手法共通）**: `packages/shared`・collector・node-env の変更
  なし。glossary への新規エントリ追加と、既存の可視化要素（フォーク色分け
  = §9、チェーンリボン = §10、mempool パネル = §11、tx ライフサイクル =
  §6.11、P2P 凡例 = CONCEPT.md）へのアンカー追加のみ
- **3つの砂場（51%攻撃・ロングレンジ攻撃・eclipse攻撃）**: §15/§16 と
  同じく frontend のみで完結。`packages/shared`・collector・node-env の
  変更なし。サイドパネルの新規 kind 3つと `attack-demo/` という新規
  フォルダを追加する

### 17.2 データフロー: `packages/shared` への型追加は不要

土台・3つの砂場のいずれも、実チェーンの観測結果（ワールドステートの
エンティティ）を新たに必要としない。土台は既存エンティティ・既存 UI への
装飾（用語アンカー・導線ボタン）に留まり、3つの砂場は §15/§16 の
`hashChainDemo`/`signatureDemo` と同型の「フロント内で完結した疑似
データ」のため、`packages/shared` の型変更もcollector側の実装も発生しない。

### 17.3 用語集の拡張（土台）

新規に6語を追加する。ファイルは既存の「A層〜D層」区分にそのまま従い、
新規ファイルは作らない（各語の `layer` フィールドが指す既存ファイルへ
追記する。§5 参照）:

| 用語キー | 配置ファイル | layer | 内容の主眼 |
| --- | --- | --- | --- |
| `fiftyOnePercentAttack` | `b-network.yaml` | `b-network` | 提案・投票権限がバリデーター集合の過半数に偏ると、その勢力が fork choice を支配できてしまうこと |
| `longRangeAttack` | `b-network.yaml` | `b-network` | genesis まで遡って別の履歴を作り直す攻撃と、finality/weak subjectivity がなぜそれを防ぐか |
| `eclipseAttack` | `b-network.yaml` | `b-network` | 特定ノードの接続ピアを攻撃者だけで埋め尽くし、そのノードに偽のネットワーク像を見せる攻撃 |
| `reorg` | `b-network.yaml` | `b-network` | フォーク（既存の `fork` エントリ）が1本の正準チェーンへ収束する現象。悪用されると51%攻撃やロングレンジ攻撃の帰結になりうることに触れる |
| `doubleSpend` | `c-transaction.yaml` | `c-transaction` | 同じ資産を2回使おうとする試みと、PoS Ethereum の finality により確定後は原理的に成立しないこと |
| `frontRunning` | `c-transaction.yaml` | `c-transaction` | mempool 内の未承認 tx を見て自分の tx を先に割り込ませる攻撃（sandwich 攻撃等の変種に触れるが個別エントリは作らない） |

- 定義文は既存エントリと同じ3拍子「定義 → なぜ起きるか/何が問題か →
  chainviz ではどう見えるか（または見えないか）」の構成に揃える（ja/en
  両方）。「chainviz ではどう見えるか」の欄で、土台のみの3手法は
  「既存の可視化要素のどれで観察できるか」を、砂場ありの3手法は
  「独立シミュレーション砂場（§17.5）で体験できる」ことを明記する
- `relatedTerms`: `fiftyOnePercentAttack` ↔ `fork` ↔ `reorg` を相互リンク、
  `eclipseAttack` ↔ `peer`/`p2p`/`discovery`、`doubleSpend` ↔
  `transaction`/`nonce`、`frontRunning` ↔ `mempool`/`transaction`/`gas`
- 「finality」という語自体は今回は独立の用語集エントリを新設しない
  （`doubleSpend`・`longRangeAttack` の説明文中で概念として触れるに留める。
  独立エントリが要るかは実装時の文章量次第で判断してよい。先回りして今は
  決めない）

### 17.4 アンカー配置（土台。Issue #124「アンカーの無い用語を作らない」を満たす）

新設する6語すべてに、glossary パネル以外の場所からのアンカーを最低1箇所
持たせる:

- `fiftyOnePercentAttack`・`reorg`: `InfraPopover.tsx` の既存 `fork`
  アンカー（§9.3。「見ている tip」行）の近傍に追加する。バリデーター数・
  投票の話と結びつくため、`validator`/`attestation`（`a-infra.yaml`）から
  の `relatedTerms` 導線も検討する
- `longRangeAttack`: `ChainRibbonCard.tsx`/`ChainRibbonPopover.tsx`
  （チェーンの連なりを見せるカード。§10）にアンカーを置く
- `eclipseAttack`: `PeerNetworkLegend.tsx`（画面隅の P2P ネットワーク凡例。
  CONCEPT.md「ピア接続」の項）に置く。ノードの接続ピアという主題が
  この凡例と直接対応するため
- `doubleSpend`: `TxLifecyclePopover.tsx`（§6.11。tx 確定までの段階表示）
  に置く。「確定後は覆らない」という説明と tx ライフサイクルの文脈が合う
- `frontRunning`: `MempoolPanel.tsx`（§11。mempool 内訳の俯瞰ビュー）に
  置く。tx 順序への言及と mempool 俯瞰の文脈が合う

### 17.5 サイドパネル新規 kind: 3つの独立シミュレーション砂場

`sidePanelView.ts` の `SidePanelView` 判別共用体に、§15/§16 と同型の
（対象を指すデータを持たない）新規 kind を3つ追加する:
...
- いずれも `useOptionalSidePanel()` 経由で開く（既存2デモと同じパターン）
- 中身は `packages/frontend/src/attack-demo/` という新規フォルダに置く
  （`crypto-demo/` とは別フォルダにする。keccak256/ecrecover のような
  「暗号プリミティブの実計算」ではなく、「バリデーターの投票集計」
  「ピア接続の掌握」のような**ネットワーク・合意レベルの疑似シミュレー
  ション**であり、性質が異なるため名前空間を分ける）
- パネルは開くたびに初期状態から始まる（§15/§16 と同じ判断。閉じたら
  state 破棄）
- 3つに共通する部品（例: 分岐ダイアグラムの描画）を先回りして共通化
  しない。§15.4 の判断（「共通化のための推測的な props は先に作らない」）
  をそのまま踏襲し、2つ目以降の砂場を実装する時点で本当に共通化が要るか
  を都度判断する

#### 17.5.1 51%攻撃（`fiftyOnePercentAttackDemo`）

- **概念モデル**: 少数（例5〜7個程度）の疑似バリデーターが、2つに分岐
  した候補チェーン（枝A/枝B）のどちらかへ投票する。各バリデーターは
  等しい重み1票を持つ疑似データとし、ユーザーは「攻撃者が支配する
  バリデーター数」をスライダー等で増減できる
- **本物の判定ロジックの原則**: §15/§16 が「記号的な図解ではなく本物の
  計算をする」ことを核としたのと同じく、この砂場も「見た目の演出」では
  なく実際に動くロジックを持たせる。具体的には各枝の得票重みを合計し
  「重みが大きい枝が canonical になる」という簡略化された fork choice
  規則（実際の LMD-GHOST の簡略化である旨を注記）を実装し、攻撃者の
  投票数を動かすたびに勝者の枝が実際に再計算されて切り替わる様子を見せる
- **既存可視化との視覚的な結びつき**: 2つの枝の配色は §9.3 のフォーク色
  パレット（CSS 変数）をそのまま流用する。実際のフォーク色分け表現を
  見たことがあるユーザーが「あの色分けの裏にある力学」として直感的に
  対応づけられるようにする
- **図解の形式**: 枝分かれのツリー/ダイアグラム表示（チェーンリボンの
  タイル表現ではなく、分岐そのものを視覚化する新しい図解要素になる
  見込み。詳細なレイアウト・アニメーションは §17.6 で UX 設計へ委ねる）

#### 17.5.2 ロングレンジ攻撃（`longRangeAttackDemo`）

- **概念モデル**: genesis から伸びる「正規のチェーン」と、同じ genesis
  から攻撃者が後から作り直した「別の履歴」を並べて見せる疑似データ
  （各3〜4ブロック程度）。攻撃者側の履歴は一瞬で正規チェーンより長く/
  新しく見せかけられる（過去に遡って書き換えるため、素朴な「一番長い/
  新しい chain を正とする」規則だけでは見分けが付かないことを体験させる）
- **本物の判定ロジックの原則**: 「finality checkpoint」（疑似データ上の
  1点）をユーザーが動かせるようにし、攻撃者の分岐点が checkpoint より
  過去にあるかどうかで採用可否が実際に切り替わる判定ロジックを持たせる
  （固定演出ではなく、checkpoint 位置と分岐点位置を比較する実ロジック）
- **既存可視化との視覚的な結びつき**: ブロックの見た目は §15 の
  `HashChainBlockRow`・§10 チェーンリボンのブロックタイルに準じた表現を
  再利用し、「これは §10 で見ているのと同じブロックの積み重なり」という
  地続き感を持たせる
- **図解の形式**: 2本の水平なブロック列（正規/攻撃者）を上下に並べ、
  finality checkpoint を縦線または錠前アイコンで示す構成が有力（詳細は
  UX設計へ委ねる）

#### 17.5.3 eclipse攻撃（`eclipseAttackDemo`）

- **概念モデル**: 疑似の「被害ノード」1つと、その接続ピアスロット
  （例8個）を持つミニ P2P グラフ。ユーザーが攻撃者ピアを1つずつ疑似
  ネットワークに追加していくと、被害ノードの接続スロットが徐々に攻撃者
  ピアで埋まる
- **本物の判定ロジックの原則**: 攻撃者ピアの占有率がスロット数に対して
  100% に達した時点で「被害ノードが見ているチェーンの見え方」が実際に
  攻撃者側の疑似データへ切り替わる、という実ロジックを持たせる（演出
  ではなく占有率の実計算で分岐する）
- **既存可視化との視覚的な結びつき**: ノード・エッジの見た目はメイン
  キャンバスの P2P エッジの視覚言語（色・線）を踏襲する。攻撃者ピアは
  既存の配色体系と衝突しない専用色で区別する
- **図解の形式**: 中心の被害ノードを取り囲むミニネットワークグラフ
  （詳細は UX 設計へ委ねる）

### 17.6 UX設計への引き継ぎ

土台（§17.3・§17.4）は既存パターンの延長（用語集エントリ追加・既存
ポップオーバーへのアンカー追加）であり、chainviz-ux への引き継ぎは不要と
判断する。

一方、3つの砂場（§17.5）は、§15/§16 が確立した「編集できる入力 → 実
アルゴリズム名を明示した処理帯 → 導出値 → 検証バッジ」という単一の型を
そのまま流用できない。3手法それぞれが「分岐ツリー」「並走する2本の履歴」
「ピアを取り囲むミニグラフ」という**個別の図解**を必要とし、ユーザーからも
「図解を重視してほしい」という明示の要望がある。したがって3つの砂場は
実装着手前に chainviz-ux の UX 設計（操作フロー・図解のレイアウト・
アニメーション・文言）を経ることを推奨する。本設計（chainviz-designer）が
決めるのは「§17.5 に書いた概念モデル・本物の判定ロジックを持たせる原則・
既存可視化要素との視覚的な結びつきの方針」までに留め、具体的な図解の
レイアウト・インタラクションの細部は UX 設計に委ねる。

チェーンリボンカードの subtitle-row（§15.3 で `hashChainDemo` の入口
ボタンを1つ置いた場所）に `fiftyOnePercentAttackDemo`・
`longRangeAttackDemo` の入口ボタンを機械的に追加すると、同じ行にボタンが
3つ並ぶことになる。カードが手狭になる懸念があり、UX設計側で「砂場一覧」
的な単一の入口（メニュー/ドロワー等）へ統合するかどうかを含めて判断して
もらう（本設計では決め打ちしない）。

### 17.7 対象外にした3手法の扱い（決定事項の確認）

リオーグ・ダブルスペンド・フロントランニングは、今回は砂場を作らず
§17.3 の用語解説のみとする。理由（`docs/worklog/issue-412.md` の比較表
より）:

- **リオーグ**: 既に §9（フォーク色分け）・§10（チェーンリボン）で
  「収束する様子」自体は可視化済み。新規の砂場を作らずとも、既存の
  可視化 + 用語解説で「フォークが1本に収束する現象」は説明できる
- **ダブルスペンド**: PoS Ethereum の finality により「確定後のブロックは
  覆らない」設計そのものが学習ポイントであり、「成功する二重支払い」を
  砂場で再現するとチェーンの正しさに反する誤った学習内容になる
- **フロントランニング**: 既存の mempool 可視化（§11）・複数ワークベンチ
  からの競合 tx 送信（既存の `runWorkbenchOperation`）の組み合わせで、
  実チェーン上である程度体感できる見込みがある。ただし「実チェーン上での
  再現」（案2）は本 Issue のスコープ外（今回は案1のみ）とし、案2を採る
  かどうかは別 Issue として後日改めて検討する
## 18. ブロック詳細パネル（保持窓内を遡る。Issue #409）

「チェーンを遡れるようにしたい」という要望への答え。チェーンリボン（§10）は
直近 8 タイルだけを映す「今」を見せるための表現で、それより前のブロックを
...
- **対応する範囲**: collector が現在保持している最大 32 ブロック（slot 12
  秒で約 6.4 分）の中で、任意のブロックの詳細（フル hash・parentHash・
  timestamp・受信ノード一覧・取り込み済み tx 全件）を見られるようにし、
  親子関係をたどって前後に移動できるようにする
- **対応しない範囲（意図的に外す）**:
  - 保持窓より前の、collector が既に忘れたブロックの参照（新規 RPC による
    任意ブロックの遡及取得はしない）
  - ハッシュ・アドレス・ブロック番号によるチェーン全体の検索 UI
    （いわゆるブロックエクスプローラー）
  - セッションリプレイ（CONCEPT.md「環境スナップショットの共有」の
    「発展の発展」として明示的に将来課題のまま残されている機能）
  - 理由: CONCEPT.md はこのアプリを「学習・開発支援のためのライブ可視化
    ツール」と位置づけており、実運用のブロックエクスプローラーを作る
    ものではない。§6.13・§10.4 でも「無限の履歴を遡る機能には拡張しない」
    ことを既に明示的に決定しており、本 Issue もこの方針を踏襲する。任意の
    過去ブロックへの遡及取得は新しいプロトコル（クエリ型コマンド）・
    ChainAdapter の拡張を要する大きな変更であり、必要になった時点で改めて
    別 Issue としてスコープする
- **`BLOCK_RETENTION`（現在 32）は本 Issue では変更しない**。パネル + 前後
  ナビゲーションだけで、遡れる範囲を「直近 8 タイル（約 96 秒）」から
  「保持窓全体（約 6.4 分）」へ広げられるため、まずこの範囲で機能させる。
  実際に使ってみて「もっと遡りたい」となった場合は改めて値の見直しを
  検討する（今使ってもいない深さを先回りで決め打ちしない。CLAUDE.md の
  固定値ルール）

### 18.2 データフロー: shared・collector の変更なし

チェーンリボン（§10）・mempool パネル（§11）と同じ方針。**`packages/shared`
の型変更なし・collector の変更なし**で、既にフロントへ届いている情報の
結合だけで成立する。

- サイドパネルの新しい kind `"blockDetail"`（`side-panel/sidePanelView.ts`）
  を追加する: `{ kind: "blockDetail"; hash: string }`（frontend 内部の型。
  §12.2 の汎用サイドパネル機構に相乗り）
- **開くトリガー**: `ChainRibbonPopover`（タイルホバー時のポップオーバー）に、
  既存の「ハッシュのしくみを見る」ボタン（Issue #401）と同型の
  「ブロック詳細を見る」ボタンを追加し、
  `sidePanel?.open({ kind: "blockDetail", hash: block.hash })` を呼ぶ。
  タイル本体をクリックで開くのではなく、既存のホバーポップオーバーの中に
  ボタンを置く: タイル本体は §10.3 の複雑なホバー凍結・強調ロジック
  （Issue #298/#351）を既に持っており、新しいクリックトリガーをタイル本体に
  足すとそれらと干渉するリスクがある。ポップオーバー内のボタンは Issue
  #401 で既に安全に共存できることが実証済みのパターンなので、これを踏襲する
- **対象ブロック・取り込み済み tx の解決**は `SidePanelHost` の呼び出し元
  （`Canvas.tsx`）が行う（`contractsByAddress` と同じ「rfNodes を filter
  するだけ」の流儀）:
  - `blocksByHash`: `rfNodes` から `type === CHAIN_RIBBON_NODE_TYPE` の
    ノード（チェーン全体で 1 つ）を見つけ、その `data.blocks`
    （保持窓内の `BlockEntity` 全件。既に `ChainRibbonNodeData.blocks` と
    して存在する。§10.1 脚注参照）を hash でインデックス化するだけ。
    `ChainRibbonNodeData` 自体の変更は不要
  - `transactions`: 既に `CanvasProps.transactions` として `App.tsx` から
    渡されている全 `TransactionEntity` 配列（mempool パネルが使っているのと
    同じもの）をそのまま使い、`tx.blockHash === 対象ブロックの hash` で
    絞り込む（`countTransactionsByBlockHash` と同じ絞り込み条件: pending
    除外・included/failed 両方を含む）
  - `SidePanelHost` にこの 2 つを渡す新しい prop（`blocksByHash:
    ReadonlyMap<string, BlockEntity>` と、対象ブロックの tx 一覧を引く
    手段。具体的な関数分割は実装時の判断に委ねる）を追加する
- **ダングリングガード**: 表示中のブロックが保持窓から外れて
  `entityRemoved` された場合（32 ブロックの窓を超えて古いブロックを表示し
  続けている間にチェーンが進行すると起こりうる。§10.2）、`contractSource`
  と同じ方針でパネルを自動的に閉じる。これは意図的な単純化（新しい UI
  要素を増やさない）。閉じる理由をトースト等で説明する追加 UI は、実際に
  QA で「唐突すぎて分かりにくい」と判明した場合に別途検討する

### 18.3 前後ナビゲーション

- **「前のブロック」**: 対象ブロックの `parentHash` を `blocksByHash` から
  引く。見つかれば移動、見つからなければ（保持窓の外、または起動直後で
  観測開始点に達した場合）「前のブロック」ボタンを disabled にし、
  「これより前は保持期間外のため表示できません」という説明を出す
- **「次のブロック」**: `blocksByHash` の全件から `parentHash === 対象
  ブロックの hash` であるブロックを探す。複数見つかった場合（フォーク）は
  §10.1 の `pickCanonicalPerNumber` と同じ tie-break（最新受信時刻が遅い方、
  同時刻なら hash 辞書順）で 1 件に絞る。見つからなければ「次のブロック」を
  disabled にし、対象が現在の最新ブロック（チェーンリボンの `latest` タイル
  と同じ hash）なら「最新のブロックです」、そうでなければ（観測が追い付い
  ていない・非正史のフォーク末端等）汎用的な「次のブロックが見つかりません」
  を出す
...
- ヘッダ: ブロック番号 + 短縮 hash
- フィールド: フル hash・親 hash（クリックで「前のブロック」と同じ移動を
  行える導線にできないか実装時に検討）・タイムスタンプ
  （`formatBlockTimestamp` 再利用）・受信ノード一覧（`deriveReceivedOrder`
  をチェーンリボンポップオーバーと同じ関数で全件表示。ポップオーバーと
  違い省スペースの制約が緩いため、全件をスクロールなしで見せられる）
- tx 一覧: 対象ブロックの `TransactionEntity` 全件を、hash 短縮表記・nonce
  （あれば）・from → to・status・contractCall（あれば関数名 + 引数）を
  1 行で表示する。並びは nonce 昇順（同一ブロック内の送信順序が意味を
  持つ）。行の情報粒度は `WalletPopoverTxItem`（§6.12・Issue #319）に揃える
  （再利用できないか実装時に検討し、新規に書き起こす場合も同じ情報粒度に
  揃える）
- 1 ブロックあたりの tx 表示件数に上限は設けない想定だが、極端に多い
  ブロック（本チェーンプロファイルの規模では想定しにくいが安全のため）に
  備え、mempool パネル（§11.3）と同じ「他 n 件」の間引き表現を実装時の
  オプションとして用意しておく
- tx 行から対象ウォレット/コントラクトカードへジャンプする機能
  （mempool パネルの行クリックと同種）は本 Issue の範囲外とする。複数
  カードへのジャンプ導線を 1 パネルに詰め込むと責務が広がるため。必要に
  なれば別 Issue で追加する

### 18.5 E2E

`packages/e2e/SCENARIOS.md` に B 層の UI シナリオとして追記し、Playwright
テストを実装する（`docs/PLAN.md` の運用ルール「UI に見える機能を実装する
ステップには SCENARIOS.md への追記 + Playwright テストの実装を含める」に
従う）。最低限、(1) ポップオーバーの「ブロック詳細を見る」からパネルが
開き対象ブロックの情報が表示される、(2)「前のブロック」で親ブロックへ
移動できる、(3) 保持期間外（前のブロックが存在しない）でボタンが disabled
になる、の 3 点を検証する。(3) は 32 ブロック分の履歴が貯まるまで待つと
実チェーン（slot 12 秒）で 6 分超かかり E2E として重いが、**collector は
起動（サブスクライブ開始）以前のブロックをそもそも観測していない**ため、
テスト環境を起動した直後の早い段階で「観測している最古のブロック」の
`parentHash` はどのみち `blocksByHash` に存在せず、そこで自然に境界へ
到達できる（32 ブロック分待つ必要はない）。この前提が成立しない構成
（collector 起動を長時間経てからテストする等）に変える場合は見直すこと。
...
- `collector` 内の状態ストア実装（インメモリのみで十分か、再起動時の
  ...
  - 確定（Issue #369）: この compose project 名（既定 `chainviz-ethereum`。
    `profiles/ethereum/docker-compose.yml` のトップレベル `name:` に対応）は
    環境変数 **`CHAINVIZ_COMPOSE_PROJECT`** で上書きできる。同じプロファイルを
    `docker compose -p <別名>` で別プロジェクト名として起動した合成環境
    （QA 検証環境等）を、collector の観測・操作（addNode/addWorkbench/
    runWorkbenchOperation 等）の対象にするための上書き口。解決は collector の
    他の実行時設定と同じく `index.ts` の `resolve*` 関数群
    （`resolveComposeProject()`。未設定・空文字なら既定値）で行い、
    `EthereumNodeLifecycle` の config へ渡す。lifecycle が参照する
    ネットワーク名・ボリューム名（`<project>_chain` / `<project>_genesis` /
    `<project>_clpeer` / `<project>_elpeer`）は Compose の命名慣習に従い
    compose project 名から導出する（前提: compose ファイル側で network /
    volume に固定の `name:` を付けないこと。現状の
    `profiles/ethereum/docker-compose.yml` はこの前提を満たす）。Docker
    Compose CLI 自身が解釈する `COMPOSE_PROJECT_NAME` は流用しない
    （collector と同じシェル環境で compose 操作をしたときに両方へ効いて
    しまう事故を避け、collector 専用の上書き口として分離するため）。
