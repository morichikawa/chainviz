...
| #377 | Issue #377 用語集パネルのフォントサイズを変更できるようにする件の起票と`docs/PLAN.md`バックログ追記のレビュー(合格。ユーザーからの要望。Issue本文とPLAN.md追記が過不足なく一致、参照事実(用語集パネル実装済み(#313)、layout/layoutStore.ts、SidePanel.tsx/SidePanelHost.tsxの共通シェル構成、類似要望#362がOPEN)の実在、フォーマット一貫性、コミット粒度、lint/build/test全通過を確認。実装着手は後日、UI形状・永続化要否・適用範囲は着手時に設計判断)、およびUX設計(ux。実機確認とスケール適用プレビューに基づき3論点を確定: ヘッダーに「A− / 100% / A+」ステッパー(中央の現在値表示がリセット兼用)・localStorage `chainviz.sidePanel.fontScale.v1` に永続化(幅と同じスカラー設定パターン)・適用範囲は3パネル(kind)共通。5段階プリセット0.85〜1.5、CSS変数 `--side-panel-font-scale` + calc() 方式。shared型変更不要) | [2026-07-18](worklog/issue-377.md) |
...
| #401 | Issue #401 ブロックハッシュの計算・親子連結・改ざん時の影響をインタラクティブに可視化する件の起票と`docs/PLAN.md`バックログ追記。ユーザーからの要望。親ハッシュによる連結自体は既にチェーンリボン(#298)で可視化済みだが、なぜそのハッシュになるのか(内容から計算される暗号学的理由)・どこで誰が計算するかの説明が無い。改ざんデモ的なインタラクション(重量・新機能)で進める方向でユーザーと合意。着手時はまずchainviz-uxによるUX設計を先行、Issue #402と一括検討 | [2026-07-18](worklog/issue-401.md) |
| #402 | Issue #402 トランザクション署名・PoS検証(attestation)の操作・処理をわかりやすく可視化する件の起票と`docs/PLAN.md`バックログ追記。ユーザーからの要望。Issue #401と同じテーマ。EOA署名は用語集・TxLifecyclePopoverで説明済みだが検証プロセス自体(誰が・どう検証するか)はUIに無く、collectorもecrecoverでの署名者復元はしていない。PoSのattestationも可視化対象外。着手時はまずchainviz-uxによるUX設計を先行、Issue #401と一括検討 | [2026-07-18](worklog/issue-402.md) |
| #406 | Issue #406 ハッシュ・署名デモの f(x) の入力(x)が不明瞭/keccak256の規格説明が無い件のUX設計(ux)。実機(モックデータ+Playwright)で両デモを操作して確認し、処理帯を2行構成にして「x = ブロック番号 | 親ブロックのハッシュ | データ」等の x 行を常時表示する設計、keccak256 用語集エントリの新設(hash拡充ではなく独立エントリ。SHA-3との違い・出力長・イーサリアムでの用途)、withTermAnchorによる文言中アンカー化、処理帯コンテナのaria-hidden解除(a11y)をまとめた。shared型変更不要。実装はchainviz-frontendが引き継ぐ | [2026-07-19](worklog/issue-406.md) |
| #410 | Issue #410 ワークベンチの操作パネルが他のツールチップ/ポップオーバーに隠れて操作できない件のUX設計(ux)。実機(モックデータ+Playwright)で再現し、原因を3点特定: (1)「操作を実行…」ボタンの予告ツールチップ(ActionHint)がパネルを開いた瞬間も表示されたまま重なる、(2)`OperationPanel`が`.infra-card`のDOM子要素のため操作パネル操作中も`mouseleave`が発火せず`InfraPopover`(ホバー詳細)が閉じ続けない、(3)用語解説ポップオーバー(z-index 30)が操作パネル(z-index 25)より前面に出て実際にポインタ操作をブロックする(Playwrightのactionabilityチェックで30秒タイムアウトするのを実測)。対応方針: 操作パネルが開いている間は`InfraPopover`を表示条件から外し(`hovered && !operationPanelOpen`)、ActionHintはパネルを開いた瞬間に明示的に閉じる。shared型変更不要。実装はchainviz-frontendが引き継ぐ | [2026-07-23](worklog/issue-410.md) |
| #412 | Issue #412 ブロックチェーンに対する一般的な攻撃手法を理解できるようにする件の案だし(designer)。実装はまだ行わず、①グロッサリー解説+既存可視化(フォーク色分け#296・チェーンリボン#298)との紐付けのみ、②実チェーン上での意図的な再現(既存観測機構を流用)、③#401/#402と同型のフロント内独立シミュレーション砂場、の3案を比較。攻撃手法(51%攻撃・ダブルスペンド・リオーグ・フロントランニング・ロングレンジ攻撃・eclipse攻撃)ごとの向き不向きも整理。#296のQA検証(issue-296.md)で判明した「host sudoを使うiptables方式のパーティション注入は本環境で実行不能」という実測結果を案2のリスクとして明記。GitHub Issue本文にも比較表を追記(`gh issue edit`)。1案には決定していない | [2026-07-24](worklog/issue-412.md) |
| #412 | Issue #412 の詳細設計(designer)。ユーザーが方針(案1を全6攻撃手法共通の土台に、案3を51%攻撃・ロングレンジ攻撃・eclipse攻撃の3手法に適用、リオーグ/ダブルスペンド/フロントランニングは解説のみ)を決定したのを受け実装可能な粒度まで具体化。`packages/shared`型変更は不要と判断(3砂場とも#401/#402と同型のフロント内完結)。glossary新規6語(`b-network.yaml`4語・`c-transaction.yaml`2語)とアンカー配置5箇所、サイドパネル新規kind3つ(`fiftyOnePercentAttackDemo`/`longRangeAttackDemo`/`eclipseAttackDemo`)を`attack-demo/`新規フォルダに設計。`docs/ARCHITECTURE.md`§17に反映。対象範囲が広いため土台+3砂場の4Issue分割を提案(実際の起票は統括に委ねる)。3砂場は個別の図解(分岐ツリー・並走2履歴・ピア包囲グラフ)が必要でchainviz-uxへの引き継ぎを推奨、土台はUX不要と判断 | [2026-07-24](worklog/issue-412.md) |
| #420 | Issue #420 バリデーターの動き自体をもっと可視化したい件の実現可能性調査(designer)。稼働中のprofiles/ethereum環境へ実際にcurlし、Beacon APIのブロックヘッダ(proposer_index)・duties API(予定)・liveness API(事後確認、エポック粒度)・lighthouse vcの`--metrics`フラグ存在(実測未確認)を確認。案A(Beacon API相関、node-env変更なし、validator_index→VC対応付けが未解決)・案B(VC Prometheusメトリクス新規スクレイプ、reth Engine APIと対称的な構造でattribution問題を回避、designer推奨)・案C(A+B併用)・案D(ログtail、不採用)を比較。活動パルスは既存の`NodeLinkActivity`/`InternalCallStats`を再利用でき`packages/shared`型変更不要と判断(「予定」まで見せるなら`NodeInternals`拡張が必要)。「内部で何をやっているか」は職務種別+実施確認+所要時間までが限界でステップ単位の内部処理復元は不可能であることも明記。GitHub Issue本文に転記。ユーザーが案Bを採用と決定後、designerが実装可能な粒度まで詳細設計(lighthouse-vc.shへの`--metrics`追加・docker-compose.yml変更不要・reth D層実装と対称的な新規collectorファイル4つ・validator_index→VCコンテナ対応付けは案Bでは不要と結論・frontendのshowsActivity反転とラベルテーブル新設)。メトリクス名はsigp/lighthouseのソース確認で暫定確定(実装時に実機再確認要)。`packages/shared`型変更なし。1 Issueのまま3パッケージ分担で実装可能と判断。`docs/ARCHITECTURE.md`§7.6.12に反映 | [2026-07-24](worklog/issue-420.md) |
