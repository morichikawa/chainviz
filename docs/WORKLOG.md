...
| #319 | Issue #319 ウォレットのtx履歴に各txのnonce値が表示されない件のfrontend側実装。設計担当がcherry-pick用に用意した`TransactionEntity.nonce?: number`(shared)を取り込み、`WalletPopoverTxItem`に`walletAddress` propを追加して送信tx限定(`tx.from`が自ウォレットと大文字小文字無視で一致、かつ`tx.nonce`観測済み)でnonceをhash直後・statusチップ前に表示。collector側の観測実装は別ブランチで並行作業中で未合流 | [2026-07-16](worklog/issue-319.md) |
