...
| #299 | Issue #299 A〜D層が同一キャンバスに共存し読み取りにくい課題のUX設計。既定の全層表示を維持したまま選択層以外を薄くする「レイヤーレンズ」(単一選択チップバー)を設計。要素→層の判定表・dim仕様・HUD/操作フィードバックの除外・ヘッダー文言「(A層)」の修正・用語`visualization-layers`追加・#298ブロック列はB/C両層扱い、を確定。shared/collector変更なし(frontendのみ)。CONCEPT.mdのレイヤー切り替え構想をレンズ方式として具体化 | [2026-07-12](worklog/issue-299.md) |
