...
| #303 | Issue #303 WorldStateStoreのTransactionEntity無制限蓄積の保持方針を設計(設計フェーズのみ・コード変更なし)。included/failed tx(blockHashあり)は対応blockのstore内存在に連動させ、`applyTransaction`の入口ガード(block不在なら取り込まない)+`applyBlock`の窓落ちblockに紐づくtxの退去で有界化(#298のBLOCK_RETENTION=32を番号窓として転用、shared型変更不要)。pending tx(blockHashなし)はblock eviction対象外にして「誤削除しない保証」を構造化し、件数上限`PENDING_TX_RETENTION=256`(安全弁)で有界化。前提=blockはincluded/failed txより先にstoreへ届く(subscribeBlocksは同期・handleBlockInclusionはreceipt取得の非同期)。frontendは`entityRemoved`汎用処理・`resolveWalletTransactions`の欠落hash除外で変更不要。実装はcollector担当。ARCHITECTURE.md §10.4参照 | [2026-07-13](worklog/issue-303.md) |
