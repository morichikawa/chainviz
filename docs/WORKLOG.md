...
| #244 | Issue #244 デプロイtxで発生したイベント(mintのTransfer等)が復号されず生チップ表示になる問題の原因調査・修正方針の設計。実機再現でカタログ登録(registerContractDeployment)がブロック取り込みより約45ms後着することを特定し、(A)handleBlockInclusion内の順序修正と(B)後着登録時のデプロイtxイベント再復号・再配信の2点を設計 | [2026-07-11](worklog/issue-244.md) |
