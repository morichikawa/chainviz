...
| #286 | Issue #286 長時間稼働スタックの短時間再起動でgenesisが古いまま再利用されbeaconが追いつき不能になる問題の対応設計。再生成判定の入力を「停止時間」から「genesis年齢(=再構築必要量)」に置き換え、稼働中かどうかはハートビートmtimeの前進をサンプリング観測して実測判別する方針を決定(設計のみ。実装はnode-envへ引き継ぎ) | [2026-07-11](worklog/issue-286.md) |
