...
| #285 | Issue #285 validatorがbeaconと結ばれず「浮いて見える」課題の設計。既存の`drivesNodeId`(駆動する側→される側の一般関係)を再利用しvalidator→beaconにも内部リンクエッジを描く方針を確定(ARCHITECTURE.md §7.6.11新設)。VCの実接続先を観測する経路が無いことを調査で確認し、beacon→rethと同じcomposeサービス名による静的解決を採用。shared型は構造変更なし(docstring一般化のみ)。実装はcollector/frontendへ引き継ぎ | [2026-07-11](worklog/issue-285.md) |
