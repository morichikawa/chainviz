...
| #251 | Issue #251 ノード追加ボタン付近の「reth+beaconペア追加」説明のUX設計。実機確認でペアの事実は#123のツールチップに既出と判明し、欠けていた「なぜペアか(The Merge以降の標準構成)」の一文とGlossaryTermアンカー(el-cl-separation)を既存ツールチップの2段目として追記する設計に決定(実装はfrontendへ引き継ぎ) | [2026-07-11](worklog/issue-251.md) |
