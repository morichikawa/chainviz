...
| #346 | Issue #346 UI層E2Eテストの一部が実.hover()依存・描画安定性不足でflakyになりうる件の起票と`docs/PLAN.md`バックログ追記のレビュー(合格。Issue本文がissue-322.mdのQA報告(4テストの個別再現・UI-B-06の対象外判断・slot非依存の切り分け・dispatchHoverへの寄せ等の期待対応)と過不足なく一致、参照事実(interactions.tsのdispatchHover・UI-D-02)の実在、PLAN.md追記のフォーマット一貫性、lint/build/test全通過を確認。実装着手は後日) | [2026-07-16](worklog/issue-346.md) |
...
| #351 | Issue #351 チェーンリボンの「親ブロック」行ホバー強調が実質使えない(ホバーが約200msで閉じる)件の起票と`docs/PLAN.md`バックログ追記のレビュー(合格。Issue本文とPLAN.md追記が過不足なく一致、参照事実(issue-298.mdの「既知の残課題」・issue-313.mdの報告記録)の実在、フォーマット一貫性、lint/build/test全通過を確認。実装着手は後日、着手時はchainviz-uxのUX設計を先行) | [2026-07-17](worklog/issue-351.md) |
| #352 | Issue #352 ノード間通信ログにRPC呼び出しのレスポンス(成否・所要時間)を追加する件の起票と`docs/PLAN.md`バックログ追記のレビュー(合格。Issue #317第1弾からの分割経緯・shared/collector変更を伴う理由・#317マージ後着手の依存関係がIssue本文と一致、参照事実(ブランチissue-317-comms-log-panel上のissue-317.md §8の実現案下書き)の実在、フォーマット一貫性、lint/build/test全通過を確認。実装着手は後日、着手時はchainviz-designerの設計を先行) | [2026-07-17](worklog/issue-352.md) |
...
| #359 | Issue #359 addNode/addWorkbenchで作成したmanagedコンテナがdocker compose down -vでも削除されない件の起票と`docs/PLAN.md`バックログ追記のレビュー(合格。Issue本文とPLAN.md追記が過不足なく一致、参照事実(ブランチissue-357-eoa-not-cleared-on-down上のissue-357.mdの実測記録と別Issue化推奨)の実在、フォーマット一貫性、コミット粒度、lint/build/test全通過を確認。実装着手は後日、実現方法は着手時に設計判断) | [2026-07-17](worklog/issue-359.md) |
| #362 | Issue #362 サイドパネル(コントラクトソース表示・用語集表示)の幅をリサイズできるようにする件の起票と`docs/PLAN.md`バックログ追記のレビュー(合格。Issue本文とPLAN.md追記が過不足なく一致、参照事実(ARCHITECTURE.md §12.2の「幅は400px目安」記載・styles.cssの`width: 420px`固定・SidePanel.tsxと3 kindの実在)の確認、フォーマット一貫性、コミット粒度、lint/build/test全通過を確認。実装着手は後日、リサイズハンドル・幅の永続化要否・最小/最大幅は着手時に設計判断) | [2026-07-17](worklog/issue-362.md) |
| #364 | Issue #364 サンプルコントラクトのトークンシンボル(CVZ等)がSolidityの定数でハードコードされておりデプロイ時に変更できない件の起票と`docs/PLAN.md`バックログ追記のレビュー(合格。Issue本文とPLAN.md追記が過不足なく一致、参照事実(ChainvizToken.solの`symbol = "CVZ"`定数と`initialSupply`のみのコンストラクタ、ChainvizNFT.solの`symbol = "CVN"`、catalog.json等のCVZ依存箇所)の実在、フォーマット一貫性、コミット粒度、lint/build/test全通過を確認。実装着手は後日、引数化か表記変更かは着手時に設計判断) | [2026-07-17](worklog/issue-364.md) |
...
| #371 | Issue #371 i18n translate()にObject.prototype由来キー(toString等)への防御が無い件の起票と`docs/PLAN.md`バックログ追記のレビュー(合格。Issue #341のレビュー中に見つかった軽微な堅牢性の指摘。Issue本文とPLAN.md追記が過不足なく一致、参照事実(i18n.tsのtranslate()に無ガードのmessages[key]参照・format()のhasOwnProperty ガード・呼び出し口がLanguageProvider.tsxの1箇所のみ)の実在、フォーマット一貫性、コミット粒度、lint/build/test全通過を確認。実装着手は後日) | [2026-07-17](worklog/issue-371.md) |
| #373 | Issue #373 UI-CMD-07: ワークベンチ削除ボタンがE2E上でstableにならないことがある(原因不明)件の起票(Issue #346から分割)と`docs/PLAN.md`バックログ追記・#346記載更新のレビュー(合格。Issue本文とPLAN.md追記が過不足なく一致、#346更新の裏付け(issue-346-e2e-hover-flakinessブランチのworklogにportal化locator問題・UI-ERR-02テスト追随漏れ・UI-CMD-07不再現の記録)の実在、フォーマット一貫性、lint/build/test全通過を確認。着手時はchainviz-detectiveによる独立合成環境での原因調査から。PLAN.mdの#346項目は両ブランチで文面が異なりマージ時コンフリクト見込み(本ブランチの文面を優先)) | [2026-07-17](worklog/issue-373.md) |
