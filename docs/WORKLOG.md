...
| #321 | Issue #321 コントラクトのソースコード表示の設計(designer)。カタログ(catalog.json)に`source`(fileName/language/code)を同梱し`ContractEntity.sourceCode?`(sharedに実装済み)へ転記、右ドックの汎用サイドパネル機構(`SidePanelView`判別共用体・排他1枚。#313/#317がkind追加で相乗り予定)を新設して表示。未知コントラクトは「バイトコードからソースは復元できない」を明示。ハイライトは表現セット内の自前軽量トークナイザ(ライブラリ不採用)。ARCHITECTURE.md §12新設 | [2026-07-16](worklog/issue-321.md) |
