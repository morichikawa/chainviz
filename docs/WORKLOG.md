...
| #409 | Issue #409 チェーンを遡れるようにしたい件の設計(designer)。本文が空だったためまず要望を解釈し、既存の類似要望(#317通信ログ・#298チェーンリボン・#320tx履歴)がいずれも「collectorが既に保持している範囲内で表示側の制限を緩める」という一貫した方針(無限の履歴を遡る機能には拡張しない。ARCHITECTURE.md §6.13)を取っていることから、今回もその路線を踏襲すると決定。collectorの保持窓(BLOCK_RETENTION=32、約6.4分)の中で任意のブロックの詳細を見られ前後に移動できる「ブロック詳細パネル」に絞り、保持窓外への新規RPC遡及取得・全チェーン検索UI(本格エクスプローラー)・セッションリプレイは対象外とした。packages/shared・collectorの変更は無く、frontendのみ(サイドパネル新kind`blockDetail`+チェーンリボンポップオーバーへの導線+前後ナビゲーション)で完結する設計。BLOCK_RETENTIONは今回変更しない(先回りしない判断)。1 Issueで完結する規模と判断しサブIssue分割はしなかった。初回QAでe2eテストがフレークし差し戻し、テスト安定化対応後に再レビュー・再QAとも合格 | [2026-07-23](worklog/issue-409.md) |
