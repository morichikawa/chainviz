...
| #274 | Issue #274 CLノード(beacon)の同期状態が永久に「同期中」(blockHeight 0)と表示されるギャップ(§7.3)の設計。Beacon API `/eth/v1/node/syncing`をD層ループで観測しsyncStatus(自己申告3フラグ)とblockHeight(=head_slot)を埋める方針を実測込みで確定。フロントはconsensus役割の高さ行ラベルを「ヘッドスロット」+用語`slot`に切り替える。shared型変更なし(コメント追記のみ)。実装はcollector/frontendへ引き継ぎ | [2026-07-11](worklog/issue-274.md) |
