...
| #298 | Issue #298 ブロックが連なって積み上がっていく様子の視覚表現のUX設計。実機評価で「パルスは一瞬で消え、ブロック高はホバー1行のみ、parentHashの連なりを見られる場所が無い」ギャップを特定し、チェーン全体で1本の常設「チェーンリボン」(直近Nタイル+着地アニメーション+ホバーで親ハッシュ連結の強調)として設計。#296(フォーク)とは独立成立とし先端の縦並置を接点として明記 | [2026-07-12](worklog/issue-298.md) |
