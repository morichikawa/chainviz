...
| #327 | Issue #327 UI全体に透明感・グラデーションを取り入れる要望の起票と`docs/PLAN.md`バックログ追記のレビュー(合格。PLAN.md追記のフォーマット一貫性・lint/build/test全通過を確認。Issue本文の「CLAUDE.mdの既存要望#32」は実際にはPLAN.mdバックログのクローズ済みIssueのため本文修正を推奨(非ブロッキング)。実装着手は後日) | [2026-07-15](worklog/issue-327.md) |
...
| #330 | Issue #330 mempool(未承認tx)全体を俯瞰できるビューの設計。既存データのみで成立させる方針(shared型変更なし・collector変更なし): `TransactionEntity`のstatus=pending抽出(ネットワーク集約・C層)を上段、`NodeEntity.internals.mempool`のノード別pending/queued(D層)を下段に束ねた常設ミニパネル(`ContractListPanel`と同型、0件でも常設表示)をfrontendに追加する。ノード選択式・#317のタブ機構先行実装は不採用。ARCHITECTURE.md §11参照 | [2026-07-16](worklog/issue-330.md) |
