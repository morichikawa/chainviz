...
describe("detectLaunchStatus", () => {
  it("listening ログが出ていれば listening と判定する", () => {
  ...
  it("別ポート宛の listening ログは無視する", () => {
    ...
    const status = detectLaunchStatus({
      logs: "[collector] WebSocket server listening on port 4000\n",
      port: 4123,
      exited: false,
      exitCode: null,
    });
...
describe("portInUseMessage", () => {
  it("原因（他プロセスとの同時実行の可能性）とポート番号を含む", () => {
    const msg = portInUseMessage(4123, "some logs");
