...
test("UI-A-02: カードにホバーすると詳細ポップオーバーが出る", async ({ page }) => {
  ...
  // InfraPopover は PopoverPortal 経由で document.body 直下へ portal 描画される
  // ため（Issue #245）、DOM 上は card の子孫にならない。card スコープの
  // `card.getByTestId(...)` では常に解決できず、page 直下から data-testid で
  // 特定する（Issue #346。node-internals UI-D-03 で判明した portal スコープ
  // 崩れと同一パターンで、こちらの UI-A-02 も同じ書き方のまま残っていた）。
  const popover = page.getByTestId(`infra-popover-${RETH1_ID}`);
...
test("UI-A-05: 用語のインライン解説が表示される", async ({ page }) => {
  await openFrontendWithCards(page);
  const card = page.getByTestId(`infra-card-${RETH1_ID}`);
  // 用語アンカー（glossary-term）はカードの子孫として描画されるため card
  // スコープで特定できる。
  const term: Locator = card.getByTestId("glossary-term-container");
  // 用語ポップオーバーは PopoverPortal 経由で document.body 直下へ portal
  // 描画されるため（Issue #245）、card の子孫にならない。page 直下から特定する
  // （Issue #346。UI-A-02 の infra-popover・node-internals UI-D-03 と同一の
  // portal スコープ崩れが同じ書き方のまま残っていた）。
  const popover: Locator = page.getByTestId("glossary-popover-container");
