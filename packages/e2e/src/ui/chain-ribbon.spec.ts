...
test("UI-B-05: チェーンリボンにブロックが連なって表示される", async ({ page }) => {
  ...
  await test.step(
    ...
    async () => {
      ...
      // ここでは実マウス経路の再現そのものが検証対象（隙間を横切る移動を
      // 経てもポップオーバーが開いたままかどうか）のため、座標非依存の
      // dispatchHover ではなく実マウス（page.mouse.move）を使う。
      const tile = page.locator(CHAIN_RIBBON_TILE_SELECTOR).last();
      const hash = (await tile.getAttribute("data-testid"))?.replace(
        "chain-ribbon-tile-",
        "",
      ...
      if (!tileBox || !rowBox) {
        throw new Error("chain ribbon tile/parent row has no bounding box");
      }
      // タイル中心 → 隙間 → ポップオーバーの「親ブロック」行中心、と
      // 複数ステップで移動し、実際のホバー経路（一足飛びのワープではなく
      // 連続した移動）を再現する。
  ...
  try {
    ...
    await test.step(
      ...
      async () => {
        ...
        // 再現・解消を確認済みの対処法）。表示窓（直近8タイル。slot time が
        // 12秒なら実時間で約96秒分に相当し、slot time を短くするほど窓も短く
        // なる）内に対象タイルが残っているうちに操作できるよう、この1操作だけを
