 ...
 * Issue #32。
 *
 * Issue #327 でキャンバス実背景に淡い色光のラジアルグラデーション（α上限
 * 0.10）を敷いたが、色光の中心は画面外（左上-8%・右下108%）にあり画面内は
 * これより暗いため、上記の混色コントラスト計算に用いた --bg 単色との差は
 * 実用上無視できる（docs/worklog/issue-327.md §4）。
 *
