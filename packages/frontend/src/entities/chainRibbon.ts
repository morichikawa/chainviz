 ...
 * （docs/worklog/issue-298.md §4.2）: ブロック産生間隔（slot 12秒。Issue #322で
 * 実チェーンの既定値に確定）×8件で1分半強の履歴が残り、「伝播パルスを
 * 見逃しても遡れる」体験に足りる最小限、という前提に基づく固定値。この前提
 * （slot間隔が概ね12秒程度のネット構成）が崩れる場合はこの値を見直すこと。
 */
