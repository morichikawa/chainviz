...
export interface OperationEdge {
  ...
  observedAt: number;
  /**
   * 呼び出しの成否（レスポンス観測。Issue #352）。ロギングプロキシが
   * 転送先のレスポンスから判定できた場合のみ入る（省略 = 判定不能。
   * 例: バッチ応答内に対応する要素が見つからない、応答ボディが解釈
   * 不能）。"ok" はプロトコル上の成功応答、"error" はエラー応答または
   * 転送自体の失敗。エラー応答の具体的な判定規則（JSON-RPC の error
   * フィールド等）は collector のプロキシ側の責務で、この型はプロトコル
   * 非依存の語彙だけを持つ（operation と同じ境界の考え方）。
   */
  outcome?: "ok" | "error";
  /**
   * 呼び出しの所要時間（ms、0 以上の整数）。プロキシがリクエストを
   * 受け取り終えてから転送先のレスポンスボディを受け取り終えるまで。
   * バッチリクエストでは 1 回の HTTP 往復を共有するため、同一バッチ内の
   * 全呼び出しが同じ値を持つ。観測できなかった場合は省略。
   */
  durationMs?: number;
}
